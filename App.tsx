...
import {
  buildDateReservationSteps,
  createDateReservationIntent,
  dateReservationStatusCopy,
  estimateDateReservationQuote,
  formatPaymentMoney,
  paymentsConfigured,
  stripePublishableKey,
  type DateReservationQuote,
  type DateReservationStatus,
} from "./src/services/payments";
import {
  ApplePayReservationButton,
  StripePaymentProvider,
  checkApplePaySupport,
  confirmApplePayReservation,
...
} from "./src/services/appPersistence";
import {
...
import {
  ...
  type GrowthNudge,
  type HomeGrowthLoop,
  type ProfileGrowthInput,
  type RetentionLoop,
} from "./src/domain/growth";
import {
  buildNetworkEffectPlan,
  type NetworkEffectPlan,
  type NetworkGrowthLoop,
} from "./src/domain/networkEffects";
import {
  annualSavingsLabel,
  billingPeriodLabel,
  ...
  sparkPacks,
  type BillingCycle,
  type PaymentEntitlementGate,
  type PaymentEntitlementSnapshot,
  type ProductKind,
} from "./src/domain/monetization";
import {
  buildReportActionPlan,
  buildSafetyChecklist,
  safetyReadinessScore,
  scanMessageSafety,
  type MessageSafetyScan,
  type SafetyChecklistItem,
} from "./src/domain/safety";
import {
  buildProductQualitySnapshot,
  type ProductQualityItem,
} from "./src/domain/productQuality";
import {
  buildInteractionAuditSnapshot,
  type InteractionAuditSnapshot,
} from "./src/domain/interactionQuality";
import {
  buildReleaseReadinessSnapshot,
  type ReleaseGate,
  type ReleaseReadinessSnapshot,
} from "./src/domain/releaseReadiness";
import {
  buildStoreReviewSnapshot,
  type StoreReviewSnapshot,
} from "./src/domain/storeReview";
import {
  buildPolicyComplianceSnapshot,
  type PolicyComplianceSnapshot,
} from "./src/domain/policyCompliance";
import {
  buildDateMarketplaceSnapshot,
  type DateMarketplaceSnapshot,
} from "./src/domain/dateMarketplace";
import {
  buildP1OperationsSnapshot,
  type P1OperationItem,
  type P1OperationsSnapshot,
} from "./src/domain/p1Operations";
import {
  buildMarketplaceOpsSnapshot,
  type MarketplaceOpsSnapshot,
} from "./src/domain/marketplaceOps";
import {
  buildTrustOpsSnapshot,
  type TrustOpsGate,
  type TrustOpsSnapshot,
} from "./src/domain/trustOps";
import {
  buildLegalStoreOpsSnapshot,
  type LegalStoreOpsGate,
  type LegalStoreOpsSnapshot,
} from "./src/domain/legalStoreOps";
import {
  buildBackendLaunchSnapshot,
  type BackendLaunchGate,
  type BackendLaunchSnapshot,
} from "./src/domain/backendReadiness";
import {
  buildNotificationReadinessSnapshot,
  type NotificationGate,
  type NotificationReadinessSnapshot,
} from "./src/domain/notificationReadiness";
import {
  buildGiftFulfillmentReadinessSnapshot,
  type GiftFulfillmentGate,
  type GiftFulfillmentReadinessSnapshot,
} from "./src/domain/giftFulfillmentReadiness";
import {
  buildPlacesReservationReadinessSnapshot,
  type PlacesReservationGate,
  type PlacesReservationReadinessSnapshot,
} from "./src/domain/placesReservationReadiness";
import {
  buildObservabilityReadinessSnapshot,
  type ObservabilityGate,
  type ObservabilityReadinessSnapshot,
} from "./src/domain/observabilityReadiness";
import {
  buildAbuseFraudReadinessSnapshot,
  type AbuseFraudGate,
  type AbuseFraudReadinessSnapshot,
} from "./src/domain/abuseFraudReadiness";
...
function isIcebreakerWaitingForOtherAnswer(data: unknown) {
  return (
    ...
    typeof data === "object" &&
    !Array.isArray(data) &&
    (data as { unlocked?: unknown }).unlocked === false
  );
}
...
function DestinyOneApp() {
  ...
  const [coinBalance, setCoinBalance] = useState(
    initialPersistedState.coinBalance,
  );
  ...
  useEffect(() => {
    ...
    loadAppState().then((saved) => {
      ...
      setTimeout(() => {
        if (active) {
          setScreen(saved.onboardingComplete ? "home" : "welcome");
          setHydrated(true);
        }
      }, remaining);
    });
  ...
  useEffect(() => {
    ...
    void fetchPersistedChatMessages(matchId).then((messages) => {
      ...
      setChatMessages((current) => ({
        ...current,
        [matchId]: mergeChatMessageList(current[matchId] ?? [], messages),
      }));
    });
  ...
  const notifyProfileView = (match: Match) =>
  ...
  const appendChatMessage = (matchId: string, message: ChatMessage) => {
    ...
    if (message.type === "date" && message.date)
      void persistDateProposal(matchId, message.date);
    void persistChatMessage(matchId, message).then((result) => {
      if (!result.saved || !isChatMessage(result.data)) return;
      setChatMessages((current) => ({
        ...current,
        [matchId]: (current[matchId] ?? []).map((existing) =>
          existing.id === message.id ? (result.data as ChatMessage) : existing,
        ),
      }));
    });
  };
  ...
  return (
    <SafeAreaProvider>
      ...
      <View style={shared.screen}>
        ...
        {screen === "auth" && (
          <Auth
            onNext={(destination, skipOtp, password) => {
              setAuthDestination(destination);
              setAuthPassword(password ?? "");
              setScreen(skipOtp ? "verify" : "otp");
            }}
        ...
        {screen === "circle" && (
          <TrustedCircle
            ...
            onAddVouch={(quality) => {
              if (vouches.length < 3 && !vouches.includes(quality)) {
        ...
        {screen === "discovery" && (
          <DiscoveryCenter
            ...
            crossedPaths={crossedPaths}
            onSmartChange={setSmartDiscovery}
            onCrossedChange={setCrossedPaths}
            onClear={() => setDiscoverySignals([])}
            onBack={() => setScreen("home")}
          />
        )}
        {screen === "coach" && (
          ...
          <VerificationHub
            ...
            onVerify={() => {
              ...
              setAppNotice({
                ...
                body: "Selfie verification is marked complete in this preview. Production will connect liveness and ID providers.",
        ...
        {screen === "chat" && (
          <Chat
            ...
            settings={
              chatSettings[selected.id] ?? {
                nickname: "",
                theme: "Ruby Velvet",
              }
            }
            initialDraft={chatDrafts[selected.id] ?? ""}
            onDraftConsumed={() =>
            ...
            roseAvailability={roseAvailability}
            onRose={() => openRose(selected)}
            onSend={(message) => appendChatMessage(selected.id, message)}
            onSpendCoins={(coins) =>
        ...
        {screen === "datePlan" && (
          <DatePlanner
            ...
            onSend={(message) => {
              appendChatMessage(selected.id, message);
              setScreen("chat");
            }}
          />
        )}
        ...
        {screen === "support" && (
          <SupportCenter onBack={() => setScreen("profile")} />
        )}
        {screen === "pricing" && (
          <Pricing
            ...
            onBuyRoses={(amount = 5) => {
              ...
              setAppNotice({
                title: "Spark pack added",
                body: `Preview pack added ${amount} Golden Sparks. Production uses Apple/Google in-app billing and restore purchase.`,
                icon: "sparkles",
                tone: "gold",
              });
            }}
        ...
        <RoseComposer
          ...
          onSend={(note) => {
            if (roseTarget) sendRose(roseTarget, note);
            setRoseTarget(null);
          }}
        />
        <RoseReceivedPopup
        ...
        <SafetyActions
          ...
          onUnmatch={() => {
            ...
            setAppNotice({
              title: "Unmatched",
              body: `${selected.name} has been removed from this preview deck and conversation flow.`,
...
export default function App() {
  ...
  return (
    <StripePaymentProvider
      ...
      merchantIdentifier="merchant.com.destinyone.app"
    >
      {app}
    </StripePaymentProvider>
  );
...
function Splash() {
  ...
  return (
    <LinearGradient
      colors={["#030001", "#120004", "#260008"]}
      locations={[0, 0.56, 1]}
      style={styles.center}
    >
      <View style={launchStyles.velvetGlowTop} />
      <View style={launchStyles.velvetGlowBottom} />
      <Animated.View
        style={[
          launchStyles.cleanHalo,
          { opacity: glowOpacity, transform: [{ scale }] },
        ]}
      />
      <Animated.View
        style={[
          launchStyles.logoFrame,
          { transform: [{ translateY }, { scale }] },
        ]}
      >
        <LinearGradient
          colors={["#FFFFFF", "#D4AF37", "#9E001E"]}
          style={launchStyles.logoRing}
        >
          <View style={launchStyles.logoWell}>
            <Image
              source={destinyOneLogo}
              resizeMode="contain"
              style={launchStyles.preloadLogo}
            />
          </View>
        </LinearGradient>
      </Animated.View>
      <Text style={launchStyles.preloadBrand}>
        Destiny<Text style={launchStyles.preloadBrandOne}>One</Text>
      </Text>
      <Text style={[styles.tagline, launchStyles.script]}>
        For something real.
      </Text>
      <Animated.Text
        style={[
          launchStyles.preloadLine,
          { opacity: wordsOpacity, transform: [{ translateY: wordsY }] },
        ]}
      >
        Slow down. Choose better. Meet with intention.
      </Animated.Text>
      <Text style={launchStyles.preloadMood}>
        Serious dating, curated softly.
      </Text>
      <View style={launchStyles.preloadPromise}>
        <View style={launchStyles.promiseDot} />
        <Text style={launchStyles.preloadPromiseText}>VERIFIED</Text>
        <View style={launchStyles.promiseDot} />
        <Text style={launchStyles.preloadPromiseText}>PRIVATE</Text>
        <View style={launchStyles.promiseDot} />
        <Text style={launchStyles.preloadPromiseText}>INTENTIONAL</Text>
      </View>
      <View style={launchStyles.preloadTrack}>
        <Animated.View
          style={[
            launchStyles.preloadFill,
...
function Welcome({ onNext }: { onNext: () => void }) {
  return (
    <LinearGradient
      colors={["#210006", colors.black, "#310009"]}
      locations={[0, 0.58, 1]}
      style={{ flex: 1 }}
    >
      ...
      <SafeAreaView style={shared.safe}>
        <View style={styles.welcomeTop}>
          ...
          <View style={styles.memberPill}>
            ...
            <Text style={styles.memberText}>Intentional dating</Text>
          </View>
        </View>
        <View style={styles.welcomeArt}>
          ...
          <View
            style={[
              styles.photoMini,
              { transform: [{ rotate: "-8deg" }], left: 25 },
            ]}
          >
            <Image source={{ uri: matches[1]!.photo }} style={styles.fill} />
          </View>
          <View
            style={[
              styles.photoMini,
              { transform: [{ rotate: "8deg" }], right: 25, top: 55 },
            ]}
          >
            <Image source={{ uri: matches[0]!.photo }} style={styles.fill} />
          </View>
          <View style={styles.heart}>
            <PremiumIcon name="heart" tone="ruby" size={54} iconSize={26} />
          </View>
          <View style={styles.valueTag}>
            <MiniPremiumIcon name="heart" tone="ruby" size={24} iconSize={11} />
            <Text style={styles.valueTagText}>Family first</Text>
          </View>
        </View>
        <View style={{ gap: 14 }}>
          <SectionTitle
            eyebrow="Made for meaningful beginnings"
            title="Meet. Match. Build something real."
            body="A curated community of Indians in the USA, here for relationships with intention."
          />
          <View style={launchStyles.trustRibbon}>
            <TrustPoint icon="shield-checkmark" label="Verified" />
            <TrustPoint icon="heart" label="Intentional" />
            <TrustPoint icon="lock-closed" label="Private" />
          </View>
          <View style={{ gap: 10, marginTop: 4 }}>
            <Button label="Get Started" icon="arrow-forward" onPress={onNext} />
            <Button
              variant="ghost"
...
function TrustPoint({
  ...
  return (
    <View style={launchStyles.trustPoint}>
      <PremiumIcon
        name={icon}
        tone={
          label === "Private" ? "dark" : label === "Verified" ? "gold" : "ruby"
        }
        size={24}
        iconSize={12}
      />
      <Text style={launchStyles.trustLabel}>{label}</Text>
    </View>
  );
}
...
const premiumIconPalettes: Record<
  PremiumIconTone,
  { colors: [string, string, string]; glow: string; icon: string }
> = {
  ruby: {
    colors: ["#C91638", "#7B0D20", "#1A0307"],
    glow: "rgba(229,9,47,.50)",
    icon: "#FFF8F4",
  },
  gold: {
    colors: ["#E8C76A", "#A77E19", "#2A1D07"],
    glow: "rgba(212,175,55,.42)",
    icon: "#1B0905",
  },
  plum: {
    colors: ["#5E28A8", "#351149", "#120018"],
    glow: "rgba(122,31,224,.42)",
    icon: "#FFF8F4",
  },
  rose: {
    colors: ["#E55A70", "#A50D2B", "#27040B"],
    glow: "rgba(255,110,128,.38)",
    icon: "#FFF8F4",
  },
  dark: {
    colors: ["#3A3035", "#201219", "#070002"],
...
function MiniPremiumIcon({
  ...
  return (
    <PremiumIcon name={name} tone={tone} size={size} iconSize={iconSize} />
  );
}
...
function Auth({
  ...
  onNext: (destination: string, skipOtp?: boolean, password?: string) => void;
  onBack: () => void;
}) {
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "phone") {
        await beginAuthentication({ mode: "phone", phone });
        onNext(phone);
      } else {
        ...
        onNext(cleanEmail, false, password);
      }
    } catch (error) {
      setAuthError(
  ...
  const socialLogin = async (provider: "Apple" | "Google" | "LinkedIn") => {
    setLoading(true);
    setAuthError("");
    setSocialStatus("");
    if (provider === "Google") {
      try {
        setSocialStatus("Opening Google sign-in…");
        const result = await signInWithGoogle();
        if (result.redirecting) {
          return;
        }
        setLoading(false);
        onNext("google-account", true);
      } catch (error) {
        setLoading(false);
        setSocialStatus("");
        setAuthError(
    ...
    setTimeout(() => {
      setLoading(false);
      onNext(`${provider.toLowerCase()}@destinyone.preview`, true);
    }, 450);
  };

  return (
    <FormPage back={onBack} step={1}>
      <SectionTitle
        eyebrow="Your invitation"
        title="Let’s get to know you."
        body="Join with your phone or email. We keep your details private."
      />
      <View style={authStyles.socialGrid}>
        ...
        >
          <MiniPremiumIcon
            ...
            iconSize={15}
          />
          <Text style={authStyles.socialText}>Apple ID</Text>
        </Pressable>
        <Pressable
          disabled={loading}
        ...
        >
          <MiniPremiumIcon
            ...
            iconSize={15}
          />
          <Text style={authStyles.socialText}>Gmail</Text>
        </Pressable>
        <Pressable
          disabled={loading}
          onPress={() => socialLogin("LinkedIn")}
          style={authStyles.socialButton}
        >
          <MiniPremiumIcon
            name="logo-linkedin"
            ...
            iconSize={15}
          />
          <Text style={authStyles.socialText}>LinkedIn</Text>
        </Pressable>
      </View>
      {!!socialStatus && (
        ...
        {mode === "phone" ? (
          <>
            ...
            <Text style={styles.helper}>
              {backendMode === "demo" || allowsPreviewOtpFallback
                ? "Preview mode: real Supabase is connected, and demo OTP 123456 also works."
          ...
          <>
            ...
            <Field
              ...
              error={
                submitted && !passwordValid ? "Use at least 8 characters." : ""
              }
            />
            <Text style={styles.helper}>
              We’ll email a real 6-digit verification code before your profile
              opens. Demo OTP is disabled for email.
            </Text>
          </>
        )}
      </View>
...
function Otp({
  ...
  const allowPreviewCode =
    !isEmail && (backendMode === "demo" || allowsPreviewOtpFallback);
  const verify = async () => {
    setLoading(true);
    setError("");
    try {
      const valid = await verifyAuthentication(destination, code, password);
      if (valid) onVerified();
      else
        setError(
          allowPreviewCode
            ? "That code doesn’t match. Try preview code 123456 or 12345."
            : "That code doesn’t match. Please use the real email code.",
        );
    } catch (error) {
      setError(
        error instanceof Error
  ...
  const resend = async () => {
    ...
    try {
      if (isEmail) {
        ...
        await beginAuthentication({ mode: "phone", phone: destination });
      }
    } catch (error) {
      setError(
  ...
  return (
    <FormPage back={onBack}>
      <SectionTitle
        ...
        body={`We sent a 6-digit code to ${masked}.`}
      />
      <View style={{ gap: 12 }}>
        <TextInput
          autoFocus
        ...
        {error ? (
          ...
          <Text style={styles.demoHint}>
            Phone preview code: 123456 or 12345
          </Text>
        ) : isEmail ? (
          <Text style={styles.helper}>
            Email requires the real 6-digit Supabase code. If your email only
            shows a link, update the Supabase Magic Link template to include{" "}
            {"{{ .Token }}"}.
          </Text>
        ) : (
          <Text style={styles.helper}>
            Production SMS verification is active.
          </Text>
        )}
      </View>
      <Pressable
      ...
      >
        ...
        <Text style={[styles.helper, { flex: 1 }]}>
          Your contact details stay private and are never shown on your profile.
        </Text>
      </View>
      <View style={shared.spacer} />
      <Button
  ...
  return (
    <FormPage step={2}>
      <SectionTitle
        ...
        title="A safer place to meet."
        body="Add a clear verification photo from your gallery first. Camera is optional if you want to retake."
      />
      <View
        style={[
          shared.card,
          { alignItems: "center", gap: 14, paddingVertical: 24 },
        ]}
      >
        ...
        <Text style={styles.cardTitle}>
          {verified ? "You’re verified" : "Photo verification"}
        </Text>
        <Text style={[shared.body, { textAlign: "center" }]}>
          {verified
            ? "Your profile now shows the Verified Member badge."
            : "Choose a clear face photo. This private verification photo won’t appear on your profile."}
        </Text>
        <View style={{ width: "100%", gap: 10 }}>
          <Button
            variant={verified ? "gold" : "secondary"}
            label={verified ? "Choose another photo" : "Choose from gallery"}
            onPress={pickVerificationPhoto}
            icon="images"
          />
          <Button
            variant="ghost"
      ...
      <Pressable onPress={() => void pickGovernmentId()} style={styles.upload}>
        <MiniPremiumIcon
          name={idUri ? "checkmark-circle" : "id-card-outline"}
          tone={idUri ? "gold" : "dark"}
          size={38}
          iconSize={18}
        />
        <View style={{ flex: 1 }}>
          <Text style={shared.label}>
            {idUri ? "Government ID selected" : "Add a government ID"}
          </Text>
          <Text style={styles.helper}>
            {idUri
              ? "Private preview file selected · backend ID verification connects later"
              : "Optional · strengthens trust"}
          </Text>
        </View>
        <MiniPremiumIcon
          name={idUri ? "images" : "chevron-forward"}
          tone={idUri ? "gold" : "dark"}
          size={30}
          iconSize={14}
        />
      ...
      <View style={shared.spacer} />
      <Button label="Continue" disabled={!verified} onPress={onNext} />
    </FormPage>
  );
}
...
function ProfileSetup({
  ...
  const citySuggestions = profileCities
    .filter((item) =>
  ...
  return (
    <FormPage step={3} scroll>
      <SectionTitle
        ...
        body="Just enough to make a thoughtful first impression."
      />
      <View style={{ gap: 9 }}>
        <Text style={shared.label}>Add 3 recent photos</Text>
        <View style={styles.photoRow}>
          {[0, 1, 2].map((index) => (
            ...
            >
              {photos[index] ? (
                <Image source={{ uri: photos[index] }} style={styles.fill} />
              ) : (
                <>
                  <MiniPremiumIcon
                    name="add"
                    ...
                    iconSize={18}
                  />
                  <Text style={mediaStyles.addPhotoText}>Add</Text>
                </>
              )}
      ...
      <View style={{ gap: 16 }}>
        ...
        <View style={styles.twoCol}>
          <View style={{ flex: 1 }}>
            <Field
              ...
              onChangeText={(text: string) =>
                updateProfile("age", text.replace(/\D/g, "").slice(0, 2))
              }
              error={
                profile.age && !ageEligible
                  ? "DestinyOne is currently for ages 25–35."
                  : ""
              }
            />
          </View>
          <View style={{ flex: 1 }}>
            <Field
              ...
              onChangeText={(text: string) => updateProfile("height", text)}
            />
          </View>
        </View>
        <View style={{ gap: 8 }}>
          ...
          <View style={selectorStyles.searchBox}>
            ...
            <TextInput
              ...
              onChangeText={(text) => {
                setCityQuery(text);
                if (text !== profile.city) updateProfile("city", "");
              }}
          ...
          {!!profile.city && (
            <View style={selectorStyles.selectedPill}>
              <MiniPremiumIcon
                name="checkmark"
                tone="gold"
                size={24}
                iconSize={11}
              />
              <Text style={selectorStyles.selectedText}>{profile.city}</Text>
            </View>
          )}
          {!!cityQuery && (
            <View style={selectorStyles.suggestionPanel}>
              {citySuggestions.length ? (
                citySuggestions.map((item) => (
                  <Pressable
                    key={item}
                    onPress={() => {
                      updateProfile("city", item);
                      setCityQuery(item);
                    }}
                    style={selectorStyles.suggestionRow}
                  >
                    <Text style={selectorStyles.suggestionText}>{item}</Text>
                    <MiniPremiumIcon
                ...
                <Text style={styles.helper}>
                  Type a major USA or Canada city. You can add more cities later
                  in preferences.
                </Text>
              )}
            </View>
        ...
        <Field
          ...
          onChangeText={(text: string) => updateProfile("profession", text)}
        />
        <View style={{ gap: 8 }}>
            ...
            {religions.map((option) => (
              <Pressable
                ...
                style={[
                  ...
                  profile.religion === option && selectorStyles.religionChipOn,
                ]}
              >
                <Text
                  style={[
                    selectorStyles.religionText,
                    profile.religion === option && { color: colors.ivory },
                  ]}
                >
                  {option}
                </Text>
        ...
        <Field
          ...
          placeholder="Punjabi, Gujarati, Telugu, mixed..."
          value={profile.community}
          onChangeText={(text: string) => updateProfile("community", text)}
        />
      </View>
    ...
    >
      ...
      <SafeAreaView style={chatStyles.sheet}>
        <SheetHeader
          ...
          subtitle="Choose a clear, recent photo for serious matches."
          onClose={onClose}
        />
        <View style={mediaStyles.photoChoiceHero}>
          <PremiumIcon name="images" tone="gold" size={54} iconSize={25} />
          <View style={{ flex: 1 }}>
            <Text style={styles.cardTitle}>Profile photos build trust.</Text>
            <Text style={styles.helper}>
              Use bright, recent photos where your face is easy to recognize.
            </Text>
          </View>
        </View>
        <View style={mediaStyles.photoChoiceGrid}>
          ...
          <Pressable onPress={onGallery} style={mediaStyles.photoChoice}>
            ...
            <Text style={mediaStyles.photoChoiceBody}>Choose from library</Text>
          </Pressable>
        </View>
        <Text style={styles.legal}>
          Photos stay in preview storage until backend upload is connected.
        </Text>
      </SafeAreaView>
    </Modal>
  );
  ...
  return (
    <View style={mediaStyles.voiceRecorder}>
      <View style={shared.row}>
        ...
        />
        <View style={{ flex: 1 }}>
          <Text style={styles.cardTitle}>Voice introduction</Text>
          <Text style={styles.helper}>
            {recorderState.isRecording
              ? `Recording · 0:${String(duration).padStart(2, "0")} / 0:30`
              : uri
                ? "Ready to help your profile feel human"
  ...
  return (
    <FormPage step={4} scroll>
      <View style={vibeStyles.hero}>
        <Text style={styles.kicker}>{value.length} OF 5 SELECTED</Text>
        <Text style={[shared.h1, { textAlign: "center" }]}>
          Pick the energy people should feel from you.
        </Text>
        <Text style={[shared.body, { textAlign: "center" }]}>
          Choose exactly five. These help the algorithm surface people with
          similar life rhythm and values.
        </Text>
      ...
      <View style={styles.vibeGrid}>
        {vibes.map((v, i) => (
          <Pressable
            onPress={() => toggle(v)}
            key={v}
            style={[styles.vibeCard, value.includes(v) && styles.vibeSelected]}
          >
            <PremiumIcon
              name={vibeIcons[i] ?? "heart"}
              tone={value.includes(v) ? "gold" : "plum"}
              size={48}
              iconSize={22}
            />
            <Text
              style={[
                styles.vibeText,
                value.includes(v) && { color: colors.ivory },
              ]}
            >
              {v}
            </Text>
            <Text style={vibeStyles.vibeMicro}>
              {value.includes(v) ? "Added to your lens" : "Tap to choose"}
            </Text>
            {value.includes(v) && (
      ...
      <View style={vibeStyles.tipCard}>
        <MiniPremiumIcon name="sparkles" tone="gold" size={36} iconSize={17} />
        <Text style={[styles.helper, { flex: 1 }]}>
          Tip: Pick who you are on a normal day—not who sounds impressive.
          Better inputs mean better matches.
        </Text>
      </View>
      <Button
        label={value.length === 5 ? "Looks like me" : "Choose 5 to continue"}
...
function Intent({
  ...
  const options = [
    {
      ...
      description: "A committed partnership with a shared future",
      icon: "heart-outline" as const,
    },
    {
      title: "Marriage",
      description: "Actively looking for a life partner",
      icon: "diamond-outline" as const,
    },
    {
      title: "Long-term, leading to Marriage",
      description: "Build the relationship thoughtfully, with marriage in mind",
  ...
  return (
    <FormPage step={5}>
      <SectionTitle
        eyebrow="Commitment only"
        title="What are you ready to build?"
        body="Every person on DestinyOne is here for a committed relationship or marriage—never casual dating."
      />
      <View style={styles.seriousPromise}>
        ...
        <Text style={styles.seriousPromiseText}>
          Casual, short-term and hookup intentions are not supported.
        </Text>
      </View>
      <View style={{ gap: 12 }}>
        {options.map(({ title, description, icon }) => (
          <Pressable
            ...
            style={[styles.intent, value === title && styles.intentSelected]}
          >
            <PremiumIcon
              name={icon}
              tone={value === title ? "gold" : "plum"}
              size={48}
              iconSize={22}
            />
            <View style={{ flex: 1 }}>
            ...
            <View style={[styles.radio, value === title && styles.radioOn]}>
              {value === title && <View style={styles.radioDot} />}
            </View>
          </Pressable>
        ))}
      </View>
      <View style={shared.spacer} />
      <Button
        label="Continue to life alignment"
        icon="arrow-forward"
        onPress={onNext}
      />
    </FormPage>
...
const alignmentQuestions = [
  {
    ...
    title: "When would marriage feel right?",
    options: [
      "Within 1–2 years",
      "Within 2–3 years",
      "When the right relationship is ready",
    ],
  },
  {
    key: "children",
    eyebrow: "FAMILY PLANS",
    title: "How do you feel about children?",
    options: [
  ...
  },
  {
    ...
    title: "What balance feels right?",
    options: [
  ...
  {
    key: "relocation",
    eyebrow: "BUILDING A HOME",
    title: "Would you relocate for the right person?",
    options: [
      "Yes, I’m open",
...
function Alignment({
  ...
  const choose = (option: string) =>
    onChange({ ...value, [current.key]: option });
  const advance = () =>
  ...
  return (
    <FormPage step={6}>
      <View style={alignmentStyles.hero}>
        ...
        <Text style={[shared.body, { textAlign: "center" }]}>
          Small answers, big clarity. We use this to match future plans—not to
          judge anyone.
        </Text>
      </View>
        ...
        {current.options.map((option, index) => (
          <Pressable
            ...
            style={[
              ...
              selected === option && styles.intentSelected,
            ]}
          >
            <MiniPremiumIcon
              name={
              ...
              tone={selected === option ? "gold" : "rose"}
              size={38}
              iconSize={18}
            />
            <View style={{ flex: 1 }}>
              <Text style={styles.answerText}>{option}</Text>
              <Text style={alignmentStyles.answerSub}>
                {index === 0
                  ? "Clear and intentional"
                  : index === 1
                    ? "Warm with room to breathe"
                    : "Honest about your rhythm"}
              </Text>
            </View>
            <MiniPremiumIcon
              name={
              ...
              tone={selected === option ? "gold" : "dark"}
              size={32}
              iconSize={15}
            />
          </Pressable>
      ...
      <Text style={[styles.helper, { textAlign: "center" }]}>
        You can update these privately any time. No percentage scores are shown
        to members.
      </Text>
      <View style={shared.spacer} />
      <Button
...
function HomeClean({
  ...
  const growth = buildHomeGrowthLoop({
  ...
  const networkPlan = buildNetworkEffectPlan({
    matches,
    selectedCities: preferences.filters.cities,
    verified: profileGrowth.verified,
    vouchesCount: profileGrowth.vouchesCount,
  });
  const primaryNudge = growth.nudges[0];
  const notificationIdea = growth.notificationIdeas[0];
  const openNudge = (nudge: GrowthNudge) => {
    if (nudge.actionScreen === "detail" && featured) {
      openDetail(featured);
      return;
    }
  ...
  const openRetention = (loop: RetentionLoop) => {
    if (loop.actionScreen === "detail" && featured) {
      openDetail(featured);
      return;
    }
    navigate(loop.actionScreen as Screen);
  };
  const openNetworkLoop = (loop: NetworkGrowthLoop) =>
    navigate(loop.actionScreen as Screen);
  const preferenceChips = [
    preferences.intent || "Marriage minded",
    preferences.filters.lookingFor,
    `${preferences.filters.minAge}-${preferences.filters.maxAge}`,
  ];
  return (
    <LinearGradient
      colors={["#23030A", "#0B0104", colors.black]}
      style={{ flex: 1 }}
    >
      <SafeAreaView
      ...
      >
        <View style={homeCleanStyles.header}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={homeCleanStyles.headerSub}>
              Indian, Punjabi & American profiles sorted by your real
              preferences.
            </Text>
          </View>
        ...
        <ScrollView
          contentContainerStyle={homeCleanStyles.content}
        ...
        >
          <View style={homeCleanStyles.hero}>
            ...
            <View style={homeCleanStyles.heroTop}>
              <View style={homeCleanStyles.roseSeal}>
                <PremiumIcon
                  name="sparkles"
                  tone="gold"
                  size={52}
              ...
              <View style={{ flex: 1 }}>
                <Text style={homeCleanStyles.script}>
                  Today’s intentional picks
                </Text>
                <Text style={homeCleanStyles.heroBody}>
                  {growth.deckHealth.body}
                </Text>
              </View>
            </View>
            <View style={homeCleanStyles.chipWrap}>
              {preferenceChips.map((chip) => (
                <View key={chip} style={homeCleanStyles.cleanChip}>
                  <Text style={homeCleanStyles.cleanChipText}>{chip}</Text>
                </View>
              ))}
            </View>
            <View style={homeCleanStyles.statsRow}>
              <View>
                <Text style={homeCleanStyles.statNumber}>
                  {retention.dailyMatches}
                </Text>
                <Text style={homeCleanStyles.statLabel}>daily picks</Text>
              </View>
              <View style={homeCleanStyles.statLine} />
              <View style={homeCleanStyles.statBlock}>
                <Text style={homeCleanStyles.statWord}>
                  {growth.deckHealth.label}
                </Text>
                <Text style={homeCleanStyles.statLabel}>deck health</Text>
              </View>
              <View style={homeCleanStyles.statLine} />
              <View style={homeCleanStyles.statBlock}>
                <Text style={homeCleanStyles.statWord}>
                  {growth.profile.stage}
                </Text>
                <Text style={homeCleanStyles.statLabel}>profile stage</Text>
              </View>
            </View>
          </View>
          ...
          {primaryNudge && (
            <GrowthNudgeCard
              nudge={primaryNudge}
              onPress={() => openNudge(primaryNudge)}
            />
          )}
          <View style={growthLoopStyles.metricGrid}>
            <GrowthLoopMetric
              ...
              body={
                retention.dailyMatches
                  ? `${retention.dailyMatches} curated profiles today. Come back tomorrow for a refreshed deck.`
                  : "Widen filters to restore daily picks."
              }
            />
            <GrowthLoopMetric
              ...
              title="Trust lift"
              body={
                growth.profile.missing[0] ??
                "Profile is ready for higher-intent intros."
              }
            />
            <GrowthLoopMetric
              icon="notifications-outline"
              title="Smart reminder"
              body={retention.profileViewNotification.title}
            />
          </View>
          ...
          <View style={coachStyles.homeGrid}>
            ...
            >
              ...
              <Text style={coachStyles.homeBody}>
                Bio, first messages, red flags and post-date feedback — helpful,
                not fake.
              </Text>
            </Pressable>
            <Pressable
            ...
            >
              ...
              <Text style={coachStyles.homeBody}>
                Chai mixers, video speed dates and community nights for serious
                singles.
              </Text>
            </Pressable>
            <Pressable
            ...
            >
              ...
              <Text style={coachStyles.homeBody}>
                Invite-only matchmaking for founders, business owners and
          ...
          <View style={coachStyles.trustStrip}>
            <TrustSignal
              icon="shield-checkmark"
              title="Safety-first"
              body="Verified profiles, report/block, check-ins and privacy controls."
            />
            <TrustSignal
              icon="eye-off"
              title="No spying"
              body="AI ranks using only filters, profile views, likes and skips inside the app."
            />
          </View>
          {notificationIdea && (
            <View style={growthLoopStyles.notificationCard}>
              <MiniPremiumIcon
              ...
              />
              <View style={{ flex: 1 }}>
                <Text style={growthLoopStyles.notificationTitle}>
                  {notificationIdea.title}
                </Text>
                <Text style={growthLoopStyles.notificationBody}>
                  {notificationIdea.body}
                </Text>
              </View>
            </View>
          )}
          ...
          {featured && (
            <View style={homeCleanStyles.featuredWrap}>
              <View style={homeCleanStyles.sectionRow}>
                <Text style={styles.sectionLabel}>FEATURED FIRST</Text>
                <Text style={homeCleanStyles.sectionHint}>
                  Swipe, tap, or send a Spark
                </Text>
              </View>
              <MatchCard
                match={featured}
                reasons={matchReasons(featured, preferences)}
          ...
          {rest.length > 0 && (
            <View style={homeCleanStyles.sectionRow}>
              <Text style={styles.sectionLabel}>MORE COMPATIBLE PROFILES</Text>
              <Text style={homeCleanStyles.sectionHint}>
                {rest.length} more
              </Text>
            </View>
          ...
          {rest.map((match) => (
            <MatchCard
              key={match.id}
              match={match}
              reasons={matchReasons(match, preferences)}
              onPress={() => openDetail(match)}
              onInterested={() => onInterested(match)}
              onSkip={() => onSkip(match)}
              onRose={() => onRose(match)}
            />
          ))}
...
function GrowthNudgeCard({
  ...
  return (
    <Pressable onPress={onPress} style={growthLoopStyles.nudgeCard}>
      <PremiumIcon
      ...
      <View style={growthLoopStyles.nudgeAction}>
        <Text style={growthLoopStyles.nudgeActionText}>
          {nudge.actionLabel}
        </Text>
      </View>
    </Pressable>
...
function GrowthLoopMetric({
  ...
  return (
    <View style={growthLoopStyles.metricCard}>
...
function RetentionCommandCenter({
  plan,
  onPress,
}: {
  plan: HomeGrowthLoop["retention"];
  onPress: (loop: RetentionLoop) => void;
}) {
  return (
    <View style={growthLoopStyles.commandCard}>
      <View style={growthLoopStyles.commandHeader}>
        <View style={{ flex: 1 }}>
          <Text style={styles.sectionLabel}>RETENTION ENGINE</Text>
          <Text style={growthLoopStyles.commandTitle}>
      ...
      <View style={growthLoopStyles.retentionStats}>
        <View style={growthLoopStyles.retentionStat}>
          <Text style={growthLoopStyles.retentionValue}>
            {plan.dailyMatches}
          </Text>
          <Text style={growthLoopStyles.retentionLabel}>daily matches</Text>
        </View>
        <View style={growthLoopStyles.retentionStat}>
          <Text style={growthLoopStyles.retentionValue}>
            {plan.weeklyDropCount}
          </Text>
          <Text style={growthLoopStyles.retentionLabel}>weekly drop</Text>
        </View>
        <View style={growthLoopStyles.retentionStat}>
          <Text style={growthLoopStyles.retentionValue}>10</Text>
          <Text style={growthLoopStyles.retentionLabel}>return loops</Text>
        </View>
      </View>
      ...
      <View style={growthLoopStyles.retentionGrid}>
        {plan.loops.map((loop) => (
          <Pressable
            ...
            style={[
              growthLoopStyles.retentionCard,
              !loop.active && growthLoopStyles.retentionCardOff,
            ]}
          >
            <MiniPremiumIcon
              name={loop.icon as keyof typeof Ionicons.glyphMap}
              tone={loop.active ? "gold" : "dark"}
              size={34}
              iconSize={16}
            />
            <View style={{ flex: 1 }}>
            ...
            <View
              style={[
                growthLoopStyles.retentionAction,
                loop.active && growthLoopStyles.retentionActionOn,
              ]}
            >
              <Text style={growthLoopStyles.retentionActionText}>
                {loop.actionLabel}
              </Text>
            </View>
          </Pressable>
...
function NetworkEffectCenter({
  plan,
  onPress,
}: {
  plan: NetworkEffectPlan;
  onPress: (loop: NetworkGrowthLoop) => void;
}) {
  const challenge = plan.inviteChallenge;
  return (
    <View style={growthLoopStyles.networkCard}>
      <View style={growthLoopStyles.commandHeader}>
        <View style={{ flex: 1 }}>
          <Text style={styles.sectionLabel}>NETWORK EFFECT</Text>
          <Text style={growthLoopStyles.commandTitle}>City growth engine.</Text>
          <Text style={growthLoopStyles.networkSub}>
            Launch density first: {plan.priorityCity.market}
          </Text>
        </View>
        <MiniPremiumIcon
      ...
      <View style={growthLoopStyles.cityRail}>
        {plan.launchCities.map((city) => (
          <View
            key={city.name}
            style={[
              growthLoopStyles.cityLaunchChip,
              city.name === plan.priorityCity.name &&
                growthLoopStyles.cityLaunchChipOn,
            ]}
          >
            <Text style={growthLoopStyles.cityLaunchName}>{city.name}</Text>
            <Text style={growthLoopStyles.cityLaunchMeta}>
              {city.currentProfiles} seeded · {city.status}
            </Text>
          </View>
        ))}
      </View>
      <View style={growthLoopStyles.challengeCard}>
        ...
        <View style={growthLoopStyles.challengeMeter}>
          <Text style={growthLoopStyles.challengeMeterText}>
            {challenge.current}/{challenge.target}
          </Text>
        </View>
      </View>
      <View style={growthLoopStyles.challengeTrack}>
        {[0, 1, 2].map((index) => (
          <View
            key={index}
            style={[
              growthLoopStyles.challengeStep,
              index < challenge.current && growthLoopStyles.challengeStepOn,
            ]}
          />
        ))}
      </View>
      ...
      <View style={growthLoopStyles.storyCard}>
        <MiniPremiumIcon
          ...
          iconSize={15}
        />
        <Text style={growthLoopStyles.promptText}>
          {plan.successStoryPrompts[0]}
        </Text>
      </View>
    </View>
  );
...
function TrustSignal({
  ...
  return (
    <View style={coachStyles.trustItem}>
      <PremiumIcon name={icon} tone="ruby" size={38} iconSize={17} />
      <View style={{ flex: 1 }}>
        <Text style={coachStyles.trustTitle}>{title}</Text>
        <Text style={coachStyles.trustBody}>{body}</Text>
      </View>
    </View>
  );
}
...
function Home({
  ...
  return (
    <LinearGradient
      colors={["#21051D", colors.black, colors.black]}
      style={{ flex: 1 }}
    >
      <SafeAreaView style={{ flex: 1 }}>
        <View style={styles.homeHead}>
          <View>
            <Text style={styles.kicker}>AI CURATION</Text>
            <Text style={shared.h2}>Your daily matches</Text>
          </View>
          <Pressable
            onPress={() => navigate("pricing")}
            style={homeStyles.packageButton}
          >
            <Ionicons name="diamond" size={16} color={colors.gold} />
            <Text style={homeStyles.packageButtonText}>Packages</Text>
          </Pressable>
          <Pressable
            onPress={() => navigate("discovery")}
            style={discoveryStyles.tuneButton}
          >
            <Ionicons name="options" size={20} color={colors.pinkSoft} />
            <View style={discoveryStyles.smartDot} />
          </Pressable>
          <View style={styles.avatar}>
        ...
        <ScrollView
          contentContainerStyle={{ padding: 18, paddingBottom: 110, gap: 18 }}
          showsVerticalScrollIndicator={false}
        >
          <Pressable
            onPress={() => navigate("pricing")}
            style={homeStyles.packageCard}
          >
            <LinearGradient
              colors={["rgba(212,175,55,.18)", "rgba(229,9,47,.10)"]}
              style={StyleSheet.absoluteFill}
            />
            <View style={homeStyles.packageIcon}>
              <Ionicons name="sparkles" size={22} color={colors.gold} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.cardTitle}>
                Packages, Sparks & visibility
              </Text>
              <Text style={styles.helper}>
                Upgrade for more curated matches, likes, and Spark packs.{" "}
                {roseAvailability.paidCredits} Sparks in wallet.
              </Text>
            </View>
          ...
          <Pressable
            ...
            style={circleStyles.homeBanner}
          >
            <View style={circleStyles.bannerFaces}>
              <View
                style={[circleStyles.tinyFace, { backgroundColor: "#7F1D68" }]}
              >
                <Text style={circleStyles.tinyInitial}>S</Text>
              </View>
              <View
                style={[
                  circleStyles.tinyFace,
                  { backgroundColor: "#42307D", marginLeft: -9 },
                ]}
              >
                <Text style={circleStyles.tinyInitial}>R</Text>
              </View>
              <View style={circleStyles.tinyPlus}>
                <Ionicons name="add" size={14} color={colors.ivory} />
              </View>
            </View>
            <View style={{ flex: 1 }}>
              <Text style={circleStyles.bannerTitle}>
                Build your Trusted Circle
              </Text>
              <Text style={circleStyles.bannerBody}>
                Friends vouch. You earn trust—and 100 gift coins.
              </Text>
            </View>
            <Ionicons
              name="chevron-forward"
          ...
          {crossedPaths && (
            <View style={discoveryStyles.crossedSection}>
              <View style={shared.row}>
                <View>
                  <Text style={styles.kicker}>CROSSED PATHS</Text>
                  <Text style={styles.cardTitle}>You were nearby</Text>
                </View>
                <View style={shared.spacer} />
                <Pressable onPress={() => navigate("discovery")}>
                  <Text style={discoveryStyles.manageText}>Manage</Text>
                </Pressable>
              </View>
              <ScrollView
                horizontal
              ...
              >
                {items.slice(0, 2).map((match, index) => (
                  <Pressable
                    key={match.id}
                    onPress={() => openDetail(match)}
                    style={discoveryStyles.crossedCard}
                  >
                    <Image
                      source={{ uri: match.photo }}
                      style={discoveryStyles.crossedImage}
                    />
                    <LinearGradient
                      colors={["transparent", "rgba(13,3,12,.92)"]}
                      style={StyleSheet.absoluteFill}
                    />
                    <View style={discoveryStyles.crossedInfo}>
                      <Text style={discoveryStyles.crossedName}>
                        {match.name}, {match.age}
                      </Text>
                      <Text style={discoveryStyles.crossedMeta}>
                        {index ? "Yesterday" : "2h ago"} · within 1 mile area
                      </Text>
                    </View>
                  </Pressable>
                ))}
          ...
          <View style={aiStyles.aiCard}>
            <View style={shared.row}>
              <View style={aiStyles.aiSpark}>
                <Ionicons name="sparkles" size={18} color={colors.ivory} />
              </View>
              <View style={{ flex: 1 }}>
                <Text style={styles.cardTitle}>AI Match Lens</Text>
                <Text style={styles.helper}>
                  Sorted by your intent, family values, location filters and
                  in-app signals only.
                </Text>
              </View>
            </View>
            <View style={aiStyles.aiPills}>
              {[
                preferences.filters.familyPriority === "high"
                  ? "Family-first priority"
                  : preferences.intent || "Intent aligned",
                `${preferences.filters.minAge}-${preferences.filters.maxAge}`,
                preferences.filters.lookingFor,
                preferences.filters.distancePreference.replaceAll("_", " "),
              ].map((item) => (
                <View key={item} style={aiStyles.aiPill}>
                  <Text style={aiStyles.aiPillText}>{item}</Text>
                </View>
              ))}
            </View>
          </View>
          <View style={styles.curated}>
            ...
            <Text style={styles.curatedText}>
              Privacy-safe AI · never phone search history
            </Text>
            <Text style={styles.curatedCount}>{items.length}</Text>
          </View>
          {items.length ? (
            items.map((match) => (
              <MatchCard
...
function TrustedCircle({
  ...
  const shareInvite = async () => {
    ...
    try {
      ...
      setInviteStatus(
        "Share sheet is not available in this browser preview. Demo invite: https://destinyone.app/vouch/demo",
      );
    }
  };
  ...
  return (
    <LinearGradient
      colors={["#31072B", colors.black, colors.black]}
      style={{ flex: 1 }}
    >
      <SafeAreaView style={shared.safe}>
        <View style={circleStyles.circleHeader}>
          <Pressable onPress={onBack} style={styles.backButton}>
            <Ionicons name="arrow-back" size={21} color={colors.ivory} />
          </Pressable>
          <View style={shared.spacer} />
          <View style={circleStyles.coinBalance}>
            <Ionicons name="sparkles" size={14} color={colors.gold} />
            <Text style={circleStyles.coinBalanceText}>{coinBalance}</Text>
          </View>
        </View>
        <ScrollView
          contentContainerStyle={circleStyles.circleContent}
          showsVerticalScrollIndicator={false}
        >
          <View style={circleStyles.circleHero}>
            <View style={circleStyles.circleOrbit}>
              <View
                style={[
                  circleStyles.friendAvatar,
                  { left: 2, top: 42, backgroundColor: "#7F1D68" },
                ]}
              >
                <Text style={circleStyles.friendInitial}>S</Text>
              </View>
              <View
                style={[
                  circleStyles.friendAvatar,
                  { right: 2, top: 42, backgroundColor: "#42307D" },
                ]}
              >
                <Text style={circleStyles.friendInitial}>R</Text>
              </View>
              <View style={circleStyles.userCircle}>
                <Text style={circleStyles.userInitial}>A</Text>
                <View style={circleStyles.trustCheck}>
                  <Ionicons name="checkmark" size={16} color={colors.ivory} />
                </View>
              </View>
            </View>
            <Text style={styles.kicker}>TRUSTED CIRCLE</Text>
            <Text style={[shared.h1, { textAlign: "center" }]}>
              The people who know you, know your heart.
            </Text>
            <Text style={[shared.body, { textAlign: "center" }]}>
              Invite up to 3 close friends to vouch for your character—not your
              dating choices.
            </Text>
          </View>
          <View style={coachStyles.trustStrip}>
            <TrustSignal
              ...
              body="Confirms profile photo belongs to the person creating the account."
            />
            <TrustSignal
              icon="id-card"
              title="Optional ID check"
              body="Production can add ID verification for higher trust badges."
            />
            <TrustSignal
              icon="people"
              title="Friend vouches"
              body="Character vouches show reliability without exposing private dating activity."
            />
            <TrustSignal
              icon="calendar"
              title="Safer date check-ins"
              body="Public date plans and check-ins help members feel safer meeting offline."
            />
          </View>
          <View style={circleStyles.progressCard}>
            <View style={shared.row}>
              <Text style={styles.cardTitle}>Your circle</Text>
              <View style={shared.spacer} />
              <Text style={circleStyles.progressCount}>
                {vouches.length}/3 vouched
              </Text>
            </View>
            <View style={circleStyles.vouchProgress}>
              {[0, 1, 2].map((index) => (
                <View
                  key={index}
                  style={[
                    circleStyles.vouchProgressStep,
                    index < vouches.length && circleStyles.vouchProgressOn,
                  ]}
                />
              ))}
            </View>
            {vouches.length ? (
              <View style={circleStyles.qualityWrap}>
                {vouches.map((value) => (
                  <View key={value} style={circleStyles.qualityPill}>
                    <Ionicons
                      name="checkmark"
                      size={12}
                      color={colors.pinkSoft}
                    />
                    <Text style={circleStyles.qualityText}>{value}</Text>
                  </View>
                ))}
              </View>
            ) : (
              <Text style={styles.helper}>
                No vouches yet. Your friends answer privately from the invite
                link.
              </Text>
            )}
          </View>
          <Button
            label="Invite a trusted friend"
            icon="share-social"
            onPress={() => void shareInvite()}
          />
          {!!inviteStatus && (
            <View style={circleStyles.boundaryCard}>
              <Ionicons name="link" size={22} color={colors.gold} />
              <Text style={[styles.helper, { flex: 1 }]}>{inviteStatus}</Text>
            </View>
          )}
          <View style={circleStyles.rewardCard}>
            <View style={circleStyles.rewardIcon}>
              <Ionicons name="gift" size={23} color={colors.gold} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={styles.cardTitle}>
                100 gift coins per completed vouch
              </Text>
              <Text style={styles.helper}>
                Rewarded only after a friend completes their genuine
                endorsement.
              </Text>
            </View>
          </View>
          <View style={circleStyles.demoCard}>
            <Text style={styles.sectionLabel}>MVP PREVIEW</Text>
            <Text style={styles.helper}>
            ...
            <View style={circleStyles.qualityWrap}>
              {demoQualities.map((quality) => (
                <Pressable
                  disabled={vouches.includes(quality) || vouches.length >= 3}
                  onPress={() => onAddVouch(quality)}
                  key={quality}
                  style={[
                    circleStyles.demoQuality,
                    vouches.includes(quality) && { opacity: 0.4 },
                  ]}
                >
                  <Text style={circleStyles.demoQualityText}>{quality}</Text>
          ...
          <View style={circleStyles.boundaryCard}>
            ...
            />
            <Text style={[styles.helper, { flex: 1 }]}>
              Friends cannot view your matches, messages, likes, or private
              preferences. You remain fully in control.
            </Text>
          </View>
        </ScrollView>
      </SafeAreaView>
...
function DiscoveryCenterLegacy({
  filters,
  onFiltersChange,
  signals,
  smartDiscovery,
  ...
  onCrossedChange: (value: boolean) => void;
  onClear: () => void;
  onBack: () => void;
}) {
  ...
  const toggleCrossed = async () => {
    ...
    if (!permission.granted) {
      setLocationError(
        "Approximate location permission is needed for Crossed Paths.",
      );
      return;
    }
    try {
      ...
      setLocationError(
        "Location is unavailable right now. Please try again outdoors.",
      );
    }
  };
  ...
  const toggleArray = (
    key: "intents" | "mustHaveVibes" | "cities",
    value: string,
  ) => {
  ...
  return (
    <LinearGradient
      colors={["#260620", colors.black, colors.black]}
      style={{ flex: 1 }}
    >
      <SafeAreaView style={shared.safe}>
        ...
        <ScrollView
          contentContainerStyle={discoveryStyles.content}
          showsVerticalScrollIndicator={false}
        >
          <SectionTitle
            eyebrow="Your choices, your control"
            title="Smarter matches without spying."
            body="DestinyOne learns only from what you choose inside the app. You can pause or erase this learning anytime."
          />
          <View style={aiStyles.filterCard}>
            <View style={shared.row}>
              <Ionicons name="sparkles" size={21} color={colors.gold} />
              <Text style={[styles.cardTitle, { marginLeft: 8 }]}>
                Detailed match filters
              </Text>
              <View style={shared.spacer} />
              <Pressable onPress={() => onFiltersChange(defaultMatchFilters)}>
                <Text style={discoveryStyles.manageText}>Reset</Text>
              </Pressable>
            </View>
            <FilterSection title="Looking for">
              {(["Women", "Men", "Everyone"] as const).map((option) => (
                <FilterChip
                  key={option}
                  label={option}
                  active={filters.lookingFor === option}
                  onPress={() => update({ lookingFor: option })}
                />
              ))}
            </FilterSection>
            <FilterSection
              title={`Age range · ${filters.minAge}-${filters.maxAge}`}
            >
              <FilterChip
                label="25-30"
                active={filters.minAge === 25 && filters.maxAge === 30}
                onPress={() => update({ minAge: 25, maxAge: 30 })}
              />
              <FilterChip
                label="25-35"
                active={filters.minAge === 25 && filters.maxAge === 35}
                onPress={() => update({ minAge: 25, maxAge: 35 })}
              />
              <FilterChip
                label="30-35"
                active={filters.minAge === 30 && filters.maxAge === 35}
                onPress={() => update({ minAge: 30, maxAge: 35 })}
              />
            </FilterSection>
            <FilterSection title="Relationship intent">
              {[
                "Marriage",
                ...
                <FilterChip
                  key={option}
                  label={option}
                  active={filters.intents.includes(option)}
                  onPress={() => toggleArray("intents", option)}
                />
              ))}
            </FilterSection>
            <FilterSection title="Family priority">
              <FilterChip
                label="Any"
                active={filters.familyPriority === "any"}
                onPress={() => update({ familyPriority: "any" })}
              />
              <FilterChip
                label="Family First"
                active={filters.familyPriority === "high"}
                onPress={() => update({ familyPriority: "high" })}
              />
              <FilterChip
                label="Balanced"
                active={filters.familyPriority === "balanced"}
                onPress={() => update({ familyPriority: "balanced" })}
              />
            </FilterSection>
            <FilterSection title="Future plans">
              <FilterChip
                label="Any children plan"
                active={filters.children === "any"}
                onPress={() => update({ children: "any" })}
              />
              <FilterChip
                label="Wants children"
                active={filters.children === "wants"}
                onPress={() => update({ children: "wants" })}
              />
              <FilterChip
                label="Open to children"
                active={filters.children === "open"}
                onPress={() => update({ children: "open" })}
              />
              <FilterChip
                label="Marriage 1-2 yrs"
                active={filters.marriageTimeline === "1_2_years"}
                onPress={() => update({ marriageTimeline: "1_2_years" })}
              />
              <FilterChip
                label="Marriage 2-3 yrs"
                active={filters.marriageTimeline === "2_3_years"}
                onPress={() => update({ marriageTimeline: "2_3_years" })}
              />
            </FilterSection>
            <FilterSection title="Must-have vibe">
              {vibes.map((option) => (
                <FilterChip
                  key={option}
                  label={option}
                  active={filters.mustHaveVibes.includes(option)}
                  onPress={() => toggleArray("mustHaveVibes", option)}
                />
              ))}
            </FilterSection>
            <FilterSection title="City focus">
              {[
                "New York",
                "Austin",
                "Chicago",
                ...
                <FilterChip
                  key={option}
                  label={option}
                  active={filters.cities.includes(option)}
                  onPress={() => toggleArray("cities", option)}
                />
              ))}
            </FilterSection>
            <FilterSection title="Relocation">
              <FilterChip
                label="Any"
                active={filters.relocation === "any"}
                onPress={() => update({ relocation: "any" })}
              />
              <FilterChip
                label="Open to relocate"
                active={filters.relocation === "open"}
                onPress={() => update({ relocation: "open" })}
              />
              <FilterChip
                label="Prefers same city"
                active={filters.relocation === "same_city"}
                onPress={() => update({ relocation: "same_city" })}
              />
            </FilterSection>
          </View>
          <View style={discoveryStyles.neverTrack}>
            ...
            />
            <View style={{ flex: 1 }}>
              <Text style={styles.cardTitle}>What we never read</Text>
              <Text style={styles.helper}>
                Browser or Google searches, messages outside DestinyOne,
                contacts, photos you don’t select, microphone activity, or usage
                in other apps.
              </Text>
            </View>
          </View>
          <DiscoveryToggle
            ...
            body="Reorders daily matches using your stated preferences, profile views, interests and skips."
            value={smartDiscovery}
            onPress={() => onSmartChange(!smartDiscovery)}
          />
          <DiscoveryToggle
            icon="walk"
            title="Crossed Paths"
            body="Shows opted-in members whose approximate area overlapped with yours. Exact place and time stay hidden."
            value={crossedPaths}
            onPress={() => void toggleCrossed()}
          />
          {!!locationError && (
            <Text style={styles.formError}>{locationError}</Text>
          )}
          <View style={discoveryStyles.activityCard}>
            <View style={shared.row}>
              <Text style={styles.cardTitle}>Your in-app activity</Text>
              <View style={shared.spacer} />
              <Text style={styles.helper}>On this device</Text>
            </View>
            <View style={discoveryStyles.stats}>
              <DiscoveryStat value={views} label="Profiles viewed" />
              <DiscoveryStat value={likes} label="Interested" />
              <DiscoveryStat value={skips} label="Skipped" />
            </View>
            <Pressable
              disabled={!signals.length}
              onPress={onClear}
              style={[
                discoveryStyles.clearButton,
                !signals.length && { opacity: 0.4 },
              ]}
            >
              <Ionicons name="trash-outline" size={17} color={colors.danger} />
              <Text style={discoveryStyles.clearText}>
                Clear activity and reset learning
              </Text>
            </Pressable>
          </View>
          <View style={aiStyles.privacyPolicyCard}>
            <Ionicons name="lock-closed" size={22} color={colors.gold} />
            <View style={{ flex: 1 }}>
              <Text style={styles.cardTitle}>AI Privacy Policy</Text>
              <Text style={styles.helper}>
                Your filters and in-app signals improve ranking. Exact scores
                stay internal, and you can reset learning anytime.
              </Text>
            </View>
          </View>
          <View style={discoveryStyles.privacyGrid}>
            <PrivacyPoint
              icon="location-outline"
              title="Approximate only"
              body="Low-accuracy location is used; raw coordinates are not saved in this MVP."
            />
            <PrivacyPoint
              icon="people-outline"
              title="Both must opt in"
              body="You appear in Crossed Paths only when both members enable it."
            />
            <PrivacyPoint
              icon="time-outline"
              title="Delayed display"
              body="Profiles appear later, never as a live location tracker."
            />
            <PrivacyPoint
              icon="shield-checkmark-outline"
              title="Block always wins"
              body="Blocked or reported members never appear in discovery."
            />
          </View>
        </ScrollView>
      </SafeAreaView>
      ...
      <SafeAreaView style={shared.safe}>
        ...
        >
          <SectionTitle
            eyebrow="Your choices, your control"
            title="Smarter matches without spying."
            body="DestinyOne learns from your filters, profile views and in-app actions only. You can reset this anytime."
          />
          <View style={aiStyles.filterCard}>
            <View style={shared.row}>
              <PremiumIcon
              ...
              <View style={shared.spacer} />
              <Pressable
                onPress={() => onFiltersChange(defaultMatchFilters)}
            ...
            <FilterSection title="Location preference">
              <FilterChip
                label="Anywhere"
                active={filters.distancePreference === "anywhere"}
                onPress={() => update({ distancePreference: "anywhere" })}
              />
              <FilterChip
                label="Selected cities only"
                active={filters.distancePreference === "selected_cities"}
                onPress={() =>
                  update({ distancePreference: "selected_cities" })
                }
              />
              <FilterChip
                label="Same state/province"
                active={filters.distancePreference === "same_state"}
                onPress={() => update({ distancePreference: "same_state" })}
              />
              <FilterChip
                label="Open to relocate"
                active={filters.distancePreference === "open_to_relocate"}
                onPress={() =>
                  update({ distancePreference: "open_to_relocate" })
                }
              />
            </FilterSection>
            <FilterSection title="Relocation">
          ...
          <View style={discoveryStyles.privacyGrid}>
            <PrivacyPoint
              icon="location-outline"
              title="Approximate only"
              body="Location filters are preference-based; Crossed Paths uses low-accuracy foreground location only."
            />
            <PrivacyPoint
              icon="people-outline"
              title="Both must opt in"
...
function DiscoveryToggle({
  ...
  return (
    <Pressable onPress={onPress} style={discoveryStyles.toggleCard}>
      <PremiumIcon
        name={icon}
        tone={value ? "gold" : "ruby"}
        size={52}
        iconSize={23}
      />
      <View style={{ flex: 1 }}>
...
function FilterSection({
  ...
  return (
    <View style={aiStyles.filterSection}>
      <Text style={styles.sectionLabel}>{title.toUpperCase()}</Text>
      <View style={aiStyles.filterWrap}>{children}</View>
    </View>
  );
}
function FilterChip({
  ...
  return (
    <Pressable
      onPress={onPress}
      style={[aiStyles.filterChip, active && aiStyles.filterChipOn]}
    >
      <Text
        style={[aiStyles.filterChipText, active && { color: colors.ivory }]}
      >
        {label}
      </Text>
      {active && (
...
function DiscoveryStat({ value, label }: { value: number; label: string }) {
  return (
    <View style={discoveryStyles.stat}>
      <Text style={discoveryStyles.statValue}>{value}</Text>
      <Text style={discoveryStyles.statLabel}>{label}</Text>
    </View>
  );
}
function PrivacyPoint({
  ...
  return (
    <View style={discoveryStyles.privacyPoint}>
      <PremiumIcon name={icon} tone="ruby" size={38} iconSize={17} />
      <Text style={discoveryStyles.privacyTitle}>{title}</Text>
      <Text style={discoveryStyles.privacyBody}>{body}</Text>
    </View>
  );
}
...
const coachCards = [
  {
    ...
    body: "Rewrite your bio so it sounds warm, serious and real—not generic.",
  },
  {
    title: "First-message helper",
    icon: "chatbubble-ellipses" as const,
    body: "Get a thoughtful opener based on values, city and profile details.",
  },
  {
    title: "Red-flag scan",
    icon: "warning" as const,
    body: "Check for pressure, money requests, inconsistent intent or unsafe meeting patterns.",
  },
  {
    title: "Post-date reflection",
    icon: "heart-circle" as const,
    body: "After a date, log what felt safe, exciting, unclear and worth exploring.",
  },
];

const eventExperiences = [
  {
    title: "Rooftop Chai Mixer",
    city: "New York, NY",
    date: "Friday · 7 PM",
    type: "In person",
    icon: "☕",
    tag: "Verified members only",
    body: "A calm 40-person Indian singles mixer with conversation prompts, safety hosts and clear intent badges.",
  },
  {
    title: "Gujarati Garba Social",
    city: "New Jersey / New York",
    date: "Saturday · 6 PM",
    type: "In person",
    icon: "💃🏽",
    tag: "Community-led",
    body: "Culture-first evening for serious singles who want family-friendly energy without old-school pressure.",
  },
  {
    title: "Punjabi Culture Night",
    city: "Toronto, ON",
    date: "Saturday · 6 PM",
    type: "In person",
    icon: "🎶",
    tag: "Community-led",
    body: "Music, food and serious singles who value family, culture and long-term compatibility.",
  },
  {
    title: "South Asian Professionals Mixer",
    city: "San Francisco, CA",
    date: "Thursday · 7 PM",
    type: "In person",
    icon: "🤝",
    tag: "Career + values",
    body: "Small groups for Indian, Punjabi and American professionals who want real relationships, not casual swiping.",
  },
  {
    ...
    icon: "🎥",
    tag: "7-minute rounds",
    body: "Private video rounds before anyone can ask for a phone number. Chat unlocks only after mutual interest.",
  },
  {
    title: "Marriage-minded Speed Dating",
    city: "Dallas, TX",
    date: "Sunday · 4 PM",
    type: "Hybrid",
    icon: "💍",
    tag: "Intent verified",
    body: "Quick values-led intros with serious relationship and marriage filters checked before the event.",
  },
  {
    title: "Premium Invite-only Dinner",
    city: "Los Angeles, CA",
    date: "Friday · 8 PM",
    type: "Private dinner",
    icon: "🍽️",
    tag: "Limited seats",
    body: "Eight verified members, hosted table, premium venue and gentle post-event concierge follow-up.",
  },
  {
    title: "Executive Private Dinner",
    city: "New York, NY",
    date: "Monthly",
    type: "Invite only",
    icon: "🥂",
    tag: "Executive Circle",
    body: "Founder and business-owner dinner for members approved through Executive Circle verification.",
  },
];
...
const placeKinds: ("All" | PlaceKind)[] = [
  "All",
  "Restaurant",
  "Cafe",
  "Tourist",
...
const datePackages: DatePackage[] = [
  {
    ...
    icon: "cafe",
    includes: [
      "Quiet public café shortlist",
      "Two time options",
      "Safety check-in reminder",
    ],
    safety: "Public, easy exit, no private address shared.",
  },
  {
    id: "chai-dessert",
    title: "Chai + Dessert Walk",
    tier: "Community favorite",
    city: "NYC · Toronto · Dallas",
    price: "$22–$45 pp",
    duration: "90 min",
    icon: "ice-cream",
    includes: [
    ...
    safety: "Busy area, daytime/evening public route.",
  },
  {
    id: "museum-coffee",
    title: "Museum + Coffee",
    ...
    icon: "color-palette",
    includes: [
      "Museum or gallery pick",
      "Coffee after",
      "Low-pressure activity",
    ],
    safety: "Staffed indoor venue with public seating.",
  },
  {
    id: "indian-dinner",
    title: "Indian Dinner Date",
    tier: "Plus",
    city: "Top metro cities",
    price: "$45–$90 pp",
    duration: "90–120 min",
    icon: "restaurant",
    includes: [
      "Vegetarian-friendly restaurant",
      "Reservation hold preview",
      "Split/host payment choice",
    ],
    safety: "Partner venue, reservation trail and check-in.",
  },
  {
    id: "rooftop-table",
    title: "Premium Rooftop Table",
    tier: "Premium",
    city: "NYC · LA · Miami · Toronto",
    price: "$95–$180 pp",
    duration: "2 hours",
    icon: "wine",
    includes: [
      "Rooftop or lounge table",
      "Mocktail/dessert option",
      "Concierge reminder",
    ],
    safety: "Staffed venue, separate arrivals encouraged.",
  },
  {
    id: "executive-dinner",
    title: "Executive Invite-only Dinner",
    tier: "Executive Circle",
    city: "NYC · SF · Dallas",
    price: "Included after approval",
    duration: "2.5 hours",
    icon: "diamond",
    includes: [
      "Verified guest list",
      "Hosted table",
      "Private concierge follow-up",
    ],
    safety: "Invite-only, ID/business verified, host present.",
  },
];
const partnerPipeline = [
  [
    "Venue database",
    "Curated USA/Canada places with safe-first-date notes, category filters and city search.",
    true,
  ],
  [
    "Partner outreach",
    "Restaurant/café partner program with package menu, refund SLA and support contact.",
    true,
  ],
  [
    "Reservation API",
    "Provider adapter planned for table holds, quote expiry, confirmation and cancellation webhooks.",
    true,
  ],
  [
    "Date safety ops",
    "Check-ins, trusted-contact share, public venue rules and report path are part of each date flow.",
    true,
  ],
  [
    "Event operations",
    "Capacity, ticketing, host check-in and verified-member list are ready for live provider connection.",
    false,
  ],
] as const;
const launchCityRoadmap = [
  {
    ...
    stage: "Launch city",
    focus: "Indian cafés, rooftop dinners, Gujarati/Punjabi mixers",
    event: "Rooftop Chai Mixer",
    icon: "business" as const,
  },
  {
    city: "Toronto",
    stage: "Launch city",
    focus: "Punjabi culture nights, dessert walks, professional mixers",
    event: "Punjabi Culture Night",
    icon: "leaf" as const,
  },
  {
    city: "Dallas",
    stage: "Fast follow",
    focus: "Marriage-minded speed dating, Indian dinner packages",
    event: "Marriage-minded Speed Dating",
    icon: "flame" as const,
  },
  {
    city: "Bay Area",
    stage: "Fast follow",
    focus: "South Asian professionals, museum + coffee dates",
    event: "Professionals Mixer",
    icon: "sparkles" as const,
  },
  {
    city: "Los Angeles",
    stage: "Premium city",
    focus: "Invite-only dinners, rooftop tables, executive introductions",
    event: "Premium Dinner",
...
const reservationOps = [
  {
    title: "Quote + hold",
    body: "Show package price, hold expiry, refund rules and venue confirmation before payment.",
    icon: "receipt-outline" as const,
  },
  {
    title: "Private acceptance",
    body: "Both members confirm the date plan before location details or reservation actions are finalized.",
    icon: "lock-closed-outline" as const,
  },
  {
    title: "Safety check-in",
    body: "Reminder before and after the date with quick “I’m safe” and support/report paths.",
    icon: "shield-checkmark-outline" as const,
  },
  {
    title: "Partner support",
    body: "Venue cancellation, late arrival, refund and support escalation are tracked as provider events.",
...
const placeDirectory: PlaceItem[] = [
  {
    ...
    area: "Central Park",
    price: "Free",
    vibe: "Classic walk, photos and quiet conversation",
    bestTime: "Saturday morning",
    safety: "Very public in daytime; meet near main paths",
    icon: "🌳",
    tags: ["walk", "tourist", "romantic"],
  },
  {
    id: "nyc-bryant",
    name: "Bryant Park Coffee Walk",
    city: "New York, NY",
    country: "USA",
    kind: "Cafe",
    area: "Midtown",
    price: "$",
    vibe: "Easy first coffee with public seating",
    bestTime: "Weekday evening",
    safety: "Busy public area near transit",
    icon: "☕",
    tags: ["coffee", "public", "quick"],
  },
  {
    id: "nyc-pier57",
    name: "Pier 57 Rooftop & Food Hall",
    city: "New York, NY",
    country: "USA",
    kind: "Restaurant",
    area: "Chelsea / Hudson River",
    price: "$$",
    vibe: "Views, food choices and low-pressure seating",
    bestTime: "Sunset",
    safety: "Public venue with multiple exits",
    icon: "🌇",
    tags: ["food hall", "views", "sunset"],
  },
  {
    id: "nyc-met",
    name: "The Met Museum Date",
    city: "New York, NY",
    country: "USA",
    kind: "Cultural",
    area: "Upper East Side",
    price: "$$",
    vibe: "Art, values and easy conversation starters",
    bestTime: "Sunday afternoon",
    safety: "Indoor public museum",
    icon: "🖼️",
    tags: ["museum", "culture", "day date"],
  },
  {
    id: "la-griffith",
    ...
    area: "Los Feliz",
    price: "Free",
    vibe: "City views, stars and meaningful talk",
    bestTime: "Golden hour",
    safety: "Public attraction; parking can be busy",
    icon: "🔭",
    tags: ["views", "tourist", "sunset"],
  },
  {
    id: "la-getty",
    name: "Getty Center Garden Walk",
    city: "Los Angeles, CA",
    country: "USA",
    kind: "Cultural",
    area: "Brentwood",
    price: "$",
    vibe: "Architecture, gardens and slow conversation",
    bestTime: "Saturday afternoon",
    safety: "Staffed public campus",
    icon: "🏛️",
    tags: ["museum", "garden", "art"],
  },
  {
    id: "la-venice",
    name: "Venice Canals Stroll",
    city: "Los Angeles, CA",
    country: "USA",
    kind: "Park",
    area: "Venice",
    price: "Free",
    vibe: "Scenic walk without loud bar energy",
    bestTime: "Morning",
    safety: "Meet in daylight and stay on public walkways",
    icon: "🌊",
    tags: ["walk", "photo", "calm"],
  },
  {
    id: "la-rooftop",
    name: "Downtown Rooftop Mocktail Lounge",
    city: "Los Angeles, CA",
    country: "USA",
    kind: "Lounge",
    area: "DTLA",
    price: "$$$",
    vibe: "Premium evening date with skyline energy",
    bestTime: "Friday 8 PM",
    safety: "Choose staffed venues and arrange own transport",
    icon: "🍸",
    tags: ["rooftop", "premium", "mocktails"],
  },
  {
    id: "chi-riverwalk",
    ...
    area: "Downtown",
    price: "Free",
    vibe: "Beautiful walk, architecture and easy stops",
    bestTime: "Summer evening",
    safety: "Public and active; avoid isolated late hours",
    icon: "🚶",
    tags: ["walk", "architecture", "views"],
  },
  {
    id: "chi-millennium",
    name: "Millennium Park + Dessert",
    city: "Chicago, IL",
    country: "USA",
    kind: "Dessert",
    area: "The Loop",
    price: "$",
    vibe: "Tourist classic plus sweet treat after",
    bestTime: "Afternoon",
    safety: "Meet near main entrances",
    icon: "🍨",
    tags: ["dessert", "tourist", "public"],
  },
  {
    id: "chi-westloop",
    name: "West Loop Dinner Row",
    city: "Chicago, IL",
    country: "USA",
    kind: "Restaurant",
    area: "West Loop",
    price: "$$$",
    vibe: "Upscale dinner options for second dates",
    bestTime: "Saturday dinner",
    safety: "Use reservation and share date plan",
    icon: "🍽️",
    tags: ["dinner", "premium", "restaurant"],
  },
  {
    id: "hou-buffalo",
    ...
    country: "USA",
    kind: "Park",
    area: "Montrose / Downtown",
    price: "Free",
    vibe: "Walk, skyline and casual outdoor energy",
    bestTime: "Morning or sunset",
    safety: "Daytime recommended for first date",
    icon: "🌿",
    tags: ["walk", "park", "skyline"],
  },
  {
    id: "hou-museum",
    name: "Museum District Café Date",
    city: "Houston, TX",
    country: "USA",
    kind: "Cafe",
    area: "Museum District",
    price: "$$",
    vibe: "Coffee before or after a museum visit",
    bestTime: "Sunday afternoon",
    safety: "Public, easy to exit politely",
    icon: "☕",
    tags: ["museum", "coffee", "culture"],
  },
  {
    id: "dal-klyde",
    ...
    area: "Arts District",
    price: "Free",
    vibe: "Food trucks, public seating and light activity",
    bestTime: "Saturday lunch",
    safety: "Busy public park",
    icon: "🌮",
    tags: ["food trucks", "park", "casual"],
  },
  {
    id: "dal-bishop",
    name: "Bishop Arts Dessert Walk",
    city: "Dallas, TX",
    country: "USA",
    kind: "Dessert",
    area: "Bishop Arts",
    price: "$$",
    vibe: "Cute shops, dessert and low-pressure wandering",
    bestTime: "Evening",
    safety: "Stay in active streets",
    icon: "🧁",
    tags: ["dessert", "shops", "walk"],
  },
  {
    id: "aus-ladybird",
    ...
    price: "Free",
    vibe: "Active, relaxed and conversation-friendly",
    bestTime: "Morning",
    safety: "Public trail; daytime first",
    icon: "🏞️",
    tags: ["walk", "fitness", "outdoor"],
  },
  {
    id: "aus-southcongress",
    name: "South Congress Coffee + Shops",
    city: "Austin, TX",
    country: "USA",
    kind: "Cafe",
    area: "SoCo",
    price: "$$",
    vibe: "Coffee, boutiques and playful photos",
    bestTime: "Saturday afternoon",
    safety: "Busy public area",
    icon: "🛍️",
    tags: ["coffee", "shops", "casual"],
  },
  {
    id: "sf-ferry",
    name: "Ferry Building Date",
    city: "San Francisco, CA",
    country: "USA",
    kind: "Restaurant",
    area: "Embarcadero",
    price: "$$",
    vibe: "Food stalls, bay views and easy stroll",
    bestTime: "Weekend lunch",
    safety: "Public indoor/outdoor marketplace",
    icon: "🌁",
    tags: ["food hall", "bay", "walk"],
  },
  {
    id: "sf-golden",
    name: "Golden Gate Park Tea Garden",
    city: "San Francisco, CA",
    country: "USA",
    kind: "Cultural",
    area: "Golden Gate Park",
    price: "$$",
    vibe: "Quiet, beautiful and intentional",
    bestTime: "Sunday afternoon",
    safety: "Daytime public attraction",
    icon: "🍵",
    tags: ["tea", "garden", "culture"],
  },
  {
    id: "sea-pike",
    ...
    price: "$$",
    vibe: "Food, flowers and playful exploration",
    bestTime: "Morning",
    safety: "Busy public market",
    icon: "💐",
    tags: ["market", "flowers", "tourist"],
  },
  {
    id: "sea-kerry",
    ...
    area: "Queen Anne",
    price: "Free",
    vibe: "Short scenic stop, best paired with coffee",
    bestTime: "Sunset",
    safety: "Public viewpoint; keep it brief for first meet",
    icon: "🌄",
    tags: ["views", "photo", "sunset"],
  },
  {
    id: "mia-wynwood",
    name: "Wynwood Walls + Café",
    city: "Miami, FL",
    country: "USA",
    kind: "Cultural",
    area: "Wynwood",
    price: "$$",
    vibe: "Art, color and easy conversation",
    bestTime: "Afternoon",
    safety: "Stay in main public art areas",
    icon: "🎨",
    tags: ["art", "coffee", "walk"],
  },
  {
    id: "mia-brickell",
    name: "Brickell Dinner Lounge",
    city: "Miami, FL",
    country: "USA",
    kind: "Lounge",
    area: "Brickell",
    price: "$$$",
    vibe: "Dressy evening with city energy",
    bestTime: "Friday evening",
    safety: "Meet inside venue, arrange own ride",
    icon: "✨",
    tags: ["lounge", "premium", "dinner"],
  },
  {
    id: "bos-seaport",
    name: "Boston Seaport Walk",
    city: "Boston, MA",
    country: "USA",
    kind: "Park",
    area: "Seaport",
    price: "Free",
    vibe: "Waterfront, clean public space and cafés nearby",
    bestTime: "Late afternoon",
    safety: "Public and active area",
    icon: "🌊",
    tags: ["waterfront", "walk", "coffee"],
  },
  {
    id: "bos-isabella",
    ...
    area: "Fenway",
    price: "$$",
    vibe: "Romantic art setting without bar pressure",
    bestTime: "Sunday afternoon",
    safety: "Staffed indoor museum",
    icon: "🏺",
    tags: ["museum", "art", "romantic"],
  },
  {
    id: "dc-mall",
    ...
    area: "National Mall",
    price: "Free",
    vibe: "Iconic monuments and meaningful talks",
    bestTime: "Morning",
    safety: "Public; avoid isolated late-night walks",
    icon: "🏛️",
    tags: ["tourist", "walk", "history"],
  },
  {
    id: "dc-georgetown",
    name: "Georgetown Waterfront Dessert",
    city: "Washington, DC",
    country: "USA",
    kind: "Dessert",
    area: "Georgetown",
    price: "$$",
    vibe: "River views, dessert and cute streets",
    bestTime: "Evening",
    safety: "Busy public area",
    icon: "🍰",
    tags: ["dessert", "waterfront", "walk"],
  },
  {
    id: "sd-balboa",
    name: "Balboa Park Garden Date",
    city: "San Diego, CA",
    country: "USA",
    kind: "Park",
    area: "Balboa Park",
    price: "Free",
    vibe: "Gardens, museums and sunshine",
    bestTime: "Saturday afternoon",
    safety: "Public daytime location",
    icon: "🌺",
    tags: ["garden", "museum", "outdoor"],
  },
  {
    id: "atl-beltline",
    name: "Atlanta BeltLine + Food Hall",
    city: "Atlanta, GA",
    country: "USA",
    kind: "Activity",
    area: "Old Fourth Ward",
    price: "$$",
    vibe: "Walk, murals and food options",
    bestTime: "Weekend afternoon",
    safety: "Stay on active trail sections",
    icon: "🚲",
    tags: ["walk", "food hall", "murals"],
  },
  {
    id: "den-union",
    name: "Denver Union Station Coffee",
    city: "Denver, CO",
    country: "USA",
    kind: "Cafe",
    area: "LoDo",
    price: "$$",
    vibe: "Cozy public coffee date with transit access",
    bestTime: "Sunday morning",
    safety: "Public landmark with staff nearby",
    icon: "🚉",
    tags: ["coffee", "public", "cozy"],
  },
  {
    id: "lv-bellagio",
    name: "Bellagio Conservatory Walk",
    city: "Las Vegas, NV",
    country: "USA",
    kind: "Tourist",
    area: "The Strip",
    price: "Free",
    vibe: "Beautiful indoor walk without casino pressure",
    bestTime: "Afternoon",
    safety: "Busy public resort area",
    icon: "🌸",
    tags: ["tourist", "indoor", "photo"],
  },
  {
    id: "orl-disney",
    name: "Disney Springs Dinner Walk",
    city: "Orlando, FL",
    country: "USA",
    kind: "Restaurant",
    area: "Lake Buena Vista",
    price: "$$",
    vibe: "Food, music and safe public energy",
    bestTime: "Evening",
    safety: "Highly public, staffed area",
    icon: "🎶",
    tags: ["restaurant", "walk", "entertainment"],
  },
  {
    id: "tor-distillery",
    name: "Distillery District Date",
    city: "Toronto, ON",
    country: "Canada",
    kind: "Cultural",
    area: "Downtown Toronto",
    price: "$$",
    vibe: "Historic streets, dessert and galleries",
    bestTime: "Saturday afternoon",
    safety: "Public pedestrian district",
    icon: "🧱",
    tags: ["culture", "dessert", "walk"],
  },
  {
    id: "tor-cn",
    name: "CN Tower Views + Dinner Nearby",
    city: "Toronto, ON",
    country: "Canada",
    kind: "Tourist",
    area: "Entertainment District",
    price: "$$$",
    vibe: "Big-city premium date energy",
    bestTime: "Sunset",
    safety: "Public landmark; book ahead",
    icon: "🗼",
    tags: ["views", "tourist", "premium"],
  },
  {
    id: "tor-yorkville",
    name: "Yorkville Café & Gallery Walk",
    city: "Toronto, ON",
    country: "Canada",
    kind: "Cafe",
    area: "Yorkville",
    price: "$$",
    vibe: "Polished café date and calm streets",
    bestTime: "Sunday afternoon",
    safety: "Busy upscale neighborhood",
    icon: "☕",
    tags: ["coffee", "gallery", "premium"],
  },
  {
    id: "van-stanley",
    ...
    area: "Stanley Park",
    price: "Free",
    vibe: "Iconic walk with ocean views",
    bestTime: "Morning",
    safety: "Daytime public route recommended",
    icon: "🌲",
    tags: ["walk", "views", "outdoor"],
  },
  {
    id: "van-granville",
    ...
    price: "$$",
    vibe: "Food market, shops and waterfront",
    bestTime: "Lunch",
    safety: "Public market",
    icon: "🛶",
    tags: ["market", "food", "waterfront"],
  },
  {
    id: "mtl-old",
    name: "Old Montréal Evening Walk",
    city: "Montreal, QC",
    country: "Canada",
    kind: "Tourist",
    area: "Old Montréal",
    price: "Free",
    vibe: "Cobblestones, lights and romantic streets",
    bestTime: "Early evening",
    safety: "Stay in active tourist streets",
    icon: "🏙️",
    tags: ["tourist", "romantic", "walk"],
  },
  {
    id: "mtl-mountroyal",
    ...
    area: "Mount Royal",
    price: "Free",
    vibe: "Views and outdoor conversation",
    bestTime: "Daytime",
    safety: "Daylight first-date option",
    icon: "⛰️",
    tags: ["views", "park", "outdoor"],
  },
  {
    id: "cal-peace",
    name: "Peace Bridge + River Café Area",
    city: "Calgary, AB",
    country: "Canada",
    kind: "Cafe",
    area: "Bow River",
    price: "$$",
    vibe: "Walk plus coffee/dessert nearby",
    bestTime: "Afternoon",
    safety: "Public river path",
    icon: "🌉",
    tags: ["walk", "coffee", "river"],
  },
  {
    id: "cal-prince",
    ...
    price: "Free",
    vibe: "Relaxed green-space date",
    bestTime: "Morning",
    safety: "Public park in daylight",
    icon: "🍃",
    tags: ["park", "walk", "calm"],
  },
  {
    id: "ott-byward",
    name: "ByWard Market Food Walk",
    city: "Ottawa, ON",
    country: "Canada",
    kind: "Restaurant",
    area: "ByWard Market",
    price: "$$",
    vibe: "Food stalls, desserts and lively streets",
    bestTime: "Weekend afternoon",
    safety: "Busy public market",
    icon: "🥐",
    tags: ["market", "food", "dessert"],
  },
  {
    id: "ott-canal",
    ...
    price: "Free",
    vibe: "Scenic walk with historic city feel",
    bestTime: "Afternoon",
    safety: "Public path; daytime recommended",
    icon: "⛸️",
    tags: ["walk", "tourist", "views"],
  },
];

const placeSearchText = (place: PlaceItem) =>
  [
    place.name,
    place.city,
    place.country,
    ...
    .join(" ")
    .toLowerCase();
const isSafeFirstDatePlace = (place: PlaceItem) =>
  /public|staffed|busy|daytime|museum|market|transit|active|indoor|partner|main/.test(
    placeSearchText(place),
  );
const isReservablePlace = (place: PlaceItem) =>
  ["Restaurant", "Cafe", "Lounge", "Cultural", "Dessert"].includes(place.kind);
const isPremiumPlace = (place: PlaceItem) =>
  place.price.includes("$$$") ||
  /premium|upscale|rooftop|dinner|views|lounge|yorkville|tower/.test(
    placeSearchText(place),
  );
const isCommunityPlace = (place: PlaceItem) =>
  /indian|chai|spice|culture|dessert|vegetarian|food|market|tea/.test(
    placeSearchText(place),
  );
const buildMarketplaceSnapshot = () =>
  buildDateMarketplaceSnapshot({
    ...
    hasIndianMixers: eventExperiences.some((event) =>
      /indian|south asian|punjabi|gujarati|chai|culture/i.test(
        `${event.title} ${event.body}`,
      ),
    ),
    hasSpeedVideoEvents: eventExperiences.some((event) =>
      /speed|video/i.test(`${event.title} ${event.body} ${event.type}`),
    ),
    hasPremiumDinners: eventExperiences.some((event) =>
      /premium|invite|dinner|executive/i.test(
...
const buildLiveMarketplaceOpsSnapshot = () =>
  buildMarketplaceOpsSnapshot({
    venueCount: placeDirectory.length,
    ...
    eventHostCount: launchMarketplaceCoverage.reduce(
      (sum, city) => sum + city.eventHosts,
      0,
    ),
...
function RelationshipCoach({
  ...
  const [topic, setTopic] = useState(
    "family, emotional safety, and first-date clarity",
  );
  const [saved, setSaved] = useState(false);
  const reasons = matchReasons(match, preferences);
  const redFlags = [
    "Asks to move off-app too fast",
    "Pushes for exact location",
    "Requests money, crypto or gift cards",
    "Avoids verification or public places",
  ];
  const output =
    selected === "First-message helper"
  ...
  return (
    <LinearGradient
      colors={["#27040A", colors.black, colors.black]}
      style={{ flex: 1 }}
    >
      <SafeAreaView style={shared.safe}>
        ...
        <ScrollView
          contentContainerStyle={coachStyles.content}
          showsVerticalScrollIndicator={false}
        >
          <View style={coachStyles.hero}>
            <PremiumIcon name="sparkles" tone="ruby" size={70} iconSize={32} />
            <Text style={launchStyles.scriptHero}>Helpful, never fake</Text>
            <Text style={[shared.h1, { textAlign: "center" }]}>
              Make dating feel clearer.
            </Text>
            <Text style={[shared.body, { textAlign: "center" }]}>
              Coach uses only DestinyOne profile inputs, filters and in-app
              signals. It helps you show up better — it does not pretend to be
              you.
            </Text>
          </View>
          <View style={coachStyles.cardGrid}>
            {coachCards.map((card, index) => (
              <Pressable
                ...
                style={[
                  coachStyles.toolCard,
                  selected === card.title && coachStyles.toolCardOn,
                ]}
              >
                <PremiumIcon
                  name={card.icon}
                  ...
                  iconSize={22}
                />
                <Text style={coachStyles.toolTitle}>{card.title}</Text>
                <Text style={coachStyles.toolBody}>{card.body}</Text>
              </Pressable>
            ))}
          </View>
          <View style={coachStyles.outputCard}>
            <Text style={styles.kicker}>WHAT SHOULD COACH FOCUS ON?</Text>
            <TextInput
              value={topic}
              onChangeText={setTopic}
              placeholder="Example: family, faith, first-date clarity"
              placeholderTextColor="#6F6875"
              style={coachStyles.coachInput}
            />
            <Text style={styles.kicker}>COACH OUTPUT</Text>
            <Text style={styles.cardTitle}>{selected}</Text>
            {selected === "Red-flag scan" ? (
              <View style={{ gap: 9 }}>
                {redFlags.map((item) => (
                  <View key={item} style={coachStyles.checkRow}>
                    <PremiumIcon
                      name="shield-checkmark-outline"
                      tone="gold"
                      size={30}
                      iconSize={14}
                    />
                    <Text style={coachStyles.checkText}>{item}</Text>
                  </View>
                ))}
              </View>
            ) : (
              <Text style={coachStyles.outputText}>“{output}”</Text>
            )}
            <View style={coachStyles.coachActions}>
              <Pressable
                onPress={saveNote}
                style={[coachStyles.rsvpButton, { flex: 1 }]}
              >
                <Text style={coachStyles.rsvpText}>
                  {saved ? "Saved" : "Save note"}
                </Text>
              </Pressable>
              <Pressable
                onPress={() => onUseInChat(output)}
                style={[
                  coachStyles.rsvpButton,
                  { flex: 1, backgroundColor: "#7A1024" },
                ]}
              >
                <Text style={coachStyles.rsvpText}>Use in chat</Text>
              </Pressable>
            </View>
            {saved && (
              <View style={coachStyles.savedNote}>
                <MiniPremiumIcon
                  name="checkmark-circle"
                  tone="gold"
                  size={28}
                  iconSize={13}
                />
                <Text style={coachStyles.savedNoteText}>
                  Coach note saved for this preview session. Production will
          ...
          <View style={coachStyles.outputCard}>
            ...
            <Text style={styles.helper}>For {match.name}, the coach sees:</Text>
            ...
            <Text style={styles.helper}>
              No percentages are shown. The algorithm keeps scoring internal and
              explains matches in plain language.
            </Text>
          </View>
          <View style={coachStyles.boundaryCard}>
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.helper}>
                Coach never reads phone search history, contacts, external chats
                or photos you did not choose.
              </Text>
            </View>
          </View>
          <Button
            label="Tune my match filters"
            variant="secondary"
            icon="options"
...
function EventsHub({
  ...
  const [rsvpEvent, setRsvpEvent] = useState<
    (typeof eventExperiences)[number] | null
  >(null);
  ...
  const [partnerRequest, setPartnerRequest] = useState<PartnerRequest>({
    ...
    city: "New York, NY",
    contact: "",
    packageTitle: "First Date Safe Café",
  });
  ...
  const tonightPicks = placeDirectory
    .filter((place) => isSafeFirstDatePlace(place) && isReservablePlace(place))
    .slice(0, 3);
  const rsvp = (event: (typeof eventExperiences)[number]) =>
    setRsvpEvent(event);
  ...
  const updatePartnerRequest = (key: keyof PartnerRequest, value: string) =>
    setPartnerRequest((current) => ({ ...current, [key]: value }));
  const submitPartnerRequest = () => {
    setPartnerStatus(
      `${partnerRequest.venue.trim() || "Partner venue"} queued for review in ${partnerRequest.city}. Production will send this to partner CRM.`,
    );
    setPartnerOpen(false);
  };
  return (
    <LinearGradient
      colors={["#260006", colors.black, colors.black]}
      style={{ flex: 1 }}
    >
      <SafeAreaView style={shared.safe}>
        <View style={coachStyles.header}>
        ...
        >
          <View style={coachStyles.hero}>
            ...
            <Text style={[shared.h1, { textAlign: "center" }]}>
              Places, packages, events.
            </Text>
            <Text style={[shared.body, { textAlign: "center" }]}>
              A safer first-date marketplace for Indians in USA/Canada: public
              venues, easy reservations, community mixers, video events and
              premium invite-only dinners.
            </Text>
          </View>
          <View style={coachStyles.eventStats}>
            <EventStat
              value={`${placeDirectory.length}`}
              label="curated places"
            />
            <EventStat
              value={`${placeCities.length - 1}`}
              label="main cities"
            />
          ...
          <View style={coachStyles.searchPanel}>
            <View style={selectorStyles.searchBox}>
              ...
              <TextInput
                ...
                placeholder="Search: safe café, Indian dinner, NYC tourist..."
                placeholderTextColor="#6F6875"
                style={selectorStyles.searchInput}
              />
              {!!query && (
            ...
            <ScrollView
              ...
              contentContainerStyle={{ gap: 8 }}
            >
              {placeKinds.map((option) => (
                <Pressable
                  key={option}
                  onPress={() => setKind(option)}
                  style={[
                    coachStyles.filterPill,
                    kind === option && coachStyles.filterPillOn,
                  ]}
                >
                  <Text
                    style={[
                      coachStyles.filterText,
                      kind === option && { color: colors.ivory },
                    ]}
                  >
                    {option}
                  </Text>
            ...
            <ScrollView
              ...
              contentContainerStyle={{ gap: 8 }}
            >
              {placeCities.map((option) => (
                <Pressable
                  key={option}
                  onPress={() => setCity(option)}
                  style={[
                    coachStyles.cityPill,
                    city === option && coachStyles.cityPillOn,
                  ]}
                >
                  <Text
                    style={[
                      coachStyles.cityText,
                      city === option && { color: colors.ivory },
                    ]}
                  >
                    {option}
                  </Text>
          ...
          <View style={coachStyles.marketFilterGrid}>
            ...
            <MarketToggle
              ...
              onPress={() => setReservableOnly((value) => !value)}
            />
            <MarketToggle
              icon="people"
              label="Indian/community"
              active={communityOnly}
              onPress={() => setCommunityOnly((value) => !value)}
            />
            <MarketToggle
              icon="diamond"
              label="Premium / invite"
              active={premiumOnly}
              onPress={() => setPremiumOnly((value) => !value)}
            />
          </View>
          <View style={coachStyles.boundaryCard}>
            <PremiumIcon
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.helper}>
                First meetings should be public, easy to leave, and never
                require sharing home address or private transport. Check-ins are
                on by default in Date Concierge.
              </Text>
            </View>
          </View>
          <TonightSafePicks
          ...
          <View style={{ gap: 12 }}>
            <View style={shared.row}>
              <Text style={styles.sectionLabel}>
                ...
                premiumOnly
                  ? "SEARCH RESULTS"
                  : "FIRST DATE SAFE PLACES"}
              </Text>
              <View style={shared.spacer} />
              <Text style={coachStyles.resultCount}>
                {filtered.length} found · {saved.length} saved
              </Text>
            </View>
            {featured.map((place) => (
              <PlaceCard
          ...
          {!filtered.length && (
            <View style={[shared.card, { alignItems: "center", gap: 10 }]}>
              <PremiumIcon name="search" tone="ruby" size={54} iconSize={25} />
              <Text style={styles.cardTitle}>No place found</Text>
              <Text style={[styles.helper, { textAlign: "center" }]}>
                Try a city, restaurant, café, park, tourist place, lounge or
                activity keyword.
              </Text>
            </View>
          )}
          <View style={{ gap: 12 }}>
            <View style={shared.row}>
              ...
              <View style={shared.spacer} />
              <Pressable onPress={onOpenDatePlan}>
                <Text style={coachStyles.inlineLink}>Open concierge</Text>
              </Pressable>
            </View>
            {datePackages.map((item) => (
              <DatePackageCard
                key={item.id}
                item={item}
                onPlan={onOpenDatePlan}
          ...
          <View style={{ gap: 12 }}>
            <Text style={styles.sectionLabel}>
              INDIAN MIXERS · VIDEO SPEED DATES · PREMIUM DINNERS
            </Text>
            {eventExperiences.map((event, index) => (
              <View key={event.title} style={coachStyles.eventCard}>
                <PremiumIcon
                  name={
                    (index === 0
                      ? "cafe"
                ...
                <View style={{ flex: 1 }}>
                  <View style={shared.row}>
                    ...
                    <View style={shared.spacer} />
                    <View style={coachStyles.eventType}>
                      <Text style={coachStyles.eventTypeText}>
                        {event.type}
                      </Text>
                    </View>
                  </View>
                  <Text style={coachStyles.eventMeta}>
                    {event.city} · {event.date}
                  </Text>
                  <Text style={styles.helper}>{event.body}</Text>
                  <View style={coachStyles.eventFooter}>
                    <View style={coachStyles.eventTag}>
                      <PremiumIcon
                        name="shield-checkmark"
                        tone="gold"
                        size={24}
                        iconSize={11}
                      />
                      <Text style={coachStyles.eventTagText}>{event.tag}</Text>
                    </View>
                    <Pressable
                      onPress={() => rsvp(event)}
                      style={coachStyles.rsvpButton}
                    >
                      <Text style={coachStyles.rsvpText}>RSVP</Text>
                    </Pressable>
                  </View>
                </View>
              </View>
          ...
          <View style={ventureStyles.section}>
            <Text style={styles.sectionLabel}>
              PARTNER / RESERVATION PIPELINE
            </Text>
            {partnerPipeline.map(([title, body, done]) => (
              <ChecklistRow key={title} title={title} body={body} done={done} />
            ))}
          </View>
          <ReservationOpsCard />
          <View style={coachStyles.partnerCta}>
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.cardTitle}>
                Restaurant/café partner intake
              </Text>
              <Text style={styles.helper}>
                Use this to queue partner venues for package menus, reservation
                holds, support SLA and safety review.
              </Text>
              {!!partnerStatus && (
                <Text style={coachStyles.partnerStatus}>{partnerStatus}</Text>
              )}
            </View>
            <Pressable
              ...
              style={coachStyles.rsvpButton}
            >
              <Text style={coachStyles.rsvpText}>Add partner</Text>
            </Pressable>
          </View>
          <View style={coachStyles.boundaryCard}>
            <PremiumIcon
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.cardTitle}>
                Live Places + Reservation API ready
              </Text>
              <Text style={styles.helper}>
                Preview uses curated data. Production can connect Google
                Places/Yelp/Tripadvisor-style venue providers plus
                OpenTable/SevenRooms/Toast-style reservation adapters.
              </Text>
            </View>
          </View>
          <Button
            label="Open Date Concierge"
            icon="calendar"
            onPress={onOpenDatePlan}
          />
          <Text style={styles.legal}>
            Places are curated preview suggestions, not live availability.
            Always verify hours and meet safely in public.
          </Text>
        </ScrollView>
        <PlaceDetailModal
          place={selected}
        ...
        />
        <PartnerInterestSheet
          visible={partnerOpen}
          request={partnerRequest}
          onChange={updatePartnerRequest}
          onClose={() => setPartnerOpen(false)}
          onSubmit={submitPartnerRequest}
        />
      </SafeAreaView>
    </LinearGradient>
  );
...
function DateMarketplaceCard({
  snapshot,
}: {
  snapshot: DateMarketplaceSnapshot;
}) {
  return (
    <View style={coachStyles.marketReadinessCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={snapshot.ready ? "checkmark-circle" : "construct"}
          tone={snapshot.ready ? "gold" : "ruby"}
          size={52}
          iconSize={24}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.cardTitle}>
            {snapshot.ready
              ? "Marketplace preview ready"
              : "Marketplace needs attention"}{" "}
            · {snapshot.score}%
          </Text>
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} pillars ready before live
      ...
      <View style={coachStyles.marketPillarGrid}>
        {snapshot.pillars.map((pillar) => (
          <View
            key={pillar.id}
            style={[
              coachStyles.marketPillar,
              pillar.ready && coachStyles.marketPillarOn,
            ]}
          >
            <MiniPremiumIcon
              name={pillar.ready ? "checkmark-circle" : "ellipse-outline"}
              tone={pillar.ready ? "gold" : "dark"}
              size={24}
              iconSize={11}
            />
            <View style={{ flex: 1 }}>
...
function LiveMarketplaceOpsCard({
  snapshot,
}: {
  snapshot: MarketplaceOpsSnapshot;
}) {
  return (
    <View style={coachStyles.liveOpsCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={
            snapshot.status === "Ready for live ops"
              ? "rocket"
              : "construct-outline"
          }
          tone={snapshot.status === "Ready for live ops" ? "gold" : "ruby"}
          size={52}
          iconSize={24}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} operations pillars ready for
            real bookings, events and support.
          </Text>
        </View>
      </View>
      <View style={adminOpsStyles.qualityTrack}>
        <View
          style={[adminOpsStyles.qualityFill, { width: `${snapshot.score}%` }]}
        />
      </View>
      ...
      <View style={coachStyles.opsCityGrid}>
        {snapshot.cityCoverage.map((city) => (
          <View key={city.city} style={coachStyles.opsCityCard}>
            <Text style={coachStyles.opsCityName}>{city.city}</Text>
            <Text style={coachStyles.opsCityMeta}>
              {city.partnerLeads} leads · {city.signedPartners} signed
            </Text>
            <Text style={coachStyles.opsCityMeta}>
              {city.eventHosts} host · {city.monthlyEvents}/mo ·{" "}
              {city.capacitySeats} seats
            </Text>
          </View>
        ))}
      </View>
      <View style={coachStyles.marketPillarGrid}>
        {snapshot.pillars.map((pillar) => (
          ...
          >
            <MiniPremiumIcon
              name={pillar.ready ? "checkmark-circle" : "construct-outline"}
              tone={pillar.ready ? "gold" : "rose"}
              size={24}
              iconSize={11}
            />
            <View style={{ flex: 1 }}>
              ...
              <Text style={coachStyles.marketPillarBody}>{pillar.body}</Text>
              {!pillar.ready && (
                <Text style={coachStyles.marketPillarNext}>
                  Next: {pillar.nextStep}
                </Text>
              )}
            </View>
          </View>
...
function CityLaunchRoadmap() {
  return (
    <View style={coachStyles.launchRoadmap}>
      ...
      >
        {launchCityRoadmap.map((item) => (
          <View key={item.city} style={coachStyles.launchCityCard}>
            <MiniPremiumIcon
              name={item.icon}
              tone={item.stage.includes("Launch") ? "gold" : "rose"}
              size={32}
              iconSize={15}
            />
            ...
            <View style={coachStyles.launchEventPill}>
              <Text style={coachStyles.launchEventText}>{item.event}</Text>
            </View>
          </View>
        ))}
      </ScrollView>
...
function MarketToggle({
  ...
  return (
    <Pressable
      onPress={onPress}
      style={[coachStyles.marketToggle, active && coachStyles.marketToggleOn]}
    >
      <MiniPremiumIcon
        name={icon}
        tone={active ? "gold" : "rose"}
        size={30}
        iconSize={14}
      />
      <Text
        style={[
          coachStyles.marketToggleText,
...
function DatePackageCard({
  ...
  return (
    <View style={coachStyles.packageCard}>
      <PremiumIcon
        name={item.icon}
        tone={item.tier.includes("Executive") ? "gold" : "ruby"}
        size={54}
        iconSize={25}
      />
      <View style={{ flex: 1 }}>
        <View style={shared.row}>
          ...
          <View style={coachStyles.packageTier}>
            <Text style={coachStyles.packageTierText}>{item.tier}</Text>
          </View>
        </View>
        ...
        <View style={coachStyles.eventFooter}>
          <View style={coachStyles.eventTag}>
            <PremiumIcon
              ...
              iconSize={11}
            />
            <Text style={coachStyles.eventTagText}>{item.safety}</Text>
          </View>
        </View>
        <View style={coachStyles.packageFooter}>
          <Text style={coachStyles.packagePrice}>{item.price}</Text>
          <Pressable onPress={onPlan} style={coachStyles.rsvpButton}>
            <Text style={coachStyles.rsvpText}>Plan package</Text>
          </Pressable>
        </View>
      </View>
    </View>
    ...
    <View style={coachStyles.tonightPanel}>
      ...
      <View style={coachStyles.tonightGrid}>
        {places.map((place) => (
          <Pressable
            key={place.id}
            onPress={() => onDetail(place)}
            style={coachStyles.tonightCard}
          >
            <MiniPremiumIcon
              name={placeKindIcon(place.kind)}
              tone="gold"
              size={32}
              iconSize={15}
            />
            <View style={{ flex: 1 }}>
              ...
              <Text style={coachStyles.tonightBody}>
                {place.city} · {place.bestTime}
              </Text>
            </View>
            <Pressable onPress={onPlan} style={coachStyles.tonightPlan}>
              <Text style={coachStyles.tonightPlanText}>Plan</Text>
            </Pressable>
          </Pressable>
        ))}
      </View>
...
function ReservationOpsCard() {
  return (
    <View style={coachStyles.opsCard}>
      <View style={shared.row}>
        <PremiumIcon
        ...
        <View style={{ flex: 1, marginLeft: 10 }}>
          <Text style={styles.cardTitle}>
            Reservation + safety operating model
          </Text>
          <Text style={styles.helper}>
            This is the logic a real billion-dollar marketplace needs before
            live bookings.
          </Text>
        </View>
      </View>
      <View style={coachStyles.opsGrid}>
        {reservationOps.map((item) => (
          <View key={item.title} style={coachStyles.opsItem}>
            <MiniPremiumIcon
              name={item.icon}
              tone="rose"
              size={32}
      ...
      <View style={coachStyles.checklistWrap}>
        {safeDateChecklist.map((item) => (
          <View key={item} style={coachStyles.safeCheckItem}>
            <MiniPremiumIcon
              name="checkmark-circle"
              tone="gold"
              size={22}
              iconSize={10}
            />
            <Text style={coachStyles.safeCheckText}>{item}</Text>
          </View>
        ))}
      </View>
    </View>
...
function PartnerInterestSheet({
  ...
  return (
    ...
    >
      ...
      <SafeAreaView style={chatStyles.sheet}>
        <SheetHeader
          title="Add restaurant/café partner"
          subtitle="Preview partner intake"
          onClose={onClose}
        />
        <View style={coachStyles.partnerIntakeHero}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.helper}>
              Production will send this to a CRM or partner table with safety,
              reservation and support checks.
            </Text>
          </View>
        </View>
        <Field
          label="Venue name"
          value={request.venue}
          onChangeText={(text: string) => onChange("venue", text)}
          placeholder="Example: Saffron Lounge"
        />
        <Field
          label="City"
          value={request.city}
          onChangeText={(text: string) => onChange("city", text)}
          placeholder="New York, NY"
        />
        <Field
          label="Partner contact"
          value={request.contact}
          onChangeText={(text: string) => onChange("contact", text)}
        ...
        <View style={{ gap: 8 }}>
          ...
          >
            {datePackages.map((item) => (
              <Pressable
                key={item.id}
                onPress={() => onChange("packageTitle", item.title)}
                style={[
                  coachStyles.partnerPackageChip,
                  request.packageTitle === item.title &&
                    coachStyles.partnerPackageChipOn,
                ]}
              >
                <Text
                  style={[
                    coachStyles.partnerPackageText,
                    request.packageTitle === item.title && {
                      color: colors.ivory,
                    },
                  ]}
        ...
        <Button
          label="Queue partner review"
          icon="checkmark-circle"
          onPress={onSubmit}
        />
        <Button label="Cancel" variant="secondary" onPress={onClose} />
        <Text style={styles.legal}>
          Preview only. Live partner onboarding needs contracts, payment terms,
          refund policy, safety SLA and provider webhooks.
        </Text>
      </SafeAreaView>
    </Modal>
  );
...
function EventRsvpSheet({
  ...
  return (
    <Modal visible transparent animationType="slide" onRequestClose={onClose}>
      ...
      <SafeAreaView style={chatStyles.sheet}>
        <SheetHeader
          title="RSVP preview saved"
          subtitle={`${event.city} · ${event.date}`}
          onClose={onClose}
        />
        <View style={coachStyles.rsvpConfirm}>
          <PremiumIcon name="ticket" tone="gold" size={58} iconSize={27} />
          <View style={{ flex: 1 }}>
            <Text style={styles.cardTitle}>{event.title}</Text>
            <Text style={styles.helper}>{event.body}</Text>
          </View>
        ...
        <View style={coachStyles.detailRows}>
          <DetailRow icon="location-outline" label="City" value={event.city} />
          <DetailRow icon="calendar-outline" label="When" value={event.date} />
          <DetailRow
            icon="shield-checkmark-outline"
            label="Trust"
            value={event.tag}
        ...
        <Text style={styles.legal}>
          Production will connect tickets, city capacity and ID-verified
          check-in.
        </Text>
      </SafeAreaView>
    </Modal>
...
function PlaceCard({
  ...
  return (
    <View
      style={[coachStyles.placeCard, compact && coachStyles.placeCardCompact]}
    >
      <PremiumIcon
        name={placeKindIcon(place.kind)}
        tone={
          place.kind === "Cafe" || place.kind === "Dessert" ? "gold" : "ruby"
        }
        size={52}
        iconSize={24}
      />
      <View style={{ flex: 1 }}>
        <View style={shared.row}>
          ...
          <Pressable onPress={onSave} style={premiumButtonStyles.iconOnly}>
            <PremiumIcon
              name={saved ? "bookmark" : "bookmark-outline"}
              tone={saved ? "gold" : "dark"}
              size={34}
              iconSize={15}
            />
          </Pressable>
        ...
        <Text style={coachStyles.eventMeta}>
          {place.city} · {place.kind} · {place.price}
        </Text>
        <Text style={styles.helper}>{place.vibe}</Text>
        <View style={coachStyles.placeLabelRow}>
          {labels.slice(0, compact ? 2 : 4).map((label) => (
            <View key={label} style={coachStyles.placeLabel}>
              <Text style={coachStyles.placeLabelText}>{label}</Text>
            </View>
          ))}
        </View>
        <View style={coachStyles.eventFooter}>
          <View style={coachStyles.eventTag}>
            <PremiumIcon
              ...
              iconSize={11}
            />
            <Text style={coachStyles.eventTagText}>{place.bestTime}</Text>
          </View>
          <Pressable onPress={onDetail} style={coachStyles.detailsButton}>
            <Text style={coachStyles.detailsText}>Details</Text>
          </Pressable>
          <Pressable onPress={onPlan} style={coachStyles.rsvpButton}>
            <Text style={coachStyles.rsvpText}>Plan</Text>
          </Pressable>
        </View>
      </View>
    </View>
  ...
  return (
    <Modal visible transparent animationType="slide" onRequestClose={onClose}>
      ...
      <SafeAreaView style={chatStyles.sheet}>
        <SheetHeader
          title={place.name}
          subtitle={`${place.city} · ${place.kind}`}
          onClose={onClose}
        />
        <View style={coachStyles.placeDetailHero}>
          <PremiumIcon
            name={placeKindIcon(place.kind)}
            tone="ruby"
            size={58}
          ...
          />
          <View style={{ flex: 1 }}>
            <Text style={styles.cardTitle}>{place.area}</Text>
            <Text style={styles.helper}>{place.vibe}</Text>
          </View>
        </View>
        <View style={coachStyles.detailRows}>
          ...
          <DetailRow
            ...
            value={
              isReservablePlace(place)
                ? "Partner-ready candidate · reservation adapter prepared"
                : "Public place · verify live hours before meeting"
            }
          />
          <DetailRow
            ...
            value={
              isReservablePlace(place)
                ? "Hold + quote + confirmation flow ready for API connection"
                : "Walk-in/date-plan suggestion only"
            }
          />
        </View>
        ...
        <View style={{ gap: 10 }}>
          <Button
            ...
            onPress={onSave}
          />
          <Button label="Plan this date" icon="calendar" onPress={onPlan} />
        </View>
        <Text style={styles.legal}>
          Live hours, map links, restaurant inventory and reservation
          confirmation connect in production.
        </Text>
      </SafeAreaView>
    </Modal>
  );
...
function DetailRow({
  ...
  return (
    <View style={coachStyles.detailRow}>
      <PremiumIcon name={icon} tone="ruby" size={34} iconSize={15} />
      <View style={{ flex: 1 }}>
        <Text style={coachStyles.detailLabel}>{label}</Text>
        <Text style={coachStyles.detailValue}>{value}</Text>
      </View>
    </View>
  );
}

function EventStat({ value, label }: { value: string; label: string }) {
  return (
    <View style={coachStyles.eventStat}>
      <Text style={coachStyles.eventStatValue}>{value}</Text>
      <Text style={coachStyles.eventStatLabel}>{label}</Text>
    </View>
  );
}
...
const executiveRequirements = [
  [
    "Business identity",
    "Founder, business owner, investor, doctor, lawyer, executive or senior professional.",
  ],
  [
    "Serious intent",
    "Long-term relationship or marriage only — no casual dating positioning.",
  ],
  [
    "Verification",
    "Selfie, phone/email, optional ID and business/profile verification before approval.",
  ],
  [
    "Privacy standard",
    "No public income display. Sensitive checks remain private and manual.",
  ],
] as const;
const executiveServices = [
  {
    title: "Private matchmaker review",
    body: "A concierge reviews goals, lifestyle, city, family expectations and privacy needs before intros.",
    icon: "person" as const,
  },
  {
    title: "Handpicked weekly introductions",
    body: "No endless swiping. Members receive a small set of serious, verified prospects.",
    icon: "heart" as const,
  },
  {
    title: "VIP date planning",
    body: "Curated restaurant, lounge, café or private event suggestions with optional reservation holds.",
    icon: "calendar" as const,
  },
  {
    title: "Luxury gifting",
    body: "Flowers, dessert, handwritten card or premium surprise gifts can be ordered inside the app.",
    icon: "gift" as const,
  },
  {
    title: "Executive privacy mode",
    body: "Hide from public discovery and appear only to approved Executive Circle members.",
    icon: "eye-off" as const,
  },
  {
    title: "Priority safety support",
    body: "Faster report review, scam checks, verified events and safer date check-ins.",
    icon: "shield-checkmark" as const,
  },
];
const executiveMatches = [
  {
    ...
    vibe: "Family-first · Ambitious · Private",
    photo: matches[4]!.photo,
  },
  {
    name: "Karan",
    age: 35,
    ...
    vibe: "Business-minded · Foodie · Spiritual",
    photo: matches[8]!.photo,
  },
  {
    name: "Dev",
    age: 33,
    ...
    city: "San Francisco, CA",
    intent: "Open to marriage",
    vibe: "Travel · Fitness · Long-term",
    photo: matches[12]!.photo,
  },
] as const;
const executiveApplicationSteps = [
  [
    "Application submitted",
    "Member answers privacy, intent, city and relationship goals.",
  ],
  [
    "Verification review",
    "Selfie, optional ID, business proof and profile quality are checked privately.",
  ],
  [
    "Concierge interview",
    "A short call confirms expectations and dating boundaries.",
  ],
  [
    "Approved circle",
    "Member unlocks private intros, events, gifting and VIP date planning.",
  ],
] as const;
function ExecutiveCircle({
  navigate,
  onBack,
  onOpenEvents,
  onOpenPricing,
  ...
  const [tab, setTab] = useState<
    "overview" | "apply" | "matches" | "concierge"
  >("overview");
  const [application, setApplication] = useState({
    role: "Founder / business owner",
    city: "New York, NY",
    intent: "Marriage in 12–24 months",
    privacy: "Hidden profile",
  });
  const [conciergeNote, setConciergeNote] = useState(
    "Plan a quiet premium dinner with serious conversation.",
  );
  ...
  const submitApplication = () => {
    setApplicationError("");
    const missing = Object.entries(application).find(
      ([, value]) => value.trim().length < 3,
    );
    if (missing) {
      setApplicationError(
        "Please complete every application field before submitting.",
      );
      return;
    }
    setStatus({
      title: "Application moved to private review",
      body: `${application.role} · ${application.city} · ${application.intent}. Next: verification + concierge interview.`,
    });
    setTab("apply");
  };
  const requestIntro = (name: string) =>
    setStatus({
      title: `Intro request queued for ${name}`,
      body: "Concierge will review compatibility, privacy preference and relationship intent before any introduction is shown.",
    });
  const sendGift = (name: string) =>
    setStatus({
      title: `Luxury gift request started for ${name}`,
      body: "Choose Real Gift in chat to create the private accept → pay → courier flow. Recipient address remains hidden.",
    });
  const askConcierge = () => {
    setConciergeError("");
    ...
    if (note.length < 20) {
      setConciergeError(
        "Write at least 20 characters so concierge has useful context.",
      );
      return;
    }
    setStatus({ title: "Concierge note saved", body: note });
  };
  return (
    <LinearGradient
      colors={["#250006", colors.black, colors.black]}
      style={{ flex: 1 }}
    >
      <SafeAreaView style={shared.safe}>
        <View style={coachStyles.header}>
        ...
        <ScrollView
          contentContainerStyle={[coachStyles.content, { paddingBottom: 120 }]}
          showsVerticalScrollIndicator={false}
        >
          <View style={ventureStyles.hero}>
            <PremiumIcon name="briefcase" tone="gold" size={68} iconSize={30} />
            <Text style={launchStyles.scriptHero}>
              Private. Verified. Serious.
            </Text>
            <Text style={[shared.h1, { textAlign: "center" }]}>
              DestinyOne Executive Circle.
            </Text>
            <Text style={[shared.body, { textAlign: "center" }]}>
              Invite-only matchmaking for founders, business owners, investors
              and high-performing professionals who value time, privacy and real
              commitment.
            </Text>
            <View style={ventureStyles.priceSeal}>
              <Text style={ventureStyles.priceSealText}>$5,000 / year</Text>
              <Text style={ventureStyles.priceSealSub}>
                application required
              </Text>
            </View>
          </View>
          <View style={ventureStyles.metricGrid}>
            {executiveMetrics.map((item) => (
              <MetricPill key={item.label} {...item} />
            ))}
          </View>
          <View style={ventureStyles.tabRow}>
            {(["overview", "apply", "matches", "concierge"] as const).map(
              (item) => (
                <Pressable
                  key={item}
                  onPress={() => setTab(item)}
                  style={[
                    ventureStyles.tabButton,
                    tab === item && ventureStyles.tabButtonOn,
                  ]}
                >
                  <Text
                    style={[
                      ventureStyles.tabText,
                      tab === item && { color: colors.ivory },
                    ]}
                  >
                    {item === "overview"
                      ? "Overview"
          ...
          {!!status.title && (
            <View style={ventureStyles.statusCard}>
              <MiniPremiumIcon
                name="checkmark-circle"
                tone="gold"
                size={34}
              ...
              />
              <View style={{ flex: 1 }}>
                <Text style={ventureStyles.statusTitle}>{status.title}</Text>
                <Text style={styles.helper}>{status.body}</Text>
              </View>
              <Pressable onPress={() => setStatus({ title: "", body: "" })}>
                <MiniPremiumIcon
                  name="close"
                  tone="dark"
                  size={28}
            ...
            <View style={ventureStyles.section}>
              ...
              <View style={coachStyles.cardGrid}>
                {executiveServices.map((service, index) => (
                  <View key={service.title} style={coachStyles.toolCard}>
                    <PremiumIcon
                      name={service.icon}
                      tone={index % 2 ? "gold" : "ruby"}
                      size={44}
                      iconSize={20}
                    />
                    <Text style={coachStyles.toolTitle}>{service.title}</Text>
                    <Text style={coachStyles.toolBody}>{service.body}</Text>
                  </View>
                ))}
              </View>
              <View style={ventureStyles.actionGrid}>
                <Button
                  label="Apply for Executive Circle"
                  icon="briefcase"
                  variant="gold"
                  onPress={() => setTab("apply")}
                />
                <Button
                  label="View private matches"
                  icon="heart"
                  variant="secondary"
                  onPress={() => setTab("matches")}
                />
                <Button
                  label="Open annual pricing"
                  icon="diamond"
                  variant="secondary"
              ...
              <View style={coachStyles.boundaryCard}>
                ...
                />
                <View style={{ flex: 1 }}>
                  <Text style={styles.cardTitle}>No public wealth display</Text>
                  <Text style={styles.helper}>
                    Income, business proof and ID checks should stay private.
                    Members only see badges like “Business verified” or
                    “Executive approved.”
                  </Text>
                </View>
              </View>
          ...
          {tab === "apply" && (
            <View style={ventureStyles.section}>
              ...
              {executiveRequirements.map(([title, body], index) => (
                <ChecklistRow
                  key={title}
                  title={title}
                  body={body}
              ...
              <View style={ventureStyles.applicationCard}>
                <Field
                  label="Professional role"
                  value={application.role}
                  onChangeText={(role: string) => {
                    setApplicationError("");
                ...
                <Field
                  ...
                  onChangeText={(city: string) => {
                    setApplicationError("");
                    setApplication((current) => ({ ...current, city }));
                  }}
                />
                <Field
                  label="Relationship intent"
                  value={application.intent}
                  onChangeText={(intent: string) => {
                    setApplicationError("");
                    setApplication((current) => ({ ...current, intent }));
                  }}
                />
                <Field
                  label="Privacy preference"
                  value={application.privacy}
                  onChangeText={(privacy: string) => {
                    setApplicationError("");
                    setApplication((current) => ({ ...current, privacy }));
                  }}
                />
                {!!applicationError && (
                  <View style={ventureStyles.errorCard}>
                    <MiniPremiumIcon
                      ...
                      iconSize={13}
                    />
                    <Text style={ventureStyles.errorText}>
                      {applicationError}
                    </Text>
                  </View>
                )}
                <Button
                  label="Submit private application"
                  icon="send"
                  onPress={submitApplication}
                />
              </View>
              <View style={ventureStyles.section}>
                <Text style={styles.sectionLabel}>APPROVAL FLOW</Text>
                {executiveApplicationSteps.map(([title, body], index) => (
                  <ChecklistRow
                    key={title}
                    title={title}
                    body={body}
              ...
              <Button
                label="Complete verification first"
                icon="id-card"
                variant="secondary"
          ...
          {tab === "matches" && (
            <View style={ventureStyles.section}>
              <View style={shared.row}>
                <Text style={styles.sectionLabel}>
                  EXECUTIVE-ONLY SAMPLE MATCHES
                </Text>
                <View style={shared.spacer} />
                <Pressable onPress={() => setTab("apply")}>
                  <Text style={coachStyles.resultCount}>Apply</Text>
                </Pressable>
              </View>
              {executiveMatches.map((person) => (
                <View
                  key={person.name}
                  style={ventureStyles.executiveMatchCard}
                >
                  <Image
                    source={{ uri: person.photo }}
                    style={ventureStyles.executivePhoto}
                  />
                  <LinearGradient
                    colors={["transparent", "rgba(10,0,3,.96)"]}
                    style={StyleSheet.absoluteFill}
                  />
                  <View style={ventureStyles.executiveMatchInfo}>
                    <Chip label="Executive approved" gold />
                    <Text style={ventureStyles.executiveName}>
                    ...
                    <Text style={styles.helper}>
                      {person.intent} · {person.vibe}
                    </Text>
                    <View style={styles.chipRow}>
                      <Pressable
                        onPress={() => requestIntro(person.name)}
                        style={coachStyles.rsvpButton}
                      >
                        <Text style={coachStyles.rsvpText}>Request intro</Text>
                      </Pressable>
                      <Pressable
                        onPress={onOpenDatePlan}
                        style={coachStyles.detailsButton}
                      >
                        <Text style={coachStyles.detailsText}>VIP date</Text>
                      </Pressable>
                      <Pressable
                        onPress={() => sendGift(person.name)}
                        style={coachStyles.detailsButton}
                      >
                        <Text style={coachStyles.detailsText}>Gift</Text>
                      </Pressable>
                    </View>
                  </View>
                </View>
          ...
          {tab === "concierge" && (
            <View style={ventureStyles.section}>
              <View style={ventureStyles.conciergeCard}>
                <PremiumIcon
                  name="person"
                  tone="gold"
                  size={64}
                ...
                <Text style={[shared.h2, { textAlign: "center" }]}>
                  Tell us what kind of introduction feels right.
                </Text>
                <TextInput
                  value={conciergeNote}
                  onChangeText={(text) => {
                    setConciergeError("");
                    setConciergeNote(text);
                  }}
                  multiline
                  placeholder="Write concierge request..."
                  placeholderTextColor="#6F6875"
                  style={supportStyles.messageBox}
                />
                <Text style={ventureStyles.charCount}>
                  {conciergeNote.trim().length}/20 minimum
                </Text>
                {!!conciergeError && (
                  <View style={ventureStyles.errorCard}>
                    <MiniPremiumIcon
                ...
                <Button
                  ...
                  icon="send"
                  onPress={askConcierge}
                />
              </View>
              <Button
                label="Book private event / dinner"
                icon="calendar"
                variant="secondary"
                onPress={onOpenEvents}
              />
              <Button
                label="Plan a VIP date"
                icon="restaurant"
                variant="secondary"
                onPress={onOpenDatePlan}
              />
              <Button
                label="Upgrade to Executive annual"
                icon="diamond"
                variant="gold"
                onPress={onOpenPricing}
              />
        ...
        <BottomNav active="executive" navigate={navigate} />
      </SafeAreaView>
    </LinearGradient>
  );
}
...
function MetricPill({
  ...
  return (
    <View style={ventureStyles.metricCard}>
      <PremiumIcon name={icon} tone="gold" size={42} iconSize={19} />
      <Text style={ventureStyles.metricLabel}>{label}</Text>
      <Text style={ventureStyles.metricValue}>{value}</Text>
    </View>
  );
}
...
function ChecklistRow({
  ...
  return (
    <View style={ventureStyles.checklistRow}>
      <MiniPremiumIcon
      ...
      />
      <View style={{ flex: 1 }}>
        <Text style={ventureStyles.checkTitle}>{title}</Text>
        <Text style={styles.helper}>{body}</Text>
      </View>
    </View>
  );
...
function VerificationHub({
  ...
  const [sessionStatus, setSessionStatus] = useState(
    "Current device trusted · session controls ready for backend.",
  );
  const [trustStatus, setTrustStatus] = useState(
    "Trust Engine preview is ready. Connect providers later for real checks.",
  );
  const idVerified = idStatus === "verified";
  const businessVerified = businessStatus === "verified";
  const trustScore =
    (verified ? 25 : 8) +
    (selfieUri || verified ? 15 : 0) +
    (hasVoiceIntro ? 12 : 0) +
    Math.min(vouches.length, 3) * 10 +
    (idVerified ? 18 : idStatus === "submitted" ? 9 : 0) +
    (businessVerified ? 12 : businessStatus === "submitted" ? 6 : 0) +
    (biometricConsent ? 8 : 0) +
    10;
  const steps = [
    {
      title: "Phone / email login",
      body: "OTP demo is available now; production connects Supabase/Twilio style phone OTP.",
      done: true,
      icon: "phone-portrait" as const,
    },
    {
      title: "Biometric consent",
      body: biometricConsent
        ? "Consent recorded for liveness provider handoff."
        : "Required before any selfie/liveness vendor runs in production.",
      done: biometricConsent,
    ...
    },
    {
      ...
      body:
        selfieUri || verified
          ? "Selfie/liveness preview completed."
          : "Add camera or gallery selfie for trust badge.",
      done: !!selfieUri || verified,
      icon: "camera" as const,
    },
    {
      title: "Optional ID check",
      body: idVerified
        ? "ID provider review marked verified."
        : idStatus === "submitted"
          ? "ID provider packet prepared for review."
          : "Production provider can verify ID without showing documents to other members.",
      done: idVerified,
    ...
    },
    {
      title: "Voice intro",
      body: hasVoiceIntro
        ? "Voice intro improves authenticity."
        : "Add a short intro so matches can hear your vibe.",
      done: hasVoiceIntro,
      icon: "mic" as const,
    },
    {
      title: "Friend vouches",
      body: `${vouches.length}/3 character vouches added.`,
      done: vouches.length > 0,
      icon: "people" as const,
    },
    {
      title: "Business verification",
      body: businessVerified
        ? "Executive/business proof marked verified."
        : businessStatus === "submitted"
          ? "Business review packet prepared."
          : "Required for Executive Circle before billing.",
  ...
  const memberBadges = [
    {
      ...
      body: "Shown after phone/email + selfie check.",
      icon: "shield-checkmark" as const,
      done: verified,
    },
    {
      title: "ID Checked",
      body: "Optional badge after private ID provider approval.",
      icon: "id-card" as const,
      done: idVerified,
    },
    {
      title: "Executive Ready",
      body: "Business verification for $5,000/year circle.",
      icon: "briefcase" as const,
      done: businessVerified,
    },
    {
      title: "Voice Intro",
      body: "Signals authenticity without exposing private data.",
      icon: "mic" as const,
      done: hasVoiceIntro,
    },
    {
      title: "Trusted Circle",
      body: "Friend vouches add confidence for serious matches.",
      icon: "people" as const,
      done: vouches.length > 0,
    },
  ];
  const runLiveness = () => {
    if (!biometricConsent) {
      setTrustStatus(
        "Please accept biometric consent before liveness verification.",
      );
      return;
    }
    onVerify();
    setTrustStatus(
      "Selfie/liveness preview completed. Production will call a liveness provider here.",
    );
  };
  const advanceId = () => {
    if (idStatus === "not_started") {
      ...
      setTrustStatus(
        "ID review packet prepared. Production will upload encrypted documents to the provider.",
      );
      return;
    }
    setIdStatus("verified");
    setTrustStatus(
      "ID check marked verified in preview. Public profile only shows a simple badge.",
    );
  };
  const advanceBusiness = () => {
    if (businessStatus === "not_started") {
      setBusinessStatus("submitted");
      setTrustStatus(
        "Business verification packet prepared for Executive Circle review.",
      );
      return;
    }
    setBusinessStatus("verified");
    setTrustStatus(
      "Business verification marked approved. Executive Circle can unlock after real review.",
    );
  };
  const refreshSession = () => {
    setSessionStatus(
      `Trusted device refreshed · ${new Date().toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}`,
    );
    setTrustStatus(
      "Session/device review refreshed. Backend will store trusted devices and revoke controls.",
    );
  };
  const providerChecklist = [
    [
      "Auth provider",
      "Phone/email OTP, resend limits, device/session logs.",
      true,
    ],
    [
      "Liveness vendor",
      "Biometric consent, selfie capture, duplicate-face checks.",
      biometricConsent,
    ],
    [
      "ID provider",
      "Encrypted document upload, age gate, manual review fallback.",
      idStatus !== "not_started",
    ],
    [
      "Business proof",
      "LinkedIn/company docs, concierge review and approval logs.",
      businessStatus !== "not_started",
    ],
    [
      "Fraud rules",
      "Duplicate accounts, money requests, velocity and off-app pressure alerts.",
      true,
    ],
  ] as const;
  return (
        ...
        >
          <View style={ventureStyles.hero}>
            <PremiumIcon
              ...
              iconSize={32}
            />
            <Text style={launchStyles.scriptHero}>Trust should feel calm</Text>
            <Text style={[shared.h1, { textAlign: "center" }]}>
              {verified ? "Verified member" : "Build your trust profile"}
            </Text>
            <Text style={[shared.body, { textAlign: "center" }]}>
              A serious dating app needs proof, privacy and safety — without
              making onboarding feel scary.
            </Text>
          </View>
          <View style={ventureStyles.trustMeter}>
            <View style={shared.row}>
              <View>
                <Text style={styles.kicker}>TRUST LEVEL</Text>
                <Text style={ventureStyles.trustScore}>
            ...
            <View style={ventureStyles.progressTrack}>
              <View
                style={[
                  ventureStyles.progressFill,
                  { width: `${Math.min(100, trustScore)}%` },
                ]}
              />
            </View>
            <Text style={styles.helper}>
              Internal trust signal only. Members see badges, not private
              scores.
            </Text>
          </View>
          <View style={trustHubStyles.badgeGrid}>
            {memberBadges.map((badge) => (
              <View
                key={badge.title}
                style={[
                  trustHubStyles.badgeCard,
                  badge.done && trustHubStyles.badgeCardOn,
                ]}
              >
                <MiniPremiumIcon
                  name={badge.done ? "checkmark-circle" : badge.icon}
                  tone={badge.done ? "gold" : "dark"}
                  size={34}
                  iconSize={16}
                />
                <Text style={trustHubStyles.badgeTitle}>{badge.title}</Text>
                <Text style={trustHubStyles.badgeBody}>{badge.body}</Text>
              </View>
            ))}
          </View>
          <View style={trustHubStyles.statusCard}>
            <MiniPremiumIcon
              name="sparkles"
              tone="gold"
              size={30}
              iconSize={14}
            />
            <Text style={trustHubStyles.statusText}>{trustStatus}</Text>
          </View>
          <Pressable
            onPress={() => {
              setBiometricConsent((value) => !value);
              setTrustStatus(
                !biometricConsent
                  ? "Biometric consent accepted for provider handoff."
                  : "Biometric consent removed in preview.",
              );
            }}
            style={[
              trustHubStyles.consentCard,
              biometricConsent && trustHubStyles.consentCardOn,
            ]}
          >
            <PremiumIcon
              name="finger-print"
              tone={biometricConsent ? "gold" : "ruby"}
              size={46}
              iconSize={21}
            />
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.helper}>
                Required before selfie/liveness checks. Consent is separate from
            ...
            <View
              style={[
                ...
                biometricConsent && discoveryStyles.switchOn,
              ]}
            >
              <View
                style={[
                  discoveryStyles.switchThumb,
                  biometricConsent && discoveryStyles.switchThumbOn,
                ]}
              />
            </View>
          </Pressable>
          <View style={trustHubStyles.actionGrid}>
            <TrustAction
              icon="camera"
              title="Run liveness preview"
              body="Completes the selfie/liveness step after consent."
              cta={verified ? "Refresh" : "Run"}
              onPress={runLiveness}
            />
            <TrustAction
              icon="id-card"
              title="ID provider packet"
              body={
                idStatus === "not_started"
                  ? "Prepare encrypted ID review handoff."
                  : idVerified
                    ? "Verified in preview."
            ...
            <TrustAction
              ...
              body={
                businessStatus === "not_started"
                  ? "Prepare Executive Circle proof review."
                  : businessVerified
                    ? "Executive proof approved."
                    : "Ready for concierge review."
              }
              cta={
                businessStatus === "not_started"
                  ? "Prepare"
          ...
          <View style={trustHubStyles.privacyPanel}>
            <PremiumIcon
              name="eye-off-outline"
              tone="gold"
              size={46}
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.helper}>
                ID documents, selfie source files, exact trust score, reports,
                blocks and safety notes are never shown on public profiles.
              </Text>
            </View>
          </View>
          <View style={ventureStyles.section}>
            {steps.map((step) => (
              <TrustStep key={step.title} {...step} />
            ))}
          </View>
          <View style={coachStyles.boundaryCard}>
            <PremiumIcon
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.helper}>
                Verification photos, ID details and safety reports stay private.
                Public profile only shows simple trust badges.
              </Text>
            </View>
          </View>
          <View style={ventureStyles.section}>
            <Text style={styles.sectionLabel}>PROVIDER READINESS</Text>
            {providerChecklist.map(([title, body, done]) => (
              <ChecklistRow key={title} title={title} body={body} done={done} />
            ))}
          </View>
          <View style={ventureStyles.section}>
            <Text style={styles.sectionLabel}>SECURITY FEATURES TO SHIP</Text>
            {[
              "End-to-end sensitive media rules",
              "Screenshot and abuse reporting hooks",
              "Device/session management",
              "Data export and delete account flow",
              "Blocked-member graph across discovery, likes and chat",
            ].map((item) => (
              <ChecklistRow
                key={item}
                title={item}
                body="Backend-ready requirement for production release."
                done={item.includes("delete") || item.includes("Device")}
              />
            ))}
          </View>
          <Button
...
function TrustAction({
  ...
  return (
    <View style={trustHubStyles.actionCard}>
      <PremiumIcon name={icon} tone="gold" size={42} iconSize={19} />
      <View style={{ flex: 1 }}>
        <Text style={trustHubStyles.actionTitle}>{title}</Text>
        <Text style={styles.helper}>{body}</Text>
      </View>
      <Pressable onPress={onPress} style={trustHubStyles.actionButton}>
        <Text style={trustHubStyles.actionButtonText}>{cta}</Text>
      </Pressable>
    </View>
  );
}
...
function TrustStep({
  ...
  return (
    <View style={ventureStyles.trustStep}>
      <PremiumIcon
        name={icon}
        tone={done ? "gold" : "ruby"}
        size={42}
...
function AdminModerationPanel({
  ...
  const [tab, setTab] = useState<"queue" | "reports" | "playbooks" | "audit">(
    "queue",
  );
  ...
  const [opsNote, setOpsNote] = useState(
    "Trust Ops is ready in preview. Actions update local case status only.",
  );
  const queue = buildModerationQueue(reports, blockedCount).map((item) => ({
    ...item,
    status: caseStatus[item.id] ?? item.status,
  }));
  ...
  const trustOpsSnapshot = buildTrustOpsSnapshot({
    ...
    targetSlaHours: summary.highOrCritical
      ? Math.min(6, summary.fastestSlaHours || 6)
      : 12,
  ...
  const paymentEntitlementSnapshot = buildPaymentEntitlementSnapshot({
    ...
    productionBillingLocked:
      appEnvironment === "production" && paymentsConfigured,
  });
  ...
  const visibleQueue =
    tab === "queue" ? queue : queue.filter((item) => item.humanReviewRequired);
  ...
  const storeReviewReady = (id: string) =>
    storeReviewSnapshot.items.find((item) => item.id === id)?.ready ?? false;
  const legalOpsSnapshot = buildLegalStoreOpsSnapshot({
    ...
    reviewerAccessReady:
      storeReviewReady("reviewer_credentials") &&
      storeReviewReady("review_notes"),
  ...
  const releaseSnapshot = buildReleaseReadinessSnapshot({
    ...
    observabilityConnected:
      observabilitySnapshot.status === "Ready for monitored launch",
    hasStoreAssets: true,
    hasStoreListing: true,
    hasStoreReviewAccess:
      storeReviewReady("reviewer_credentials") &&
      storeReviewReady("review_notes"),
  ...
  const marketplaceSnapshot = buildMarketplaceSnapshot();
  const networkSnapshot = buildNetworkEffectPlan({
    matches,
    selectedCities: [],
    verified: true,
    vouchesCount: 3,
  ...
  const p1Snapshot = buildP1OperationsSnapshot({
    ...
    hasSupportSla: trustOpsSnapshot.status === "Ready for staffed pilot",
    hasLegalDrafts: legalOpsSnapshot.gates.some(
      (gate) => gate.id === "legal_documents" && gate.started,
    ),
    legalUrlsPublished: legalOpsSnapshot.gates.some(
      (gate) => gate.id === "public_urls" && gate.ready,
    ),
  });
  const updateCase = (
  ...
  const automationGuards = [
    [
      "Money scam lock",
      "Gift, payment and chat-send limits trigger when money/crypto/gift-card risk is high.",
    ],
    [
      "Profile integrity",
      "Photo edits and verification mismatches create a trust-review case before more discovery exposure.",
    ],
    [
      "Block graph",
      "Blocked and reported members are removed across discovery, likes and chat.",
    ],
    [
      "Human override",
      "Critical actions require an audit trail and reviewer note before permanent ban.",
    ],
    [
      "Appeal route",
      "Members can contact support if a decision affects access or billing.",
    ],
  ] as const;
  return (
    <LinearGradient
      colors={["#210006", colors.black, colors.black]}
      style={{ flex: 1 }}
    >
      <SafeAreaView style={shared.safe}>
        <View style={coachStyles.header}>
        ...
        >
          <View style={ventureStyles.hero}>
            <PremiumIcon name="analytics" tone="plum" size={70} iconSize={32} />
            <Text style={launchStyles.scriptHero}>
              Safety scales with operations
            </Text>
            <Text style={[shared.h1, { textAlign: "center" }]}>
              Moderation dashboard.
            </Text>
            <Text style={[shared.body, { textAlign: "center" }]}>
              Reports, blocks, scam signals, trust checks and support
              escalations now flow into one human-review queue.
            </Text>
          </View>
          <View style={adminOpsStyles.statGrid}>
            <AdminOpsStat value={`${summary.total}`} label="open cases" />
            <AdminOpsStat
            ...
            />
            <AdminOpsStat
              value={`${summary.fastestSlaHours}h`}
              label="fastest SLA"
            />
          </View>
          <TrustOpsSlaCard snapshot={trustOpsSnapshot} />
          <View style={adminOpsStyles.statusCard}>
            <MiniPremiumIcon
              name="checkmark-circle"
              tone="gold"
              size={30}
              iconSize={14}
            />
            <Text style={adminOpsStyles.statusText}>{opsNote}</Text>
          </View>
          <View style={adminOpsStyles.tabRow}>
            {(["queue", "reports", "playbooks", "audit"] as const).map(
              (item) => (
                <Pressable
                  key={item}
                  onPress={() => setTab(item)}
                  style={[
                    adminOpsStyles.tab,
                    tab === item && adminOpsStyles.tabOn,
                  ]}
                >
                  <Text
                    style={[
                      adminOpsStyles.tabText,
                      tab === item && { color: colors.ivory },
                    ]}
                  >
                    {item === "queue"
          ...
          {tab === "queue" && (
            <View style={ventureStyles.section}>
              <Text style={styles.sectionLabel}>LIVE REVIEW QUEUE</Text>
                ...
                <ModerationCaseCard
                  ...
                  onFreeze={() =>
                    updateCase(
                      item,
                      "frozen",
                      "chat/payment abilities frozen pending review",
                    )
                  }
                  onEscalate={() =>
                    updateCase(
                  ...
                  onResolve={() =>
                    updateCase(
                      item,
                      "resolved",
                      "case resolved with reviewer note",
                    )
                  }
                  onEvidence={() =>
                    setOpsNote(
                      `${item.member}: evidence packet includes ${item.evidence.join(", ")}`,
                    )
                  }
                />
              ))}
          ...
          {tab === "reports" && (
            <View style={ventureStyles.section}>
              <Text style={styles.sectionLabel}>SESSION REPORTS</Text>
              {reports.length ? (
                reports
                  .slice()
                  .reverse()
                  .map((report) => (
                    <View key={report.id} style={adminOpsStyles.reportCard}>
                      <View style={shared.row}>
                        <MiniPremiumIcon
                          name="flag-outline"
                          tone="ruby"
                        ...
                        <Text style={[styles.cardTitle, { flex: 1 }]}>
                          Report on {report.matchId}
                        </Text>
                        <Text style={adminOpsStyles.timeText}>
                      ...
                      <Text style={styles.helper}>{report.reason}</Text>
                      {!!report.details && (
                        <Text style={adminOpsStyles.reportDetails}>
                          {report.details}
                        </Text>
                      )}
                      <View style={adminOpsStyles.reportFooter}>
                        <Text style={adminOpsStyles.footerText}>
                          Saved locally now · Supabase reports table later
                        </Text>
                      </View>
                    </View>
                  ))
              ) : (
                <View style={adminOpsStyles.emptyCard}>
                  <PremiumIcon
                    name="shield-checkmark"
                    tone="gold"
                    size={46}
                    iconSize={21}
                  />
                  <Text style={styles.cardTitle}>No local reports yet</Text>
                  <Text style={styles.helper}>
                    Use any profile/chat safety menu → Report to create a live
                    moderation item.
                  </Text>
                </View>
              )}
          ...
          {tab === "playbooks" && (
            <View style={ventureStyles.section}>
              <Text style={styles.sectionLabel}>AUTOMATION GUARDS</Text>
              {automationGuards.map(([title, body], index) => (
                <ChecklistRow
                  key={title}
                  title={title}
                  body={body}
              ...
              <View style={coachStyles.boundaryCard}>
                ...
                />
                <View style={{ flex: 1 }}>
                  <Text style={styles.cardTitle}>Human-first safety</Text>
                  <Text style={styles.helper}>
                    AI can prioritize and freeze risky surfaces, but permanent
                    bans, sensitive identity decisions and billing-impact
                    actions need human review.
                  </Text>
                </View>
              </View>
            </View>
          ...
          {tab === "audit" && (
            <View style={ventureStyles.section}>
              <BackendLaunchGateCard snapshot={backendLaunchSnapshot} />
              <PaymentEntitlementGateCard
                snapshot={paymentEntitlementSnapshot}
              />
              <NotificationReadinessCard snapshot={notificationSnapshot} />
              <GiftFulfillmentReadinessCard
                snapshot={giftFulfillmentSnapshot}
              />
              <PlacesReservationReadinessCard
                snapshot={placesReservationSnapshot}
              />
              <ObservabilityReadinessCard snapshot={observabilitySnapshot} />
              <AbuseFraudReadinessCard snapshot={abuseFraudSnapshot} />
              <TrustOpsSlaCard snapshot={trustOpsSnapshot} />
              <LegalStoreOpsCard snapshot={legalOpsSnapshot} />
              <P1OperationsCard snapshot={p1Snapshot} />
              <ProductQualityCard snapshot={qualitySnapshot} />
              <InteractionQualityCard snapshot={interactionSnapshot} />
              <PolicyComplianceCard snapshot={policyComplianceSnapshot} />
              <StoreReviewCard snapshot={storeReviewSnapshot} />
              <ReleaseReadinessCard snapshot={releaseSnapshot} />
              <Text style={styles.sectionLabel}>AUDIT READINESS</Text>
              {(
                [
                  [
                    "Reviewer notes",
                    "Every freeze, escalation and resolution needs reviewer ID + note.",
                  ],
                  [
                    "Evidence packet",
                    "Reports, chat IDs, gift/payment events, profile edits and block graph stay linked.",
                  ],
                  [
                    "Member notification",
                    "Warnings and support outcomes are sent without exposing reporter identity.",
                  ],
                  [
                    "Appeals",
                    "Support ticket can reopen a resolved moderation case.",
                  ],
                  [
                    "Data deletion",
                    "Deletion workflow respects safety-retention holds when legally required.",
                  ],
                ] as const
...
function AdminOpsStat({ value, label }: { value: string; label: string }) {
  return (
    <View style={adminOpsStyles.stat}>
      <Text style={adminOpsStyles.statValue}>{value}</Text>
      <Text style={adminOpsStyles.statLabel}>{label}</Text>
    </View>
  );
}

function backendLaunchGateIcon(
  id: BackendLaunchGate["id"],
...
function BackendLaunchGateCard({
  snapshot,
}: {
  snapshot: BackendLaunchSnapshot;
}) {
  const ready = snapshot.status === "Ready for production backend";
  return (
    <View style={adminOpsStyles.backendLaunchCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={ready ? "cloud-done" : "cloud-upload-outline"}
          tone={ready ? "gold" : "rose"}
          size={54}
          iconSize={25}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          <Text style={styles.kicker}>BACKEND / SUPABASE GATE</Text>
          <Text style={adminOpsStyles.qualityTitle}>
            {snapshot.status} · {snapshot.score}%
          </Text>
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} backend gates ready. Schema
            can be ready while OTP/SMS, server secrets and monitoring remain
            final launch work.
          </Text>
        </View>
      </View>
      ...
      <View style={adminOpsStyles.areaGrid}>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Schema</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.schemaCoverage}%
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Realtime</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.realtimeModules}
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Providers</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.providerModules}
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Blockers</Text>
          <Text style={adminOpsStyles.areaScore}>
      ...
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.gates.map((gate) => (
          <BackendLaunchGateRow key={gate.id} gate={gate} />
        ))}
      </View>
    </View>
  );
...
function BackendLaunchGateRow({ gate }: { gate: BackendLaunchGate }) {
  const tone: PremiumIconTone = gate.ready
    ? "gold"
    : gate.started
      ? "rose"
      : "ruby";
  return (
    <View style={adminOpsStyles.qualityRow}>
      <MiniPremiumIcon
        name={gate.ready ? "checkmark-circle" : backendLaunchGateIcon(gate.id)}
        tone={tone}
        size={28}
        iconSize={13}
      />
      <View style={{ flex: 1 }}>
        ...
        {!gate.ready && (
          <Text style={adminOpsStyles.nextTiny}>Next: {gate.nextStep}</Text>
        )}
      </View>
    </View>
  );
...
function paymentEntitlementGateIcon(
  id: PaymentEntitlementGate["id"],
): keyof typeof Ionicons.glyphMap {
  const icons: Record<
    PaymentEntitlementGate["id"],
    keyof typeof Ionicons.glyphMap
  > = {
...
function PaymentEntitlementGateCard({
  snapshot,
}: {
  snapshot: PaymentEntitlementSnapshot;
}) {
  ...
  return (
    <View style={adminOpsStyles.paymentEntitlementCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={ready ? "card" : "card-outline"}
          tone={ready ? "gold" : "ruby"}
          size={54}
          iconSize={25}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} billing gates ready. Paid UI
            can be preview-ready while App Store products, receipts and webhooks
            remain final launch work.
          </Text>
        </View>
      </View>
      <View style={adminOpsStyles.qualityTrack}>
      ...
      <View style={adminOpsStyles.areaGrid}>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Products</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.paidProductCount}
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Ready</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.readyCount}/{snapshot.total}
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Blockers</Text>
          <Text style={adminOpsStyles.areaScore}>{snapshot.blockerCount}</Text>
        </View>
      </View>
      <View style={adminOpsStyles.nextOpsCard}>
        <MiniPremiumIcon
      ...
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.gates.map((gate) => (
          <PaymentEntitlementGateRow key={gate.id} gate={gate} />
        ))}
      </View>
    </View>
  );
...
function PaymentEntitlementGateRow({ gate }: { gate: PaymentEntitlementGate }) {
  ...
  return (
    <View style={adminOpsStyles.qualityRow}>
      <MiniPremiumIcon
        name={
          gate.ready ? "checkmark-circle" : paymentEntitlementGateIcon(gate.id)
        }
        tone={tone}
        size={28}
        iconSize={13}
...
function NotificationReadinessCard({
  snapshot,
}: {
  ...
  return (
    <View style={adminOpsStyles.notificationCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={ready ? "notifications" : "notifications-outline"}
          tone={ready ? "gold" : "rose"}
          size={54}
          iconSize={25}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} notification gates ready.
            Push provider and physical-device QA stay final launch work until
      ...
      <View style={adminOpsStyles.areaGrid}>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Events</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.eventCoverage}%
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Ready</Text>
      ...
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.gates.map((gate) => (
          <NotificationGateRow key={gate.id} gate={gate} />
        ))}
      </View>
    </View>
  );
...
function NotificationGateRow({ gate }: { gate: NotificationGate }) {
  ...
  return (
    <View style={adminOpsStyles.qualityRow}>
      <MiniPremiumIcon
        name={gate.ready ? "checkmark-circle" : notificationGateIcon(gate.id)}
        tone={tone}
        size={28}
        iconSize={13}
      />
...
function giftFulfillmentGateIcon(
  id: GiftFulfillmentGate["id"],
): keyof typeof Ionicons.glyphMap {
  const icons: Record<
    GiftFulfillmentGate["id"],
    keyof typeof Ionicons.glyphMap
  > = {
...
function GiftFulfillmentReadinessCard({
  snapshot,
}: {
  snapshot: GiftFulfillmentReadinessSnapshot;
}) {
  const ready = snapshot.status === "Ready for live gift orders";
  return (
    <View style={adminOpsStyles.giftFulfillmentCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={ready ? "gift" : "gift-outline"}
          tone={ready ? "gold" : "rose"}
          size={54}
          iconSize={25}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} gift operations ready.
            Catalog and private-recipient preview can be ready while delivery
            partners, webhooks and live QA remain final launch work.
          </Text>
        </View>
      </View>
      <View style={adminOpsStyles.qualityTrack}>
      ...
      <View style={adminOpsStyles.areaGrid}>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Catalog</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.catalogItemCount}
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Coverage</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.providerCoverage}%
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Blockers</Text>
      ...
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.gates.map((gate) => (
          <GiftFulfillmentGateRow key={gate.id} gate={gate} />
        ))}
      </View>
    </View>
  );
...
function GiftFulfillmentGateRow({ gate }: { gate: GiftFulfillmentGate }) {
  ...
  return (
    <View style={adminOpsStyles.qualityRow}>
      <MiniPremiumIcon
        name={
          gate.ready ? "checkmark-circle" : giftFulfillmentGateIcon(gate.id)
        }
        tone={tone}
        size={28}
        iconSize={13}
...
function placesReservationGateIcon(
  id: PlacesReservationGate["id"],
): keyof typeof Ionicons.glyphMap {
  const icons: Record<
    PlacesReservationGate["id"],
    keyof typeof Ionicons.glyphMap
  > = {
...
function PlacesReservationReadinessCard({
  snapshot,
}: {
  snapshot: PlacesReservationReadinessSnapshot;
}) {
  const ready = snapshot.status === "Ready for live reservations";
  return (
    <View style={adminOpsStyles.placesReservationCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={ready ? "calendar" : "calendar-outline"}
          tone={ready ? "gold" : "rose"}
          size={54}
          iconSize={25}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} venue gates ready. Curated
            places can be preview-ready while live hours, reservations, refunds
            and provider QA remain final launch work.
          </Text>
        </View>
      </View>
      <View style={adminOpsStyles.qualityTrack}>
      ...
      <View style={adminOpsStyles.areaGrid}>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Venues</Text>
          <Text style={adminOpsStyles.areaScore}>{snapshot.venueCount}</Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Cities</Text>
          <Text style={adminOpsStyles.areaScore}>{snapshot.cityCount}</Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Partners</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.partnerCoverage}%
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Blockers</Text>
      ...
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.gates.map((gate) => (
          <PlacesReservationGateRow key={gate.id} gate={gate} />
        ))}
      </View>
    </View>
  );
...
function PlacesReservationGateRow({ gate }: { gate: PlacesReservationGate }) {
  ...
  return (
    <View style={adminOpsStyles.qualityRow}>
      <MiniPremiumIcon
        name={
          gate.ready ? "checkmark-circle" : placesReservationGateIcon(gate.id)
        }
        tone={tone}
        size={28}
        iconSize={13}
...
function observabilityGateIcon(
  id: ObservabilityGate["id"],
): keyof typeof Ionicons.glyphMap {
...
function ObservabilityReadinessCard({
  snapshot,
}: {
  snapshot: ObservabilityReadinessSnapshot;
}) {
  const ready = snapshot.status === "Ready for monitored launch";
  return (
    <View style={adminOpsStyles.observabilityCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={ready ? "pulse" : "pulse-outline"}
          tone={ready ? "gold" : "plum"}
          size={54}
          iconSize={25}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} monitoring gates ready.
            Analytics and crash providers stay final launch work, but event
            payloads must remain privacy-safe now.
          </Text>
        </View>
      </View>
      <View style={adminOpsStyles.qualityTrack}>
      ...
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.gates.map((gate) => (
          <ObservabilityGateRow key={gate.id} gate={gate} />
        ))}
      </View>
    </View>
  );
...
function ObservabilityGateRow({ gate }: { gate: ObservabilityGate }) {
  ...
  return (
    <View style={adminOpsStyles.qualityRow}>
      <MiniPremiumIcon
        name={gate.ready ? "checkmark-circle" : observabilityGateIcon(gate.id)}
        tone={tone}
        size={28}
        iconSize={13}
      />
...
function abuseFraudGateIcon(
  id: AbuseFraudGate["id"],
): keyof typeof Ionicons.glyphMap {
...
function AbuseFraudReadinessCard({
  snapshot,
}: {
  snapshot: AbuseFraudReadinessSnapshot;
}) {
  const ready = snapshot.status === "Ready for safe scale";
  return (
    <View style={adminOpsStyles.abuseFraudCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={ready ? "shield-checkmark" : "shield-half-outline"}
          tone={ready ? "gold" : "ruby"}
          size={54}
          iconSize={25}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} protection gates ready. Core
            anti-scam rules are app-ready; device risk, CAPTCHA and real-device
            drills remain final scale work.
          </Text>
        </View>
      </View>
      <View style={adminOpsStyles.qualityTrack}>
      ...
      <View style={adminOpsStyles.areaGrid}>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Core</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.coreProtectionScore}%
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Provider</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.providerProtectionScore}%
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Blockers</Text>
      ...
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.gates.map((gate) => (
          <AbuseFraudGateRow key={gate.id} gate={gate} />
        ))}
      </View>
    </View>
  );
...
function AbuseFraudGateRow({ gate }: { gate: AbuseFraudGate }) {
  ...
  return (
    <View style={adminOpsStyles.qualityRow}>
      <MiniPremiumIcon
        name={gate.ready ? "checkmark-circle" : abuseFraudGateIcon(gate.id)}
        tone={tone}
        size={28}
        iconSize={13}
      />
...
function TrustOpsSlaCard({ snapshot }: { snapshot: TrustOpsSnapshot }) {
  ...
  return (
    <View style={adminOpsStyles.trustOpsCard}>
      <View style={shared.row}>
        <PremiumIcon
        ...
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} safety operations gates
            ready. Backend tools can connect later, but the launch operating
      ...
      <View style={adminOpsStyles.areaGrid}>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Reviewers</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.requiredReviewers}
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Fastest SLA</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.fastestSlaHours}h
          </Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>High risk</Text>
          <Text style={adminOpsStyles.areaScore}>{snapshot.highRiskCases}</Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Human review</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.humanReviewCases}
          </Text>
        </View>
      </View>
      <View style={adminOpsStyles.nextOpsCard}>
      ...
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.gates.map((gate) => (
          <TrustOpsGateRow key={gate.id} gate={gate} />
        ))}
      </View>
    </View>
  );
...
function TrustOpsGateRow({ gate }: { gate: TrustOpsGate }) {
  ...
  return (
    <View style={adminOpsStyles.qualityRow}>
      <MiniPremiumIcon
        name={gate.ready ? "checkmark-circle" : trustOpsGateIcon(gate.id)}
        tone={tone}
        size={28}
        iconSize={13}
      />
...
function legalStoreOpsGateIcon(
  id: LegalStoreOpsGate["id"],
): keyof typeof Ionicons.glyphMap {
...
function LegalStoreOpsCard({ snapshot }: { snapshot: LegalStoreOpsSnapshot }) {
  ...
  return (
    <View style={adminOpsStyles.legalOpsCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={ready ? "ribbon" : "document-lock-outline"}
          tone={ready ? "gold" : "rose"}
          size={54}
          iconSize={25}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} store/legal gates ready. This
            keeps Play Store/App Store submission honest before production
      ...
      <View style={adminOpsStyles.areaGrid}>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Ready</Text>
          <Text style={adminOpsStyles.areaScore}>{snapshot.readyCount}</Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Blockers</Text>
          <Text style={adminOpsStyles.areaScore}>
        ...
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Public URLs</Text>
          <Text style={adminOpsStyles.areaScore}>
            {snapshot.gates.find((gate) => gate.id === "public_urls")?.ready
              ? "Ready"
              : "No"}
          </Text>
        </View>
      ...
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.gates.map((gate) => (
          <LegalStoreOpsGateRow key={gate.id} gate={gate} />
        ))}
      </View>
    </View>
  );
...
function LegalStoreOpsGateRow({ gate }: { gate: LegalStoreOpsGate }) {
  ...
  return (
    <View style={adminOpsStyles.qualityRow}>
      <MiniPremiumIcon
        name={gate.ready ? "checkmark-circle" : legalStoreOpsGateIcon(gate.id)}
        tone={tone}
        size={28}
        iconSize={13}
      />
...
function P1OperationsCard({ snapshot }: { snapshot: P1OperationsSnapshot }) {
  return (
    <View style={adminOpsStyles.releaseCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={snapshot.status === "P1 ready" ? "rocket" : "layers-outline"}
          tone={snapshot.status === "P1 ready" ? "gold" : "rose"}
          size={52}
          iconSize={24}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.helper}>
            {snapshot.readyCount} ready · {snapshot.startedCount} started ·{" "}
            {snapshot.blockedCount} blocked. This is the bridge from polished
            MVP to real city operations.
          </Text>
        </View>
      </View>
      <View style={adminOpsStyles.qualityTrack}>
      ...
      <View style={adminOpsStyles.areaGrid}>
        ...
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Started</Text>
          <Text style={adminOpsStyles.areaScore}>{snapshot.startedCount}</Text>
        </View>
        <View style={adminOpsStyles.areaPill}>
          <Text style={adminOpsStyles.areaLabel}>Blocked</Text>
          <Text style={adminOpsStyles.areaScore}>{snapshot.blockedCount}</Text>
        </View>
      </View>
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.items.map((item) => (
          <P1OperationRow key={item.id} item={item} />
        ))}
      </View>
    </View>
  );
...
function P1OperationRow({ item }: { item: P1OperationItem }) {
  ...
  return (
    <View style={adminOpsStyles.qualityRow}>
      ...
      <View style={{ flex: 1 }}>
        <View style={shared.row}>
          <Text style={[adminOpsStyles.qualityRowTitle, { flex: 1 }]}>
            {item.title}
          </Text>
          {item.storeCritical && (
            <View style={adminOpsStyles.storeCriticalPill}>
              <Text style={adminOpsStyles.storeCriticalText}>Store</Text>
            </View>
          )}
        </View>
        <Text style={adminOpsStyles.qualityRowBody}>{item.body}</Text>
        {item.status !== "ready" && (
          <Text style={adminOpsStyles.nextTiny}>Next: {item.nextStep}</Text>
        )}
      </View>
    </View>
  );
...
function ProductQualityCard({
  ...
  snapshot: ReturnType<typeof buildProductQualitySnapshot>;
}) {
  const status = snapshot.blockers.length
    ? "Needs fixes"
    : snapshot.important.length
      ? "Almost ready"
      : "Ready";
  return (
    <View style={adminOpsStyles.qualityCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={snapshot.blockers.length ? "warning" : "checkmark-circle"}
          tone={snapshot.blockers.length ? "ruby" : "gold"}
          size={52}
          iconSize={24}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={adminOpsStyles.qualityTitle}>
            {status} · {snapshot.score}%
          </Text>
          <Text style={styles.helper}>
            {snapshot.readyItems}/{snapshot.totalItems} readiness checks
            complete. Backend remains last by design.
          </Text>
        </View>
      ...
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.items.map((item) => (
          <ProductQualityRow key={item.id} item={item} />
        ))}
      </View>
    </View>
  );
...
function ProductQualityRow({ item }: { item: ProductQualityItem }) {
  const tone: PremiumIconTone = item.ready
    ? "gold"
...
function InteractionQualityCard({
  snapshot,
}: {
  snapshot: InteractionAuditSnapshot;
}) {
  const criticalOk = snapshot.criticalMissing.length === 0;
  const topAreas = snapshot.areaSummary
    .filter((area) => area.total >= 3)
    .slice(0, 6);
  return (
    <View style={adminOpsStyles.interactionCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={criticalOk ? "flash" : "warning"}
          tone={criticalOk ? "gold" : "ruby"}
          size={52}
          iconSize={24}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={adminOpsStyles.qualityTitle}>
            {snapshot.score}% button-flow coverage
          </Text>
          <Text style={styles.helper}>
            {snapshot.implemented}/{snapshot.total} important interactions
            mapped · {snapshot.criticalTotal} critical actions protected.
          </Text>
        </View>
      </View>
      <View style={adminOpsStyles.areaGrid}>
        {topAreas.map((area) => (
          <View key={area.area} style={adminOpsStyles.areaPill}>
            <Text style={adminOpsStyles.areaLabel}>
              {area.area.replace("_", " ")}
            </Text>
            <Text style={adminOpsStyles.areaScore}>
              {area.implemented}/{area.total}
            </Text>
          </View>
        ))}
      </View>
      <View style={adminOpsStyles.interactionNotice}>
        <MiniPremiumIcon
          name={criticalOk ? "checkmark-circle" : "alert-circle-outline"}
          tone={criticalOk ? "gold" : "ruby"}
          size={30}
          iconSize={14}
        />
        <Text style={adminOpsStyles.interactionNoticeText}>
          {criticalOk
            ? "No critical interaction gaps detected. Chat tools, gifts, pricing, safety, support and dates have mapped outcomes."
            : `${snapshot.criticalMissing.length} critical interaction(s) need attention before release.`}
        </Text>
      </View>
    </View>
  );
...
function PolicyComplianceCard({
  ...
  return (
    <View style={adminOpsStyles.releaseCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={snapshot.ready ? "shield-checkmark" : "shield-outline"}
          tone={snapshot.ready ? "gold" : "rose"}
          size={52}
          iconSize={24}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={adminOpsStyles.qualityTitle}>
            {snapshot.ready ? "Policy-ready" : "Policy blockers"} ·{" "}
            {snapshot.score}%
          </Text>
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} store-policy controls ready
            for dating, chat, subscriptions, safety and real-world meetups.
          </Text>
        </View>
      </View>
      <View style={adminOpsStyles.qualityTrack}>
      ...
      <View style={adminOpsStyles.areaGrid}>
        {snapshot.items.map((item) => (
          <View key={item.id} style={adminOpsStyles.areaPill}>
            <Text style={adminOpsStyles.areaLabel}>
              {item.area.replace("_", " ")}
            </Text>
            <Text style={adminOpsStyles.areaScore}>
      ...
      <View style={adminOpsStyles.qualityRows}>
        {snapshot.items.map((item) => (
          <View key={item.id} style={adminOpsStyles.qualityRow}>
            <MiniPremiumIcon
              name={item.ready ? "checkmark-circle" : "alert-circle-outline"}
              tone={item.ready ? "gold" : "ruby"}
              size={28}
              iconSize={13}
            />
            <View style={{ flex: 1 }}>
...
function StoreReviewCard({ snapshot }: { snapshot: StoreReviewSnapshot }) {
  return (
    <View style={adminOpsStyles.releaseCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={snapshot.ready ? "storefront" : "clipboard"}
          tone={snapshot.ready ? "gold" : "rose"}
          size={52}
          iconSize={24}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={adminOpsStyles.qualityTitle}>
            {snapshot.ready ? "Reviewer-ready" : "Reviewer blockers"} ·{" "}
            {snapshot.score}%
          </Text>
          <Text style={styles.helper}>
            {snapshot.readyCount}/{snapshot.total} review checks complete. This
            is the handoff reviewers need to enter and test the app.
          </Text>
        </View>
      </View>
      <View style={adminOpsStyles.qualityTrack}>
      ...
      <View style={adminOpsStyles.releaseList}>
        <Text style={adminOpsStyles.releaseListTitle}>
          Reviewer instructions
        </Text>
        {snapshot.reviewerInstructions.map((instruction, index) => (
          <View key={instruction} style={adminOpsStyles.releaseGateRow}>
            <MiniPremiumIcon
              name={index < 2 ? "key-outline" : "navigate-circle-outline"}
              tone={index < 2 ? "gold" : "rose"}
              size={28}
              iconSize={13}
            />
            <Text style={[adminOpsStyles.qualityRowBody, { flex: 1 }]}>
              {instruction}
            </Text>
          </View>
        ))}
...
function ReleaseReadinessCard({
  ...
  return (
    <View style={adminOpsStyles.releaseCard}>
      <View style={shared.row}>
        <PremiumIcon
          name={snapshot.storeReady ? "rocket" : "cloud-upload-outline"}
          tone={snapshot.storeReady ? "gold" : "rose"}
          size={52}
          iconSize={24}
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={adminOpsStyles.qualityTitle}>
            {snapshot.storeReady ? "Store ready" : "Final connections pending"}
          </Text>
          <Text style={styles.helper}>
            Preview {snapshot.previewScore}% · Store-critical{" "}
            {snapshot.storeScore}% · {snapshot.storeBlockers.length} store
            blocker(s).
          </Text>
        </View>
      </View>
      ...
      {finalItems.length > 0 && (
        <View style={adminOpsStyles.releaseList}>
          <Text style={adminOpsStyles.releaseListTitle}>
            Final connection items
          </Text>
          {finalItems.map((item) => (
            <ReleaseGateRow key={item.id} gate={item} />
          ))}
        </View>
      )}
...
function ReleaseMeter({ label, value }: { label: string; value: number }) {
  return (
    <View style={adminOpsStyles.releaseMeter}>
      <View style={shared.row}>
        <Text style={adminOpsStyles.releaseMeterLabel}>{label}</Text>
        <View style={shared.spacer} />
        <Text style={adminOpsStyles.releaseMeterValue}>{value}%</Text>
      </View>
      <View style={adminOpsStyles.qualityTrack}>
        <View style={[adminOpsStyles.qualityFill, { width: `${value}%` }]} />
      </View>
    </View>
  );
}
  ...
  const icon =
    gate.status === "ready"
      ...
      : gate.status === "blocked"
        ...
        : "construct-outline";
  return (
    <View style={adminOpsStyles.releaseGateRow}>
      <MiniPremiumIcon name={icon} tone={tone} size={28} iconSize={13} />
      <View style={{ flex: 1 }}>
        <Text style={adminOpsStyles.qualityRowTitle}>{gate.title}</Text>
        <Text style={adminOpsStyles.qualityRowBody}>{gate.body}</Text>
...
function ModerationCaseCard({
  ...
  const riskStyle =
    item.risk === "Critical"
      ? adminOpsStyles.riskCritical
      : item.risk === "High"
        ? adminOpsStyles.riskHigh
        : item.risk === "Medium"
          ? adminOpsStyles.riskMedium
          : adminOpsStyles.riskLow;
  return (
    <View style={adminOpsStyles.caseCard}>
      <View style={shared.row}>
        <View style={[adminOpsStyles.riskDot, riskStyle]} />
        <View style={{ flex: 1 }}>
          <Text style={styles.cardTitle}>{item.member}</Text>
          <Text style={adminOpsStyles.caseMeta}>
            {item.category.replace("_", " ")} · SLA {item.slaHours}h · score{" "}
            {item.riskScore}
          </Text>
        </View>
        <View style={[adminOpsStyles.riskPill, riskStyle]}>
          <Text style={adminOpsStyles.riskText}>{item.risk}</Text>
        </View>
      </View>
      <Text style={styles.helper}>{item.reason}</Text>
      <View style={adminOpsStyles.evidenceWrap}>
        {item.evidence.map((evidence) => (
          <View key={evidence} style={adminOpsStyles.evidencePill}>
            <Text style={adminOpsStyles.evidenceText}>{evidence}</Text>
          </View>
        ))}
      </View>
      <View style={ventureStyles.nextStep}>
        <MiniPremiumIcon name="construct" tone="rose" size={30} iconSize={14} />
        <Text style={ventureStyles.nextText}>{item.action}</Text>
      </View>
      <View style={adminOpsStyles.caseFooter}>
        <ModerationStatusPill status={item.status} />
        {item.humanReviewRequired && (
          <View style={adminOpsStyles.reviewPill}>
            <Text style={adminOpsStyles.reviewText}>Human review</Text>
          </View>
        )}
        {item.canAutoHide && (
          <View style={adminOpsStyles.autoPill}>
            <Text style={adminOpsStyles.autoText}>Auto-hide eligible</Text>
          </View>
        )}
      </View>
      <View style={adminOpsStyles.actionRow}>
        <Pressable onPress={onEvidence} style={adminOpsStyles.ghostAction}>
          <Text style={adminOpsStyles.ghostActionText}>Evidence</Text>
        </Pressable>
        <Pressable onPress={onFreeze} style={adminOpsStyles.ghostAction}>
          <Text style={adminOpsStyles.ghostActionText}>Freeze</Text>
        </Pressable>
        <Pressable onPress={onEscalate} style={adminOpsStyles.primaryAction}>
          <Text style={adminOpsStyles.primaryActionText}>Escalate</Text>
        </Pressable>
        <Pressable onPress={onResolve} style={adminOpsStyles.ghostAction}>
          <Text style={adminOpsStyles.ghostActionText}>Resolve</Text>
        </Pressable>
      </View>
    </View>
  );
      ...
      : status === "triage"
        ...
        : status === "frozen"
          ...
          : status === "escalated"
            ...
            : "Resolved";
  return (
    <View style={adminOpsStyles.statusPill}>
      <Text style={adminOpsStyles.statusPillText}>{label}</Text>
    </View>
  );
}
...
const dateVenues = [
  {
    ...
    price: "$$",
    vibe: "Quiet tables · Great conversation",
    icon: "☕",
  },
  {
    id: "cafe-2",
    name: "The Garden Coffee Room",
    category: "Café",
    area: "A lively public neighborhood",
    price: "$$",
    vibe: "Bright · Relaxed · Weekend-friendly",
    icon: "🌿",
  },
  {
    id: "walk-1",
    ...
    category: "Walk",
    area: "Popular waterfront area",
    price: "Free",
    vibe: "Scenic · Public · Easygoing",
    icon: "🌅",
  },
  {
    id: "walk-2",
    ...
    category: "Walk",
    area: "Central garden district",
    price: "$",
    vibe: "Calm · Beautiful · Daytime",
    icon: "🌷",
  },
  {
    id: "dinner-1",
    name: "Candlelight Kitchen",
    category: "Dinner",
    area: "Restaurant district",
    price: "$$$",
    vibe: "Warm · Vegetarian-friendly options",
    icon: "🍽️",
  },
  {
    id: "dinner-2",
    name: "Spice & Stories",
    category: "Dinner",
    area: "Busy public square",
    price: "$$",
    vibe: "Indian-inspired · Conversation-friendly",
    icon: "✨",
  },
  {
    id: "activity-1",
    ...
    area: "Arts district",
    price: "$$",
    vibe: "Creative · Low pressure · Memorable",
    icon: "🎨",
  },
  {
    id: "activity-2",
    name: "Mini Golf Social",
    category: "Activity",
    area: "Entertainment district",
    price: "$$",
    vibe: "Playful · Public · Easy icebreaker",
    icon: "⛳",
  },
];
const dateTimes = [
...
function DatePlanner({
  ...
  const reserveDate = async () => {
    ...
    try {
      const intent = await createDateReservationIntent({
        venueId: selectedVenue.id,
        venueName: selectedVenue.name,
      ...
      if (Platform.OS !== "ios" || !applePaySupported || !intent.clientSecret)
        throw new Error("Apple Pay is not available on this device.");
      await confirmApplePayReservation(
        intent.clientSecret,
        selectedVenue.name,
        "10.00",
      );
  ...
  return (
    <LinearGradient
      colors={["#2D0727", colors.black, colors.black]}
      style={{ flex: 1 }}
    >
      <SafeAreaView style={shared.safe}>
...
function ReservationCheckout({
  ...
  const useApplePay =
    Platform.OS === "ios" && paymentsConfigured && applePaySupported;
    ...
    <View style={launchStyles.checkoutCard}>
      <View style={shared.row}>
        ...
        <View style={{ flex: 1, marginLeft: 11 }}>
          ...
          <Text style={styles.helper}>
            {dateReservationStatusCopy(status, quote)}
          </Text>
        </View>
        {status === "reserved" && (
          <MiniPremiumIcon
      ...
      <View style={dateStyles.reservationPolicy}>
        <GiftQuoteInfoRow icon="shield-checkmark" text={quote.safetyPolicy} />
        <GiftQuoteInfoRow icon="refresh-circle" text={quote.refundPolicy} />
        <GiftQuoteInfoRow
          icon="card"
      ...
      {status === "reserved" ? (
        <View style={launchStyles.reservedPill}>
          ...
          <Text style={launchStyles.reservedText}>
            Reservation hold confirmed · production Apple Pay activates after
            Stripe and Apple Merchant credentials.
          </Text>
        </View>
      ) : useApplePay ? (
        <ApplePayReservationButton onPress={onReserve} />
      ) : (
        <Button
          label={
            status === "processing"
              ...
              : paymentsConfigured
                ? `Reserve securely · ${formatPaymentMoney(quote.amountCents, quote.currency)}`
                : `Try reservation demo · ${formatPaymentMoney(quote.amountCents, quote.currency)}`
          }
...
function MatchCard({
  ...
  const roseOpacity = pan.y.interpolate({
    inputRange: [-150, -35],
    outputRange: [1, 0],
    extrapolate: "clamp",
  });
  ...
  const alignmentLabel =
    match.familyPriority === "high" ? "Family-first" : "Balanced future";
  const reset = () =>
    Animated.spring(pan, {
      toValue: { x: 0, y: 0 },
      friction: 6,
  ...
  const fly = (toValue: { x: number; y: number }, done: () => void) =>
    Animated.timing(pan, {
      toValue,
      duration: 190,
      easing: Easing.out(Easing.cubic),
  ...
  const panResponder = useRef(
    PanResponder.create({
      onMoveShouldSetPanResponder: (_, gesture) =>
        Math.abs(gesture.dx) > 8 || Math.abs(gesture.dy) > 10,
      onPanResponderMove: (_, gesture) =>
        pan.setValue({ x: gesture.dx, y: gesture.dy }),
      onPanResponderRelease: (_, gesture) => {
        if (gesture.dx > 105) {
          fly({ x: 430, y: gesture.dy }, onInterested);
          return;
        }
        if (gesture.dx < -105) {
          fly({ x: -430, y: gesture.dy }, onSkip);
          return;
        }
        if (gesture.dy < -120) {
          fly({ x: 0, y: -520 }, onRose);
          return;
        }
        reset();
      },
    ...
    >
      <Pressable onPress={onPress} style={{ width: "100%", height: "100%" }}>
        ...
        <Animated.View
          pointerEvents="none"
          style={[
            swipeStyles.swipeOverlay,
            swipeStyles.swipeYes,
            { opacity: yesOpacity },
          ]}
        >
          <Text style={swipeStyles.swipeLabel}>SERIOUS YES</Text>
        </Animated.View>
        <Animated.View
          pointerEvents="none"
          style={[
            swipeStyles.swipeOverlay,
            swipeStyles.swipeNope,
            { opacity: nopeOpacity },
          ]}
        >
          <Text style={swipeStyles.swipeLabel}>NOT FOR ME</Text>
        </Animated.View>
        <Animated.View
          pointerEvents="none"
          style={[swipeStyles.swipeRose, { opacity: roseOpacity }]}
        >
          <PremiumIcon name="sparkles" tone="gold" size={46} iconSize={21} />
          <Text style={swipeStyles.swipeRoseText}>SEND SPARK</Text>
        </Animated.View>
        <View style={styles.matchTop}>
          <View style={swipeStyles.premiumRibbon}>
            <Chip label={match.match} gold />
            <View style={swipeStyles.matchSwipeHint}>
              <MiniPremiumIcon
                ...
                iconSize={11}
              />
              <Text style={swipeStyles.matchSwipeHintText}>Swipe</Text>
            </View>
          </View>
        </View>
        <View style={styles.matchInfo}>
          ...
          <View style={swipeStyles.profileSummary}>
            <View style={swipeStyles.summaryItem}>
              <Text style={swipeStyles.summaryLabel}>Intent</Text>
              <Text style={swipeStyles.summaryValue}>{match.intent}</Text>
            </View>
            <View style={swipeStyles.summaryDivider} />
            <View style={swipeStyles.summaryItem}>
              <Text style={swipeStyles.summaryLabel}>Values</Text>
              <Text style={swipeStyles.summaryValue}>{alignmentLabel}</Text>
            </View>
            <View style={swipeStyles.summaryDivider} />
            <View style={swipeStyles.summaryItem}>
              <Text style={swipeStyles.summaryLabel}>Trust</Text>
              <Text style={swipeStyles.summaryValue}>
                {match.vouches.count} vouches
              </Text>
            </View>
          </View>
          {visibleReasons.length > 0 && (
            <View style={swipeStyles.reasonCard}>
              <MiniPremiumIcon
                name="sparkles"
                tone="gold"
                size={28}
              ...
              />
              <View style={{ flex: 1 }}>
                <Text style={swipeStyles.reasonTitle}>
                  Why this feels aligned
                </Text>
                <Text style={swipeStyles.reasonBody}>
                  {visibleReasons.join(" · ")}
                </Text>
              </View>
            </View>
          ...
          <View style={styles.chipRow}>
            ...
            {hiddenVibes > 0 && (
              <View style={swipeStyles.morePill}>
                <Text style={swipeStyles.morePillText}>+{hiddenVibes}</Text>
              </View>
            )}
          </View>
          <View style={swipeStyles.actionHint}>
            <Text style={swipeStyles.actionHintText}>
              Swipe left/right · Tap for full profile · Swipe up Spark
            </Text>
          </View>
          <View style={styles.cardActions}>
            <Pressable onPress={onSkip} style={styles.nope}>
              <PremiumIcon name="close" tone="dark" size={52} iconSize={24} />
            </Pressable>
            <Pressable onPress={onRose} style={aiStyles.roseAction}>
              <PremiumIcon
                name="sparkles"
                tone="gold"
                size={30}
                iconSize={14}
              />
              <Text style={aiStyles.roseActionText}>Spark</Text>
            </Pressable>
            <Pressable onPress={onInterested} style={styles.yes}>
              <MiniPremiumIcon
                name="heart"
                tone="ruby"
                size={38}
    ...
    <View style={{ flex: 1 }}>
      <ScrollView contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        <View style={styles.detailBody}>
          ...
          <Text style={styles.sectionLabel}>HER VIBE</Text>
          <View style={styles.chipRow}>
            {match.vibes.map((x) => (
...
function Mutual({
  ...
  return (
    <LinearGradient
      colors={["#2E0710", colors.black, colors.black]}
      style={styles.center}
    >
      <SafeAreaView
        style={[
      ...
      <View style={{ gap: 12 }}>
        {[
          ...
          <Pressable
            ...
            style={[
              styles.answer,
              answer === x && styles.intentSelected,
              loading && { opacity: 0.72 },
            ]}
          >
            <Text style={styles.answerText}>{x}</Text>
            <MiniPremiumIcon
              name={answer === x ? "checkmark-circle" : "ellipse-outline"}
              tone={answer === x ? "gold" : "dark"}
              size={34}
              iconSize={16}
            />
          </Pressable>
...
const chatGifs = Array.from({ length: 100 }, (_, index) => ({
  title: gifMoodTitles[index % gifMoodTitles.length]!,
  uri: chatGifUrls[index % chatGifUrls.length]!,
}));
const digitalGifts = [
...
const physicalGifts = [
  ...
  },
  {
    ...
    emoji: "🍨",
    priceCents: 2600,
    caption: "Pick 3 premium flavors",
    eta: "45–75 min",
    photo:
      "https://images.unsplash.com/photo-1501443762994-82bd5dace89a?auto=format&fit=crop&w=1600&q=90",
  },
  {
    id: "chai-duo",
    name: "Chai & Coffee Duo",
    emoji: "☕",
    priceCents: 2200,
    caption: "A cozy little break",
    eta: "45–75 min",
    photo:
  ...
  },
  {
    ...
    emoji: "🍫",
    priceCents: 3600,
    caption: "Handcrafted box of 12",
    eta: "Same day",
    photo:
      "https://images.unsplash.com/photo-1481391319762-47dff72954d9?auto=format&fit=crop&w=1600&q=90",
  },
  {
    id: "mini-cake",
  ...
  },
  {
    ...
    emoji: "🪴",
    priceCents: 3900,
    caption: "Something that grows",
    eta: "1–2 days",
    photo:
      "https://images.unsplash.com/photo-1485955900006-10f4d324d411?auto=format&fit=crop&w=1600&q=90",
  },
  {
    id: "book-date",
    name: "Bookstore Surprise",
    emoji: "📚",
    priceCents: 3200,
    caption: "A thoughtful new read",
    eta: "1–2 days",
    photo:
      "https://images.unsplash.com/photo-1495446815901-a7297e633e8d?auto=format&fit=crop&w=1600&q=90",
  },
  {
    id: "self-care",
    name: "Self-care Box",
  ...
  },
  {
    ...
    emoji: "🕯️",
    priceCents: 3400,
    caption: "Warm amber & rose",
    eta: "Same day",
    photo:
      "https://images.unsplash.com/photo-1603006905003-be475563bc59?auto=format&fit=crop&w=1600&q=90",
  },
  {
    id: "fruit",
    ...
    emoji: "🍓",
    priceCents: 4500,
    caption: "Bright and beautiful",
    eta: "Same day",
    photo:
      "https://images.unsplash.com/photo-1619566636858-adf3ef46400b?auto=format&fit=crop&w=1600&q=90",
  },
  {
    id: "card",
    ...
    emoji: "💌",
    priceCents: 1800,
    caption: "Your words, delivered",
    eta: "1–2 days",
    photo:
      "https://images.unsplash.com/photo-1516383740770-fbcc5ccbece0?auto=format&fit=crop&w=1600&q=90",
  },
  {
    id: "movie-night",
    ...
    emoji: "🍿",
    priceCents: 4400,
    caption: "Snacks, soda and cozy vibes",
    eta: "Same day",
    photo:
      "https://images.unsplash.com/photo-1578849278619-e73505e9610f?auto=format&fit=crop&w=1600&q=90",
  },
];
...
const coupleGames = [
  {
    ...
    color: "#A71D35",
    prompt:
      "Send two true facts and one dream you want to build. Your match guesses the dream.",
  },
  {
    ...
    color: "#7A1FE0",
    prompt:
      "Choose fast: cozy coffee date ☕ or spontaneous road trip 🚗 — and explain why.",
  },
  {
    title: "Values Rank",
    icon: "diamond" as const,
    color: "#D4AF37",
    prompt: "Rank these for a future together: family, career, travel, home.",
  },
  {
    title: "Memory Builder",
    icon: "images" as const,
    color: "#B9293F",
    prompt: "Describe one perfect Sunday together in three lines.",
  },
];
const coupleThemes = [
  {
...
function Chat({
  ...
  const sendText = () => {
    ...
    if (value) {
      onSend(createMessage({ type: "text", text: value }));
      setText("");
      setShowEmoji(false);
    }
  };
  const startVoiceNote = async () => {
    ...
    const permission = await requestRecordingPermissionsAsync();
    if (!permission.granted) {
      setChatError("Microphone permission is needed to send a voice note.");
      return;
    }
    await setAudioModeAsync({ allowsRecording: true, playsInSilentMode: true });
    await recorder.prepareToRecordAsync();
  ...
  const sendOrRecord = () => {
    ...
    void (recorderState.isRecording ? stopVoiceNote() : startVoiceNote());
  };
  const shareLiveLocation = async () => {
    ...
    try {
      ...
      if (locationMessage.location)
        void persistLiveLocationShare(match.id, locationMessage.location);
      setShowAttachments(false);
    } catch {
      setChatError(
  ...
  const sendPhoto = async () => {
    ...
    const permission = await ImagePicker.requestMediaLibraryPermissionsAsync();
    if (!permission.granted) {
      setChatError("Photo permission is needed to share an image.");
      return;
    }
    const result = await ImagePicker.launchImageLibraryAsync({
      mediaTypes: ["images"],
    ...
    if (!result.canceled && result.assets[0]) {
      onSend(createMessage({ type: "image", uri: result.assets[0].uri }));
      setShowAttachments(false);
    }
  };
  ...
  const sendGif = (uri: string) => {
    onSend(createMessage({ type: "gif", uri }));
    setGifOpen(false);
    setShowAttachments(false);
  };
    ...
    onSend(
      createMessage({
        ...
        gift: {
          ...
          provider: order.quote.providerLabel,
  ...
  const startGame = (game: (typeof coupleGames)[number]) => {
    onSend(
      createMessage({ type: "text", text: `🎮 ${game.title}: ${game.prompt}` }),
    );
    setGamesOpen(false);
    setShowAttachments(false);
  };
  ...
  return (
    <LinearGradient
      colors={[activeTheme.bg, colors.black, activeTheme.bg]}
      style={{ flex: 1 }}
    >
      <SafeAreaView style={chatPremiumStyles.safeArea}>
        <View
          style={[
            styles.chatHead,
            chatPremiumStyles.chatHead,
            {
              backgroundColor: "rgba(14,3,7,.96)",
        ...
        >
          ...
          />
          <View style={{ flex: 1 }}>
            <Text style={shared.label}>{displayName}</Text>
            <View style={chatStyles.onlineRow}>
              ...
              <Text style={styles.onlineText}>
                {settings.nickname.trim() ? `${match.name} · Online` : "Online"}
              </Text>
            </View>
          </View>
          <Pressable
        ...
        <View style={[styles.safety, chatPremiumStyles.safety]}>
          <Ionicons
            name="shield-checkmark-outline"
            size={15}
            color={colors.gold}
          ...
          <Text style={styles.safetyText}>
            Safe chats stay inside DestinyOne.
          </Text>
        </View>
        <View
          style={[
            coachStyles.chatCoach,
            {
              backgroundColor: "rgba(15,3,7,.88)",
        ...
        >
          <View style={shared.row}>
            ...
            <Text style={coachStyles.chatCoachTitle}>Reply coach</Text>
            <View style={shared.spacer} />
            <Pressable
              onPress={() => navigate("coach")}
              style={chatStyles.coachOpen}
            >
              <Text style={chatStyles.coachOpenText}>Open</Text>
            </Pressable>
          </View>
          <ScrollView
            horizontal
          ...
          >
            {chatCoachSuggestions.map((item) => (
              <Pressable
                key={item.label}
                onPress={() => setText(item.message(match))}
                style={[
        ...
        {!!chatError && (
          ...
          >
            ...
            <MiniPremiumIcon name="close" tone="dark" size={28} iconSize={13} />
          </Pressable>
        )}
        <ScrollView
          contentContainerStyle={[styles.messages, chatPremiumStyles.messages]}
        >
          <View style={styles.iceReveal}>
            <Text style={styles.kicker}>ICEBREAKER REVEALED</Text>
            <Text style={styles.revealText}>
              You both chose:{" "}
              <Text style={{ color: colors.ivory }}>Road trip 🚗</Text>
            </Text>
          </View>
          <Text style={chatStyles.dayLabel}>TODAY</Text>
          <View style={[styles.theirBubble, chatPremiumStyles.theirBubble]}>
        ...
        >
          ...
          {showAttachments && (
            <ScrollView
              horizontal
              showsHorizontalScrollIndicator={false}
              contentContainerStyle={chatStyles.attachmentTray}
            >
              <Attachment
                icon="images"
                label="Gallery"
              ...
              <Attachment
                ...
                label="Camera"
                color="#E5092F"
                onPress={() => void sendCameraPhoto()}
              />
              <Attachment
                icon="location"
                label="Location"
                color="#D4AF37"
                onPress={() => void shareLiveLocation()}
              />
              <Attachment
                icon="aperture"
                label="Snap"
                color="#B9293F"
                onPress={() => setSnapOpen(true)}
              />
              <Attachment
                icon="person-circle"
                label="Funny Cam"
                color="#89162C"
                onPress={() => setFaceEmojiOpen(true)}
              />
              <Attachment
          ...
          {showEmoji && (
            <View style={chatStyles.emojiPanel}>
              <View style={chatStyles.emojiHeader}>
                <Text style={chatStyles.emojiTitle}>Emojis</Text>
                <Text style={chatStyles.emojiCount}>
                  {quickEmojis.length} daily-use
                </Text>
              </View>
              <ScrollView
                showsVerticalScrollIndicator={false}
                contentContainerStyle={chatStyles.emojiTray}
              >
                {quickEmojis.map((emoji, index) => (
                  ...
                  >
                    <Text style={chatStyles.emoji}>{emoji}</Text>
                  </Pressable>
                ))}
              </ScrollView>
            </View>
          ...
          <View style={[styles.composer, chatPremiumStyles.composer]}>
            ...
            <Pressable onPress={onRose} style={aiStyles.chatRose}>
              <PremiumIcon
                name="sparkles"
                tone="gold"
                size={32}
                iconSize={14}
              />
              {roseAvailability.freeAvailable && (
                <View style={aiStyles.freeDot} />
              )}
            </Pressable>
            <View
              style={[
            ...
            <Pressable
              ...
              style={[styles.send, { backgroundColor: "transparent" }]}
            >
              <PremiumIcon
                name={
                  text.trim()
                ...
                tone={recorderState.isRecording ? "gold" : "ruby"}
                size={40}
                iconSize={18}
              />
            </Pressable>
...
function ChatBubble({
  ...
  return (
    <View
      style={[
        styles.myBubble,
        message.type === "text" && accent
          ? {
              backgroundColor: "rgba(145,12,35,.94)",
              borderWidth: 1,
    ...
    >
      ...
      {message.type === "gift" && message.gift && (
        <>
          {message.gift.physical && message.gift.name.includes("Rose") ? (
            <RoseMark size={62} />
          ) : (
            <PremiumIcon
              name={message.gift.physical ? "gift" : "sparkles"}
              tone="gold"
              size={62}
              iconSize={29}
            />
          ...
          <Text style={chatStyles.giftCaption}>
            {message.text ??
              (message.gift.physical
                ? "Delivery request sent · address stays private"
                : "A digital gift sent with intention")}
          </Text>
          {message.gift.physical && (
            <View style={chatStyles.orderStatus}>
              <View style={chatStyles.orderDot} />
              <Text style={chatStyles.orderStatusText}>
                {giftStatusLabel(message.gift.deliveryStatus)}
              </Text>
            </View>
          )}
        </>
      ...
      {message.type === "gift" &&
        message.gift?.physical &&
        message.gift.steps && <GiftTrackingMini gift={message.gift} />}
      {message.type === "date" && message.date && (
        <>
          ...
          <DatePlanStatusMini safetyCheckIn={!!message.date.safetyCheckIn} />
          <Text style={dateStyles.waitingText}>Waiting for a response</Text>
        </>
      )}
        ...
        <View key={step[0]} style={dateStyles.dateFlowItem}>
          <View
            style={[
              ...
              index === 0 && dateStyles.dateFlowDotDone,
            ]}
          />
          <Text
            style={[
              dateStyles.dateFlowText,
              index === 0 && dateStyles.dateFlowTextOn,
            ]}
          >
            {step[1]}
          </Text>
...
function GiftTrackingMini({
  ...
  const openTracking = () => {
    setTrackingNotice("");
    if (gift.trackingUrl) {
      void Linking.openURL(gift.trackingUrl).catch(() =>
        setTrackingNotice(
          `Tracking is ready for order ${gift.orderId ?? "preview"}, but this browser could not open the partner link.`,
        ),
      );
      return;
    }
    setTrackingNotice(
      "Tracking opens after the recipient accepts privately and the delivery partner confirms the order.",
    );
  };
  return (
    <View style={giftFlowStyles.chatTrack}>
      <View style={shared.row}>
        <View style={{ flex: 1 }}>
          <Text style={giftFlowStyles.chatTrackTitle}>
            {gift.provider ?? "Delivery partner"} ·{" "}
            {gift.etaLabel ?? "ETA pending"}
          </Text>
          {gift.serviceLevel && (
            <Text style={giftFlowStyles.chatTrackFine}>
              {gift.serviceLevel} · {gift.acceptanceWindowMinutes ?? 30} min
              private accept window · {gift.etaConfidence ?? "ETA"} confidence
            </Text>
          )}
        </View>
        <Pressable onPress={openTracking}>
          <Text style={giftFlowStyles.trackLink}>Track</Text>
        </Pressable>
      </View>
      {!!trackingNotice && (
      ...
      {steps.map((step, index) => (
        <View key={`${step.key}-${index}`} style={giftFlowStyles.chatStep}>
          <View
            style={[
              giftFlowStyles.chatStepDot,
          ...
          <View style={{ flex: 1 }}>
            <Text
              style={[
                giftFlowStyles.chatStepLabel,
                step.status === "active" && { color: colors.ivory },
              ]}
            >
              {step.label}
            </Text>
            <Text style={giftFlowStyles.chatStepBody}>{step.body}</Text>
          </View>
        </View>
      ))}
      {gift.totalCents && (
        <Text style={giftFlowStyles.chatTrackFine}>
          Estimated total {formatGiftMoney(gift.totalCents)} · address remains
          private
        </Text>
      )}
      {gift.paymentPolicy && (
        <Text style={giftFlowStyles.chatTrackFine}>{gift.paymentPolicy}</Text>
      )}
  ...
  return (
    ...
    >
      ...
      <View style={chatStyles.voiceWave}>
        {[10, 18, 13, 24, 16, 21, 12, 19, 14, 23].map((height, index) => (
          <View
            key={index}
            style={[
              chatStyles.voiceBar,
              { height, opacity: status.playing || index % 2 === 0 ? 1 : 0.45 },
            ]}
          />
        ))}
      </View>
      <Text style={chatStyles.voiceDuration}>
        {seconds < 60
...
function Attachment({
  ...
  const tone: PremiumIconTone =
    color === "#D4AF37" || label === "Real Gift" || label === "Location"
      ? "gold"
      : label === "Games"
        ? "plum"
        : label === "GIF"
          ...
          : "ruby";
  return (
    <Pressable onPress={onPress} style={chatStyles.attachment}>
      <PremiumIcon name={icon} tone={tone} size={49} iconSize={21} />
      <Text style={chatStyles.attachmentLabel}>{label}</Text>
    </Pressable>
  );
}

function RoseComposer({
  visible,
  recipientName,
  availability,
  onClose,
  ...
  return (
    ...
    >
      ...
      <SafeAreaView style={[chatStyles.sheet, { maxHeight: "88%" }]}>
        <SheetHeader
          title="Choose a GIF"
          subtitle="100 everyday reaction GIFs"
          onClose={onClose}
        />
        <ScrollView
          showsVerticalScrollIndicator={false}
        ...
        >
          {chatGifs.map((gif, index) => (
            <Pressable
              key={`${gif.title}-${index}`}
              onPress={() => onSelect(gif.uri)}
              style={chatStyles.gifCard}
            >
              <Image source={{ uri: gif.uri }} style={styles.fill} />
              <Text style={chatStyles.gifTitle}>{gif.title}</Text>
            </Pressable>
          ))}
        </ScrollView>
      </SafeAreaView>
    ...
    >
      ...
      <SafeAreaView style={[chatStyles.sheet, { maxHeight: "92%" }]}>
                ...
                <View style={shared.row}>
                  ...
                  <View style={{ flex: 1, marginLeft: 10 }}>
                    ...
                    <Text style={giftFlowStyles.quoteMeta}>
                      {selectedQuote.serviceLevelLabel} ·{" "}
                      {selectedQuote.providerLabel} · ETA{" "}
          ...
          <>
            <View style={chatStyles.balance}>
              <MiniPremiumIcon
                ...
                iconSize={15}
              />
              <Text style={chatStyles.balanceText}>{balance} coins</Text>
              <Text style={chatStyles.balanceNote}>Demo wallet</Text>
            </View>
            <ScrollView contentContainerStyle={chatStyles.giftGrid}>
        ...
        <Text style={chatStyles.billingNote}>
          Production needs a contracted delivery partner, webhook tracking,
          recipient consent, and secure payment capture.
        </Text>
      </SafeAreaView>
    </Modal>
  );
...
function GiftFlowPreview({ quote }: { quote: GiftOrderQuote | null }) {
  const steps = [
    ...
    {
      title: "Prepare",
      body: quote?.pickupPartnerName ?? "Local partner prepares",
    },
      ...
      {plan.map((item) => (
        <View key={item.title} style={giftFlowStyles.readinessRow}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={giftFlowStyles.readinessBody}>{item.body}</Text>
          </View>
        </View>
      ))}
    </View>
...
function GameSheet({
  ...
  visible: boolean;
  onClose: () => void;
  onPlay: (game: (typeof coupleGames)[number]) => void;
}) {
  return (
    <Modal
      visible={visible}
    ...
    >
      ...
      <SafeAreaView style={chatStyles.sheet}>
        <SheetHeader
          title="Couple games"
          subtitle="Light, playful prompts that keep serious chats alive"
          onClose={onClose}
        />
        <View style={gameStyles.hero}>
          ...
          />
          <View style={{ flex: 1 }}>
            <Text style={styles.cardTitle}>Play inside chat</Text>
            <Text style={styles.helper}>
              Pick a game and DestinyOne sends the first prompt. Both people
              reply in the same conversation.
            </Text>
          </View>
        </View>
        <ScrollView contentContainerStyle={gameStyles.grid}>
          {coupleGames.map((game) => (
            ...
            >
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={gameStyles.body}>{game.prompt}</Text>
              </View>
              <MiniPremiumIcon
                name="play-circle"
                tone="rose"
        ...
        <Text style={chatStyles.billingNote}>
          Games are mutual-match only. Production can add live scoring, timers
          and private rooms later.
        </Text>
      </SafeAreaView>
    </Modal>
  );
    ...
    >
      ...
      <SafeAreaView style={[chatStyles.sheet, { maxHeight: "90%" }]}>
        <SheetHeader
          ...
          subtitle="Nickname, vibe and theme for this match"
          onClose={onClose}
        />
        <LinearGradient
        ...
        <View style={coupleStyles.section}>
          ...
          <Text style={styles.helper}>
            Only you see this nickname unless you choose to share it later.
          </Text>
        </View>
        <View style={coupleStyles.section}>
          <View style={shared.row}>
            <Text style={styles.sectionLabel}>10 DEFAULT COUPLE THEMES</Text>
            <View style={shared.spacer} />
            <Pressable
              onPress={() =>
                setStatus(
                  "Custom theme builder will unlock custom colors, emoji wallpaper and couple avatars. Preview includes 10 premium defaults now.",
                )
              }
              style={premiumButtonStyles.smallGhost}
            >
        ...
        <Text style={chatStyles.billingNote}>
          Themes save locally in preview. Production will sync them securely
          between devices after mutual match.
        </Text>
      </SafeAreaView>
    </Modal>
  );
...
function CallModal({
  ...
  return (
    <Modal visible transparent animationType="fade" onRequestClose={onClose}>
      <LinearGradient
        colors={["rgba(22,0,7,.97)", "rgba(8,0,3,.98)"]}
      ...
      >
        <SafeAreaView style={callStyles.content}>
          ...
          <View style={callStyles.avatarWrap}>
            ...
            />
            <View style={callStyles.callPulse} />
          </View>
          <Text style={callStyles.callName}>{match.name}</Text>
          <Text style={callStyles.callStatus}>
            {muted
              ? "You are muted"
              : mode === "video" && cameraOn
                ? "Secure video preview active"
                ...
                <>
                  ...
                  <View style={callStyles.callStatePill}>
                    <MiniPremiumIcon
                      ...
                      iconSize={11}
                    />
                    <Text style={callStyles.callStateText}>Camera on</Text>
                  </View>
                </>
              ) : (
                <>
          ...
          <Text style={callStyles.callFine}>
            Preview simulates call controls locally. Production will connect
            WebRTC, permissions, push ringing and reporting.
          </Text>
        </SafeAreaView>
      </LinearGradient>
    </Modal>
...
function SheetHeader({
  ...
  return (
    <View style={chatStyles.sheetHeader}>
      <View>
        <Text style={shared.h2}>{title}</Text>
...
function AppNoticeSheet({
  ...
  return (
    <Modal visible transparent animationType="slide" onRequestClose={onClose}>
      <Pressable style={chatStyles.modalBackdrop} onPress={onClose} />
      <SafeAreaView style={noticeStyles.sheet}>
        <View style={noticeStyles.hero}>
          <PremiumIcon
            name={notice.icon}
            tone={notice.tone ?? "gold"}
            size={58}
            iconSize={27}
          />
          <View style={{ flex: 1 }}>
            <Text style={noticeStyles.title}>{notice.title}</Text>
            <Text style={noticeStyles.body}>{notice.body}</Text>
          </View>
        </View>
        <View style={noticeStyles.actions}>
          {notice.actionLabel && notice.actionScreen ? (
            <Button
              label={notice.actionLabel}
              icon="arrow-forward"
              variant="gold"
              onPress={() => onAction(notice.actionScreen!)}
            />
          ) : null}
          <Button
            label="Done"
            variant={notice.actionLabel ? "secondary" : "primary"}
            onPress={onClose}
          />
        </View>
      </SafeAreaView>
...
function SafetyActions({
  ...
  const confirmCopy =
    confirmAction === "block"
      ? {
          ...
          body: "You will no longer see each other in matches, likes or chats. They are not notified.",
      ...
      : {
          ...
          body: "This removes the connection and conversation from your DestinyOne preview.",
  ...
  return (
    ...
    >
      <Pressable style={chatStyles.modalBackdrop} onPress={close} />
      <SafeAreaView style={[chatStyles.sheet, { maxHeight: "82%" }]}>
        {confirmAction ? (
          <>
            <SheetHeader
              title={confirmCopy.title}
              subtitle="This action helps keep your space intentional."
              onClose={close}
            />
            <View style={safetyStyles.confirmCard}>
              <PremiumIcon
                name={confirmCopy.icon}
                tone="ruby"
                size={58}
                iconSize={27}
              />
              <View style={{ flex: 1 }}>
                <Text style={styles.cardTitle}>{confirmCopy.title}</Text>
                <Text style={styles.helper}>{confirmCopy.body}</Text>
              </View>
            </View>
            <View style={safetyStyles.confirmActions}>
              <Button
          ...
          <>
            <SheetHeader
              ...
              subtitle={`${match.name} will not be notified`}
              onClose={close}
            />
            <ScrollView contentContainerStyle={{ gap: 9 }}>
              {reportReasons.map((item) => (
                <Pressable
                  ...
                  style={[
                    safetyStyles.reason,
                    reason === item && safetyStyles.reasonOn,
                  ]}
                >
                  <Text style={safetyStyles.reasonText}>{item}</Text>
                  <MiniPremiumIcon
                    name={
                      reason === item ? "checkmark-circle" : "ellipse-outline"
                    }
                    tone={reason === item ? "gold" : "dark"}
                    size={32}
                    iconSize={15}
                  />
                </Pressable>
              ...
              <TextInput
                ...
                placeholderTextColor="#856F81"
                style={safetyStyles.reportInput}
              />
            </ScrollView>
            <Button
              disabled={!reason}
              label="Submit report"
              onPress={() => {
                onReport(reason, details.trim() || undefined);
                close();
              }}
            />
          </>
          ...
          <>
            <SheetHeader
              title={match.name}
              subtitle="Your safety and boundaries come first"
              onClose={close}
            />
            <SafetyAction
              icon="shield-checkmark-outline"
              title="Safety Center"
              body="Check-ins, guidance and privacy controls"
              onPress={onSafetyCenter}
            />
            <SafetyAction
              icon="flag-outline"
              title="Report"
              body="Privately alert the DestinyOne safety team"
              onPress={() => setReportMode(true)}
            />
            <SafetyAction
              icon="person-remove-outline"
              title="Unmatch"
              body="Remove this connection and conversation"
              danger
              onPress={() => setConfirmAction("unmatch")}
            />
            <SafetyAction
              icon="ban-outline"
              title="Block"
              body="Immediately hide each other everywhere"
              danger
...
function SafetyAction({
  ...
  return (
    <Pressable onPress={onPress} style={safetyStyles.action}>
      <PremiumIcon
        name={icon}
        tone={danger ? "ruby" : "rose"}
        size={44}
        iconSize={20}
      />
      <View style={{ flex: 1 }}>
        <Text style={[styles.cardTitle, danger && { color: colors.danger }]}>
          {title}
        </Text>
        <Text style={styles.helper}>{body}</Text>
      </View>
...
function SafetyCenter({
  ...
  const checklist = buildSafetyChecklist({
    ...
    blockedCount,
    datePlansCount: datePlans.length,
    safeCheckInsCount: safeCheckIns.length,
  });
  return (
    <LinearGradient
      colors={["#260620", colors.black, colors.black]}
    ...
    >
      <SafeAreaView style={shared.safe}>
        ...
        <ScrollView
          contentContainerStyle={safetyStyles.content}
          showsVerticalScrollIndicator={false}
        >
          <View style={safetyStyles.hero}>
            <PremiumIcon
              name="shield-checkmark"
              tone="gold"
              size={76}
            ...
            <Text style={[shared.body, { textAlign: "center" }]}>
              Tools for boundaries, safer dates and complete control of your
              data.
            </Text>
          </View>
          <View style={safetyStyles.overview}>
            <SafetyStat value={reports.length} label="Reports submitted" />
            <SafetyStat value={blockedCount} label="People blocked" />
            <SafetyStat value={safeCheckIns.length} label="Safe check-ins" />
          </View>
          <SafetyReadinessCard checklist={checklist} />
          {reports.length > 0 && (
            <View style={safetyStyles.section}>
              <Text style={styles.sectionLabel}>LATEST REPORT ACTIONS</Text>
              {reports
                .slice(-2)
                .reverse()
                .map((report) => (
                  <SafetyReportPlan key={report.id} report={report} />
                ))}
            </View>
          )}
          {datePlans.length > 0 && (
            <View style={safetyStyles.section}>
              <Text style={styles.sectionLabel}>DATE CHECK-INS</Text>
              {datePlans.slice(-2).map((message) => (
                <View key={message.id} style={safetyStyles.checkInCard}>
                  <PremiumIcon
                    name="calendar"
                    tone="gold"
                    size={42}
                  ...
                  />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.cardTitle}>{message.date?.venue}</Text>
                    <Text style={styles.helper}>
                      {message.date?.time} · {message.date?.area}
                    </Text>
                  </View>
                  {safeCheckIns.includes(message.id) ? (
                    <View style={safetyStyles.safeDone}>
                      <MiniPremiumIcon
                        name="checkmark"
                        tone="gold"
                        size={24}
                        iconSize={11}
                      />
                      <Text style={safetyStyles.safeDoneText}>I’m safe</Text>
                    </View>
                  ) : (
                    <Pressable
                      onPress={() => onCheckIn(message.id)}
                      style={safetyStyles.checkInButton}
                    >
                      <Text style={safetyStyles.checkInButtonText}>
                        Check in
                      </Text>
                    </Pressable>
                  )}
          ...
          <View style={safetyStyles.warning}>
            <PremiumIcon
            ...
            <View style={{ flex: 1 }}>
              <Text style={styles.cardTitle}>
                Money requests are a red flag
              </Text>
              <Text style={styles.helper}>
                Never send money, crypto, gift cards or financial details to
                someone you met through the app. Report the conversation
                immediately.
              </Text>
            </View>
          </View>
          <View style={safetyStyles.section}>
            ...
            <SafetyAction
              ...
              body="Let someone you trust know where you’ll be"
              onPress={() => setActiveTool("plan")}
            />
            <SafetyAction
              icon="call-outline"
              title="Emergency help"
              body="Call local emergency services if you are in immediate danger"
              onPress={() => setActiveTool("emergency")}
            />
            <SafetyAction
              icon="document-lock-outline"
              title="Privacy controls"
              body="Review discovery, location and stored activity"
              onPress={() => setActiveTool("privacy")}
            />
          </View>
          <View style={safetyStyles.section}>
            <Text style={styles.sectionLabel}>YOUR DATA</Text>
            <Pressable
              onPress={() => setActiveTool("data")}
              style={safetyStyles.dataCard}
            >
              <PremiumIcon
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.helper}>
                  Backend-ready export request for profile, matches and
                  messages.
                </Text>
              </View>
              <MiniPremiumIcon
            ...
            <Pressable
              onPress={() => setActiveTool("delete")}
              style={safetyStyles.deleteCard}
            >
              <PremiumIcon
                name="trash-outline"
                tone="ruby"
          ...
          <Text style={safetyStyles.disclaimer}>
            DestinyOne cannot guarantee another person’s identity or behavior.
            Meet in public, arrange your own transport and trust your instincts.
          </Text>
        </ScrollView>
        {activeTool && (
          <SafetyToolSheet
...
function SafetyReadinessCard({
  ...
  return (
    <View style={safetyStyles.guardianCard}>
      <View style={shared.row}>
        <PremiumIcon
        ...
        />
        <View style={{ flex: 1, marginLeft: 10 }}>
          <Text style={styles.kicker}>SAFETY GUARDIAN</Text>
          <Text style={styles.cardTitle}>
            {score}% launch-ready safety layer
          </Text>
          <Text style={styles.helper}>
            Reports, blocks, date check-ins and Trust Ops routing are wired for
            preview before backend keys.
          </Text>
        </View>
      </View>
      <View style={safetyStyles.guardianTrack}>
        <View style={[safetyStyles.guardianFill, { width: `${score}%` }]} />
      </View>
      {checklist.map((item) => (
        <View key={item.key} style={safetyStyles.guardianRow}>
          <MiniPremiumIcon
          ...
          />
          <View style={{ flex: 1 }}>
            <Text style={safetyStyles.guardianTitle}>{item.title}</Text>
            <Text style={safetyStyles.guardianBody}>{item.body}</Text>
          </View>
        </View>
      ))}
    </View>
  ...
  const riskTone =
    plan.risk === "Critical" || plan.risk === "High"
      ...
      : plan.risk === "Medium"
        ...
        : "rose";
  return (
    <View style={safetyStyles.reportPlanCard}>
      <MiniPremiumIcon
        name="flag-outline"
        tone={riskTone}
        size={34}
      ...
      />
      <View style={{ flex: 1 }}>
        <Text style={safetyStyles.guardianTitle}>
          {plan.category.replace("_", " ")} · {plan.risk}
        </Text>
        <Text style={safetyStyles.guardianBody}>{plan.primaryAction}</Text>
        <Text style={safetyStyles.guardianBody}>{plan.memberCopy}</Text>
      </View>
      <Text style={safetyStyles.reportPlanScore}>{plan.riskScore}</Text>
    </View>
  );
}

function SafetyToolSheet({
  tool,
  datePlans,
  safeCheckIns,
  onCheckIn,
  ...
  const planText = latestPlan?.date
    ? `DestinyOne date plan: ${latestPlan.date.venue}, ${latestPlan.date.area}, ${latestPlan.date.time}. Safety check-in: ${latestPlan.date.safetyCheckIn ? "on" : "off"}.`
    : "No date plan is saved yet. Plan a date from chat first.";
  const sharePlan = async () => {
    setSharedPlan(true);
    setSharePlanStatus("");
    try {
      await Share.share({ title: "DestinyOne date plan", message: planText });
      setSharePlanStatus(
        "Date plan share card is ready for your trusted contact.",
      );
    } catch {
      setSharePlanStatus(
        "Share sheet is unavailable in this preview. Copy this plan from the card above and send it to someone you trust.",
      );
    }
  };
  const callEmergency = () => {
    setEmergencyFallback(false);
    void Linking.openURL("tel:911").catch(() => setEmergencyFallback(true));
  };
  const titles = {
    plan: [
      "Share date plan",
      "Send a public-place plan to someone you trust.",
    ] as const,
    emergency: [
      "Emergency help",
      "Fast actions if something feels unsafe.",
    ] as const,
    privacy: [
      "Privacy controls",
      "Control what signals are visible and what the app remembers.",
    ] as const,
    data: [
      "Download my data",
      "Prepare a private export request for your account data.",
    ] as const,
  ...
  return (
    <Modal visible transparent animationType="slide" onRequestClose={onClose}>
      <Pressable style={chatStyles.modalBackdrop} onPress={onClose} />
      <SafeAreaView style={[chatStyles.sheet, { maxHeight: "86%" }]}>
        <SheetHeader
          title={titles[tool][0]}
          subtitle={titles[tool][1]}
          onClose={onClose}
        />
        {tool === "plan" && (
          <View style={{ gap: 12 }}>
            <View style={safetyStyles.toolHero}>
              <PremiumIcon
                name="location"
                tone="gold"
                size={50}
              ...
              <View style={{ flex: 1 }}>
                <Text style={styles.cardTitle}>
                  {latestPlan?.date?.venue ?? "No active date plan yet"}
                </Text>
                <Text style={styles.helper}>
                  {latestPlan?.date
                    ? `${latestPlan.date.time} · ${latestPlan.date.area}`
                    : "Open chat → Date to create one before sharing."}
                </Text>
              </View>
            </View>
            <View style={safetyStyles.sharePreview}>
              <Text style={safetyStyles.shareText}>{planText}</Text>
            </View>
            {!!sharePlanStatus && (
              <View style={safetyStyles.inlineNotice}>
                <MiniPremiumIcon
                  name="share-social"
                  tone="gold"
                  size={28}
                  iconSize={13}
                />
                <Text style={safetyStyles.inlineNoticeText}>
                  {sharePlanStatus}
                </Text>
              </View>
            )}
            {latestPlan && !safeCheckIns.includes(latestPlan.id) && (
              <Pressable
                onPress={() => onCheckIn(latestPlan.id)}
                style={safetyStyles.checkInWide}
              >
                <MiniPremiumIcon
                  name="shield-checkmark"
                  tone="gold"
                  ...
                  iconSize={12}
                />
                <Text style={safetyStyles.checkInWideText}>
                  Mark this plan as safe after date
                </Text>
              </Pressable>
            )}
            <Button
        ...
        {tool === "emergency" && (
          <View style={{ gap: 12 }}>
            <View style={safetyStyles.emergencyCard}>
              <PremiumIcon name="warning" tone="ruby" size={54} iconSize={25} />
              <View style={{ flex: 1 }}>
                <Text style={[styles.cardTitle, { color: colors.danger }]}>
                  If you are in immediate danger
                </Text>
                <Text style={styles.helper}>
                  Leave the situation if you can, move to a public place, and
            ...
            />
            {emergencyFallback && (
              <View style={safetyStyles.emergencyFallback}>
                <MiniPremiumIcon
                  name="call"
                  tone="ruby"
                  size={30}
                  iconSize={14}
                />
                <Text style={safetyStyles.emergencyFallbackText}>
                  This preview could not open your phone dialer. Please call
                  local emergency services immediately from your device.
                </Text>
              </View>
            )}
            <View style={safetyStyles.toolList}>
              {[
                "Keep conversations in DestinyOne until trust is built.",
                "Do not share exact home/work address early.",
                "Use your own transport for first dates.",
                "Report pressure, money requests, threats or fake identity.",
              ].map((item) => (
                <View key={item} style={safetyStyles.toolRow}>
                  <MiniPremiumIcon
                    name="checkmark-circle"
                    tone="gold"
                    size={26}
                    iconSize={12}
                  />
                  <Text style={safetyStyles.toolRowText}>{item}</Text>
                </View>
              ))}
            </View>
          </View>
        ...
        {tool === "privacy" && (
          <View style={{ gap: 11 }}>
            <SafetyLocalToggle
              title="Hide last online"
              body="Keep your recent activity private from matches."
              value={privacy.hideLastSeen}
              onPress={() =>
                setPrivacy((current) => ({
                  ...current,
                  hideLastSeen: !current.hideLastSeen,
                }))
              }
            />
            <SafetyLocalToggle
              title="Limit approximate location"
              body="Use city-level discovery and avoid exact-place matching."
              value={privacy.pauseLocation}
              onPress={() =>
                setPrivacy((current) => ({
                  ...current,
                  pauseLocation: !current.pauseLocation,
                }))
              }
            />
            <SafetyLocalToggle
              title="Private profile-view alerts"
              body="Only send profile-view notifications after deeper views."
              value={privacy.limitProfileViews}
              onPress={() =>
                setPrivacy((current) => ({
                  ...current,
                  limitProfileViews: !current.limitProfileViews,
                }))
              }
            />
            <View style={safetyStyles.privacySummary}>
              <MiniPremiumIcon
                name="lock-closed"
                tone="gold"
                size={30}
              ...
              />
              <Text style={[styles.helper, { flex: 1 }]}>
                Preview controls update instantly here. Production will sync
                them securely to your account.
              </Text>
            </View>
          </View>
        )}
          ...
          <View style={{ gap: 12 }}>
            <View style={safetyStyles.toolHero}>
              ...
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.helper}>
                  Profile, preferences, match decisions, safety reports and chat
                  metadata.
                </Text>
              </View>
            </View>
            <View style={safetyStyles.toolList}>
              {[
                ...
                "Match decisions and filters",
                "Reports, blocks and safety check-ins",
                "Messages export after identity verification",
              ].map((item) => (
                <View key={item} style={safetyStyles.toolRow}>
                  <MiniPremiumIcon
            ...
            {exportRequested ? (
              <View style={safetyStyles.exportReady}>
                <MiniPremiumIcon
                  name="checkmark-circle"
                  tone="gold"
                  size={32}
                  iconSize={15}
                />
                <Text style={safetyStyles.exportReadyText}>
                  Export request prepared. Backend will email a secure link
                  after identity verification.
                </Text>
              </View>
            ) : (
              <Button
                label="Prepare export request"
                icon="download"
        ...
        {tool === "delete" && (
          <View style={{ gap: 12 }}>
            <View style={safetyStyles.deleteConfirm}>
              <PremiumIcon
                name="trash-outline"
                tone="ruby"
                size={58}
              ...
              <View style={{ flex: 1 }}>
                <Text style={[styles.cardTitle, { color: colors.danger }]}>
                  Delete your DestinyOne account?
                </Text>
                <Text style={styles.helper}>
                  This deletes your profile and associated data. Active
                  app-store subscriptions must be managed separately.
                </Text>
              </View>
            </View>
            <View style={safetyStyles.toolList}>
              {[
                "Your profile will stop appearing in matches.",
                "Reports and safety records may be retained where legally required.",
                "This preview action clears local app data after confirmation.",
              ].map((item) => (
                <View key={item} style={safetyStyles.toolRow}>
                  <MiniPremiumIcon
                    name="alert-circle-outline"
...
function SafetyLocalToggle({
  ...
  return (
    <Pressable onPress={onPress} style={safetyStyles.localToggle}>
      <View style={{ flex: 1 }}>
        <Text style={safetyStyles.localToggleTitle}>{title}</Text>
        <Text style={styles.helper}>{body}</Text>
      </View>
      <View style={[discoveryStyles.switch, value && discoveryStyles.switchOn]}>
        <View
...
function SafetyStat({ value, label }: { value: number; label: string }) {
  return (
    <View style={safetyStyles.stat}>
      <Text style={safetyStyles.statValue}>{value}</Text>
      <Text style={safetyStyles.statLabel}>{label}</Text>
    </View>
  );
}
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      >
        <SectionTitle
          ...
          body="Upgrade to DestinyOne Plus to see everyone who’s interested."
        />
        <View style={styles.likesGrid}>
          {matches.slice(0, 2).map((m) => (
            <View key={m.id} style={styles.likeCard}>
              <Image
                source={{ uri: m.photo }}
                blurRadius={18}
                style={styles.fill}
        ...
        <View style={[shared.card, { gap: 14, borderColor: "#6C5520" }]}>
          ...
          <Text style={shared.body}>
            Plus members can see likes, meet up to 5 daily matches, and hear
            voice intros.
          </Text>
          <Button
      ...
      <BottomNav active="likes" navigate={navigate} />
    </SafeAreaView>
  );
}
...
function Profile({
  ...
  const providerModules = productionDataModules.filter(
    (module) => module.needsProvider,
  );
  const shareProfileTag = async () => {
    setProfileShareStatus("");
    try {
      await Share.share({
        title: "DestinyOne profile tag",
        message: `My DestinyOne tag is ${profileTag}. Serious matches can find me here: ${profileLink}`,
      });
      setProfileShareStatus(
        "Profile tag share card is ready. Keep this link for people you already trust.",
      );
    } catch {
      setProfileShareStatus(
        `Share sheet is unavailable in this preview. Copy this profile tag: ${profileTag} · ${profileLink}`,
      );
    }
  };
  const actions = [
    ...
    <SafeAreaView style={{ flex: 1 }}>
      ...
      >
        ...
        >
          ...
          <View style={profilePremiumStyles.avatarHalo}>
            <View style={profilePremiumStyles.avatarRing}>
              {profilePhoto ? (
                ...
                <Text style={[styles.avatarText, { fontSize: 38 }]}>
          ...
          <View style={profilePremiumStyles.nameRow}>
          ...
          <Text style={profilePremiumStyles.meta}>
            {displayProfession} · {displayCity}
          </Text>
          <View style={mediaStyles.mediaBadges}>
            {verified && <Chip label="Selfie verified" selected />}
            {hasVoiceIntro && <Chip label="Voice intro" selected />}
            <Chip label="Serious intent" gold />
          </View>
          <View style={profilePremiumStyles.stats}>
            <View style={profilePremiumStyles.stat}>
              <Text style={profilePremiumStyles.statValue}>24</Text>
              <Text style={profilePremiumStyles.statLabel}>profile views</Text>
            </View>
            ...
            <View style={profilePremiumStyles.stat}>
              ...
              <Text style={profilePremiumStyles.statLabel}>strength</Text>
            </View>
            <View style={profilePremiumStyles.statLine} />
            <View style={profilePremiumStyles.stat}>
              <Text style={profilePremiumStyles.statValue}>
                {lastSeenVisible ? "12m" : "Hidden"}
              </Text>
              <Text style={profilePremiumStyles.statLabel}>last online</Text>
            </View>
          </View>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.helper}>
              {profileTag} · Share only with people you trust
            </Text>
          </View>
          <PremiumIcon
            name="share-social"
        ...
        {!!profileShareStatus && (
          <View style={profilePremiumStyles.shareStatus}>
            <MiniPremiumIcon name="link" tone="gold" size={30} iconSize={14} />
            <Text style={profilePremiumStyles.shareStatusText}>
              {profileShareStatus}
            </Text>
          </View>
        )}
        <View style={privacyStyles.card}>
          <PremiumIcon
            name={lastSeenVisible ? "eye-outline" : "eye-off-outline"}
            tone="gold"
            size={43}
            iconSize={20}
          />
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.helper}>
              {lastSeenVisible
                ? "Matches can see “last online 12 min ago”."
                : "Your last online time is hidden. Online dot is private."}
            </Text>
            <View style={privacyStyles.toggleRow}>
              <Pressable
                onPress={() => onLastSeenVisibleChange(true)}
                style={[
                  privacyStyles.toggle,
                  lastSeenVisible && privacyStyles.toggleOn,
                ]}
              >
                <Text
                  style={[
                    privacyStyles.toggleText,
                    lastSeenVisible && privacyStyles.toggleTextOn,
                  ]}
                >
                  Show
                </Text>
              ...
              <Pressable
                onPress={() => onLastSeenVisibleChange(false)}
                style={[
                  privacyStyles.toggle,
                  !lastSeenVisible && privacyStyles.toggleOn,
                ]}
        ...
        >
          <View>
            ...
            <Text style={styles.plusTitle}>More thoughtful possibilities.</Text>
          </View>
          <PremiumIcon
        ...
        {actions.map((action) => (
          ...
          >
            <PremiumIcon
              name={action.icon}
              tone={
                action.label.includes("Executive")
                  ? "gold"
                  : action.label.includes("Coach")
                    ? "plum"
                    : "rose"
              }
              size={42}
              iconSize={19}
            />
            <Text style={[shared.label, { flex: 1 }]}>{action.label}</Text>
            <MiniPremiumIcon
              name="chevron-forward"
              tone="dark"
              size={30}
        ...
        <Pressable onPress={onReset} style={styles.resetButton}>
          ...
          <Text style={styles.resetText}>
            {backendMode === "demo" ? "Reset local preview" : "Sign out"}
          </Text>
        </Pressable>
      </ScrollView>
...
function ProfileSettingsSheet({
  ...
  const [settingsStatus, setSettingsStatus] = useState(
    "Settings are saved on this device for the preview.",
  );
  const toggleNotifications = () => {
    ...
    setSettingsStatus(
      next
        ? "Notifications turned on for matches, Sparks and calls."
        : "Notifications turned off for this preview.",
    );
  };
  const togglePrivateMode = () => {
    ...
    setSettingsStatus(
      next
        ? "Private profile mode is on. You are hidden from discovery in this preview."
  ...
  const togglePauseDiscovery = () => {
    ...
    setSettingsStatus(
      next
        ? "Discovery paused. New daily decks will stop in preview mode."
        : "Discovery resumed. Daily decks can continue.",
    );
  };
    ...
    >
      ...
      <SafeAreaView style={[chatStyles.sheet, { maxHeight: "86%" }]}>
        ...
        <View style={settingsSheetStyles.hero}>
          ...
          <View style={{ flex: 1 }}>
            <Text style={styles.cardTitle}>
              Make DestinyOne feel private by default.
            </Text>
            <Text style={styles.helper}>
              These controls are local preview states. Backend sync comes later.
            </Text>
          </View>
        </View>
        <SettingsSwitch
          ...
          body="Get alerts for matches, Sparks, calls and support updates."
          value={notifications}
...
function SettingsSwitch({
  ...
  return (
    <Pressable onPress={onPress} style={settingsSheetStyles.switchRow}>
      <PremiumIcon
        name={icon}
...
function BackendReadyCard({
  snapshot,
  providerModules,
}: {
  snapshot: ReturnType<typeof getLaunchReadinessSnapshot>;
  providerModules: readonly AppDataModule[];
}) {
  const topProviders = providerModules.slice(0, 4);
  return (
    <View style={backendReadyStyles.card}>
      <View style={shared.row}>
        <PremiumIcon
          name="layers-outline"
          tone="gold"
        ...
        <View style={{ flex: 1, marginLeft: 10 }}>
          ...
          <Text style={styles.cardTitle}>
            Data model is ready before API keys.
          </Text>
          <Text style={styles.helper}>
            Supabase/API linking will be the final step; app screens now follow
      ...
      <View style={backendReadyStyles.stats}>
        <BackendReadyStat
          value={`${snapshot.backendReadyModules}/${snapshot.totalModules}`}
          label="modules"
        />
        <BackendReadyStat
          value={`${snapshot.realtimeModules}`}
          label="realtime"
        />
        <BackendReadyStat
          value={`${snapshot.adminReviewModules}`}
          label="admin review"
        />
      </View>
      <View style={backendReadyStyles.providerQueue}>
        <Text style={styles.sectionLabel}>PROVIDER QUEUE</Text>
        {topProviders.map((module) => (
          <View key={module.key} style={backendReadyStyles.providerRow}>
            <MiniPremiumIcon
              name={
                module.adminReview
                  ? "shield-checkmark-outline"
              ...
              tone={module.adminReview ? "gold" : "rose"}
              size={28}
              iconSize={13}
            />
            <View style={{ flex: 1 }}>
              <Text style={backendReadyStyles.providerTitle}>
                {module.label}
              </Text>
              <Text style={backendReadyStyles.providerBody}>
                {module.backendTable}
              </Text>
            </View>
          </View>
...
function BackendReadyStat({ value, label }: { value: string; label: string }) {
  return (
    <View style={backendReadyStyles.stat}>
...
function SupportCenter({ onBack }: { onBack: () => void }) {
  ...
  const submit = async () => {
    ...
    setSubmitting(true);
    try {
      const saved = await submitSupportTicket(
      ...
      setTicketNote(
        saved
          ? "Saved in Supabase. Support can review this when backend session is active."
          : "Saved in preview. Supabase session is needed for real ticket storage.",
      );
      setMessage("");
      setSupportInfo({
        ...
        tone: "gold",
        bullets: [
          saved ? "Synced to support storage" : "Saved in preview mode",
          "You can keep using the app while support reviews it",
          "Sensitive details stay inside the safety/support flow",
        ],
      });
    } catch (error) {
      setTicket(ticketId);
      setTicketNote(
        error instanceof Error
          ? error.message
          : "Backend ticket storage is not available yet. Saved locally in preview.",
      );
      setMessage("");
      setSupportInfo({
        ...
        tone: "rose",
        bullets: [
          "Backend ticket storage is not available right now",
          "Your preview ticket is still shown on this screen",
          "Production will retry or store this in Supabase",
        ],
      });
    } finally {
      setSubmitting(false);
  ...
  const emailSupport = () =>
    Linking.openURL(
      `mailto:support@destinyone.app?subject=${encodeURIComponent(`DestinyOne ${topic} help`)}&body=${encodeURIComponent(message || "Hi DestinyOne team, I need help with ")}`,
    ).catch(() =>
      setSupportInfo({
        title: "Email support",
        body: "support@destinyone.app",
        icon: "mail-outline",
        tone: "gold",
        bullets: [
          "Copy this email if your device cannot open mail automatically",
          "Include your ticket ID if you already submitted one",
  ...
  const faqs = [
    [
      ...
      "Verification photos stay private and help reduce fake profiles.",
    ],
    [
      "How matching works",
      "Your stated preferences, filters and in-app activity rank matches. We never read phone searches.",
    ],
    [
      "Refunds and billing",
      "Subscriptions and Spark packs use app-store billing, so users can restore and cancel safely.",
    ],
  ];
  return (
    <LinearGradient
      colors={["#260007", colors.black, colors.black]}
      style={{ flex: 1 }}
    >
      <SafeAreaView style={shared.safe}>
        ...
        <ScrollView
          contentContainerStyle={supportStyles.content}
          showsVerticalScrollIndicator={false}
        >
          <View style={supportStyles.hero}>
            <PremiumIcon name="headset" tone="ruby" size={68} iconSize={31} />
            <View style={supportStyles.liveStatus}>
              <View style={supportStyles.liveDot} />
              <Text style={supportStyles.liveText}>Support preview online</Text>
            </View>
            <Text style={[shared.h1, { textAlign: "center" }]}>
              We’re here when something feels off.
            </Text>
            <Text style={[shared.body, { textAlign: "center" }]}>
              Choose a topic, send a note, or use safety actions. Production
              connects this page to tickets, moderation and billing support.
            </Text>
          </View>
          {ticket ? (
            <View style={supportStyles.ticketCard}>
              <PremiumIcon
                name="checkmark-circle"
                tone="gold"
                size={44}
              ...
              />
              <View style={{ flex: 1 }}>
                <Text style={styles.cardTitle}>Latest ticket: {ticket}</Text>
                <Text style={styles.helper}>
                  {ticketNote ||
                    "Saved locally in this preview. Backend step will store tickets in Supabase and notify the support team."}
                </Text>
              </View>
            </View>
          ) : null}
          <View style={supportStyles.topicGrid}>
            {topics.map((item) => (
              <Pressable
                key={item.label}
                onPress={() => setTopic(item.label)}
                style={[
                  supportStyles.topicCard,
                  topic === item.label && supportStyles.topicCardOn,
                ]}
              >
                <PremiumIcon
                  name={item.icon}
                  tone={topic === item.label ? "gold" : "rose"}
                  size={44}
                  iconSize={20}
                />
                <Text style={supportStyles.topicText}>{item.label}</Text>
                <Text style={supportStyles.topicSla}>{item.sla}</Text>
              </Pressable>
            ))}
          </View>
          <View style={shared.card}>
            <View style={shared.row}>
              <View style={{ flex: 1 }}>
                ...
                <Text style={styles.helper}>Selected queue: {topic}</Text>
              </View>
              <MiniPremiumIcon
                name="lock-closed"
                tone="gold"
            ...
            <Button
              ...
              label={submitting ? "Creating ticket…" : "Submit support request"}
              icon="send"
              onPress={() => void submit()}
            />
          </View>
          <View style={supportStyles.quickGrid}>
            <SupportQuickCard
              icon="shield-checkmark-outline"
              title="Safety guide"
              body="Dating safety, boundaries and reporting."
              onPress={() =>
                setSupportInfo({
                  title: "Safety guide",
                  body: "Small habits make first meetings safer.",
                  icon: "shield-checkmark-outline",
                  tone: "gold",
                  bullets: [
                    "Meet in public for early dates",
                    "Keep early conversations inside DestinyOne",
                    "Use date check-ins and share plans with someone trusted",
                    "Report pressure, threats, fake identity or money requests",
                  ],
                })
              }
            />
            <SupportQuickCard
              icon="card-outline"
              title="Billing help"
              body="Subscriptions, restore purchase and refunds."
              onPress={() =>
                setSupportInfo({
                  title: "Billing help",
                  body: "Payments are designed to stay store-compliant and restorable.",
                  icon: "card-outline",
                  tone: "gold",
                  bullets: [
                    "Subscriptions and Spark packs use Apple/Google in-app billing",
                    "Members can restore purchases from their store account",
                    "Real-world date holds use secure payment partners after venue confirmation",
            ...
            <SupportQuickCard
              icon="mail-outline"
              title="Email team"
              body="Open a pre-filled support email."
              onPress={emailSupport}
            />
            <SupportQuickCard
              icon="bug-outline"
              title="Send diagnostics"
              body="Preview app/device details for support."
              onPress={() =>
                setSupportInfo({
                  title: "Diagnostics ready",
                  body: "Production can attach safe app context without exposing private conversations.",
                  icon: "bug-outline",
                  tone: "rose",
                  bullets: [
                    `Device surface: ${Platform.OS}`,
                    "App area: Support Center",
                    "Backend mode and app version only",
                    "No passwords, OTPs, photos or message content",
                  ],
                })
              }
            />
          ...
          <View style={{ gap: 10 }}>
            ...
            {faqs.map(([title, body]) => (
              <View key={title} style={supportStyles.faqCard}>
                <Text style={styles.cardTitle}>{title}</Text>
                <Text style={styles.helper}>{body}</Text>
              </View>
            ))}
...
function SupportQuickCard({
  ...
  return (
    <Pressable onPress={onPress} style={supportStyles.quickCard}>
    ...
    <Modal visible transparent animationType="slide" onRequestClose={onClose}>
      ...
      <SafeAreaView style={[chatStyles.sheet, { maxHeight: "82%" }]}>
        <SheetHeader
          ...
          onClose={onClose}
        />
        <View style={supportStyles.infoHero}>
          <PremiumIcon
            name={info.icon}
            tone={info.tone ?? "gold"}
            size={54}
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.helper}>{info.body}</Text>
          </View>
        </View>
        {info.bullets?.length ? (
          <View style={supportStyles.infoList}>
            {info.bullets.map((item) => (
              <View key={item} style={supportStyles.infoRow}>
                <MiniPremiumIcon
                  name="checkmark-circle"
                  tone="gold"
                  size={26}
                  iconSize={12}
                />
                <Text style={supportStyles.infoText}>{item}</Text>
              </View>
            ))}
          </View>
        ) : null}
...
function Pricing({
  ...
  const restorePurchases = () =>
    setRestoreStatus(
      buildRestorePreview(["DestinyOne Plus", "Social Weekend Pack"]),
    );
  return (
    <LinearGradient
      colors={["#260620", colors.black, colors.black]}
    ...
    >
      <SafeAreaView style={shared.safe}>
        ...
        >
          ...
          <View style={pricingStyles.billingToggle}>
            <Pressable
          ...
          <View style={pricingStyles.entitlementPanel}>
            <View style={shared.row}>
              <PremiumIcon
                name="layers-outline"
                tone="gold"
              ...
              />
              <View style={{ flex: 1, marginLeft: 10 }}>
                <Text style={styles.cardTitle}>Entitlement logic is ready</Text>
                <Text style={styles.helper}>
                  Every paid product maps to daily matches, filters, Spark
            ...
            return (
              <View
                ...
                style={[
                  ...
                  {
                    borderColor: accent,
                    backgroundColor: plan.recommended ? "#19160F" : "#20070D",
                  },
                ]}
              >
                <View style={shared.row}>
                  ...
                  <View style={{ flex: 1, marginLeft: 11 }}>
                    ...
                    <Text style={pricingStyles.planFor}>{plan.forLabel}</Text>
                  </View>
                ...
                <View style={pricingStyles.entitlementRow}>
                  {membershipEntitlementSummary(plan).map((item) => (
                    <View key={item} style={pricingStyles.entitlementPill}>
                      <Text style={pricingStyles.entitlementText}>{item}</Text>
                    </View>
                  ))}
                </View>
                {plan.features.map((x) => (
                ...
                <View style={launchStyles.secureRow}>
                  <MiniPremiumIcon
                    ...
                    iconSize={11}
                  />
                  <Text style={launchStyles.secureText}>
                    One tap · Restore anytime · Cancel in store settings
                  </Text>
                </View>
              </View>
            );
          })}
          <View style={pricingStyles.executiveCard}>
            <LinearGradient
            ...
            <View style={shared.row}>
              ...
              <View style={{ flex: 1, marginLeft: 12 }}>
                ...
                <Text style={pricingStyles.executiveTitle}>
                  {executivePlan.name}
                </Text>
                <Text style={pricingStyles.planFor}>
            ...
            <View style={pricingStyles.priceRow}>
              ...
              <Text style={styles.per}>{executivePlan.period}</Text>
            </View>
            {executivePlan.features.map((x) => (
              <View key={x} style={pricingStyles.featureRow}>
                <MiniPremiumIcon
                  name="checkmark-circle"
                  tone="gold"
          ...
          <View style={[aiStyles.roseWallet, { alignItems: "flex-start" }]}>
            ...
            <View style={{ flex: 1, gap: 7 }}>
              <Text style={aiStyles.roseTitle}>Golden Spark packs</Text>
              <Text style={aiStyles.roseBody}>
                After the daily free Spark, users can buy extra Spark packs
                through official store billing.
              </Text>
              <View style={pricingStyles.sparkGrid}>
                {sparkPacks.map((pack) => (
                  ...
                  >
                    <Text style={pricingStyles.sparkCount}>{pack.sparks}</Text>
                    <Text style={pricingStyles.sparkLabel}>Sparks</Text>
                    <Text style={pricingStyles.sparkPrice}>
          ...
          <View style={pricingStyles.restoreCard}>
            <PremiumIcon
              name="refresh-circle"
              tone="gold"
              size={44}
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={styles.helper}>{restoreStatus}</Text>
            </View>
            <Pressable
              onPress={restorePurchases}
              style={pricingStyles.restoreButton}
            >
              <Text style={pricingStyles.restoreText}>Restore</Text>
            </Pressable>
          </View>
          <View style={launchStyles.billingPromise}>
            <PremiumIcon
              name="shield-checkmark"
              tone="gold"
              size={44}
            ...
            />
            <View style={{ flex: 1 }}>
              <Text style={styles.cardTitle}>Clear payments, no surprises</Text>
              <Text style={styles.helper}>
          ...
          <Text style={[styles.legal, { paddingBottom: 10 }]}>
...
function MembershipCheckoutSheet({
  ...
  useEffect(() => {
    if (plan) setStage("idle");
  }, [plan]);
  if (!plan) return null;
  const steps = checkoutSteps(plan.kind, plan.executive);
  const activeStep =
    stage === "confirmed" ? steps.length - 1 : stage === "prepared" ? 1 : 0;
  const advance = () => {
    if (stage === "idle") {
      setStage("prepared");
      return;
    ...
    setStage("confirmed");
    if (plan.kind === "spark_pack") onComplete?.(plan.sparkAmount);
  };
  const buttonLabel =
    stage === "idle"
      ? plan.executive
        ? "Prepare executive application"
        : "Prepare secure checkout"
      : plan.executive
        ? "Submit application request"
        : plan.kind === "spark_pack"
          ? "Confirm Spark preview"
          : "Confirm preview unlock";
  const successCopy = plan.executive
    ? "Executive application request is ready. Approval must happen before billing."
    : plan.kind === "spark_pack"
      ? `${plan.sparkAmount ?? 0} Golden Sparks added in preview. Production validates the store receipt before unlocking.`
      : "Membership preview unlocked. Production will open Apple/Google billing here.";
  return (
    <Modal visible transparent animationType="slide" onRequestClose={onClose}>
      <Pressable style={chatStyles.modalBackdrop} onPress={onClose} />
      <SafeAreaView style={chatStyles.sheet}>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.helper}>
              {plan.price}
              {plan.period} · official app-store billing flow
            </Text>
          </View>
        </LinearGradient>
          ...
          <>
            <View style={pricingStyles.checkoutReady}>
              <MiniPremiumIcon
                ...
                iconSize={16}
              />
              <Text style={pricingStyles.checkoutReadyText}>{successCopy}</Text>
            </View>
            <Button label="Done" variant="secondary" onPress={onClose} />
          </>
        ) : (
        ...
        <Text style={styles.legal}>
          No real charge in this preview. Production connects app-store
          subscriptions, restore purchase, cancellation and receipts.
        </Text>
      </SafeAreaView>
    </Modal>
  );
...
function FormPage({
  ...
  return (
    <LinearGradient
      colors={["#20041B", colors.black, "#140311"]}
      locations={[0, 0.48, 1]}
      style={{ flex: 1 }}
    >
...
function Segment({
  ...
  return (
    ...
    >
      <Text style={[styles.segmentText, active && { color: colors.ivory }]}>
        {label}
      </Text>
    </Pressable>
  );
...
function Info({ title, body }: { title: string; body: string }) {
  return (
    <View style={{ gap: 8 }}>
      <Text style={styles.sectionLabel}>{title.toUpperCase()}</Text>
      <Text style={[shared.body, { color: "#D3CED6" }]}>{body}</Text>
    </View>
  );
}
      ...
      <View style={circleStyles.profileVouch}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={circleStyles.profileVouchBody}>
            People who know {match.name} describe her as:
          </Text>
          <View style={circleStyles.qualityWrap}>
...
function BottomNav({
  ...
  const items: [
    ...
    ["Matches", "heart", "home", "ruby"],
    ["Filters", "options", "discovery", "plum"],
    ["Coach", "sparkles", "coach", "gold"],
    ["Executive", "briefcase", "executive", "gold"],
    ["Likes", "heart-circle", "likes", "rose"],
    ["Chat", "chatbubble", "chat", "ruby"],
    ["Profile", "person", "profile", "dark"],
  ];
  return (
    <View accessibilityRole="tablist" style={bottomNavStyles.nav}>
      <ScrollView
        horizontal
        showsHorizontalScrollIndicator={false}
        contentContainerStyle={bottomNavStyles.navScroller}
      >
        {items.map(([label, icon, target, tone]) => {
          ...
          return (
            ...
            >
              {selected ? (
                <PremiumIcon name={icon} tone={tone} size={31} iconSize={15} />
              ) : (
                <PremiumIcon
              ...
              <Text
                style={[
                  ...
                  selected && bottomNavStyles.navTextOn,
                ]}
              >
                {label}
              </Text>
...
const selectorStyles = StyleSheet.create({
  ...
  suggestionPanel: {
    ...
    borderWidth: 1,
    borderColor: colors.line,
    backgroundColor: "rgba(27,9,13,.96)",
    overflow: "hidden",
  },
  suggestionRow: {
    minHeight: 42,
...
const bottomNavStyles = StyleSheet.create({
  nav: {
    ...
    paddingTop: 9,
    paddingBottom: 8,
    backgroundColor: "rgba(18,3,9,.97)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.12)",
    borderRadius: 30,
    shadowColor: "#FF2448",
...
const gameStyles = StyleSheet.create({
  hero: {
    padding: 14,
    borderRadius: 22,
    backgroundColor: "rgba(122,31,224,.10)",
    borderWidth: 1,
    borderColor: "rgba(122,31,224,.30)",
    flexDirection: "row",
    alignItems: "center",
    gap: 12,
  },
...
const trustHubStyles = StyleSheet.create({
  ...
  badgeCardOn: {
    backgroundColor: "rgba(212,175,55,.09)",
  ...
  actionButton: {
    ...
    paddingHorizontal: 12,
    borderRadius: 19,
    backgroundColor: "rgba(229,9,47,.16)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.38)",
    alignItems: "center",
    justifyContent: "center",
  },
  actionButtonText: {
  ...
  privacyPanel: {
    padding: 15,
    borderRadius: 22,
    backgroundColor: "rgba(212,175,55,.075)",
    borderWidth: 1,
    borderColor: "rgba(212,175,55,.24)",
    flexDirection: "row",
    alignItems: "center",
...
const adminOpsStyles = StyleSheet.create({
  ...
  backendLaunchCard: {
    ...
    padding: 15,
    borderRadius: 26,
    backgroundColor: "rgba(37,8,14,.94)",
    borderWidth: 1,
    borderColor: "rgba(212,175,55,.25)",
    shadowColor: colors.gold,
    shadowOpacity: 0.09,
  ...
  paymentEntitlementCard: {
    gap: 12,
    padding: 15,
    borderRadius: 26,
    backgroundColor: "rgba(55,7,16,.92)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.28)",
  ...
  notificationCard: {
    ...
    padding: 15,
    borderRadius: 26,
    backgroundColor: "rgba(34,9,39,.90)",
    borderWidth: 1,
    borderColor: "rgba(255,139,160,.22)",
    shadowColor: "#7A1FE0",
    shadowOpacity: 0.08,
    shadowRadius: 16,
  },
  giftFulfillmentCard: {
    ...
    padding: 15,
    borderRadius: 26,
    backgroundColor: "rgba(48,18,7,.92)",
    borderWidth: 1,
  ...
  placesReservationCard: {
    gap: 12,
    padding: 15,
    borderRadius: 26,
    backgroundColor: "rgba(18,35,32,.90)",
    borderWidth: 1,
    borderColor: "rgba(212,175,55,.24)",
    shadowColor: "#45C2A4",
    shadowOpacity: 0.08,
  ...
  observabilityCard: {
    ...
    padding: 15,
    borderRadius: 26,
    backgroundColor: "rgba(22,15,42,.90)",
    borderWidth: 1,
    borderColor: "rgba(139,117,255,.24)",
    shadowColor: "#8B75FF",
    shadowOpacity: 0.08,
    shadowRadius: 16,
  },
  abuseFraudCard: {
    ...
    padding: 15,
    borderRadius: 26,
    backgroundColor: "rgba(39,8,12,.92)",
    borderWidth: 1,
    borderColor: "rgba(255,107,130,.24)",
    shadowColor: "#FF4968",
    shadowOpacity: 0.08,
    shadowRadius: 16,
  },
  trustOpsCard: {
    ...
    padding: 15,
    borderRadius: 26,
    backgroundColor: "rgba(229,9,47,.06)",
    borderWidth: 1,
    borderColor: "rgba(212,175,55,.22)",
    shadowColor: colors.gold,
    shadowOpacity: 0.08,
  ...
  legalOpsCard: {
    ...
    padding: 15,
    borderRadius: 26,
    backgroundColor: "rgba(212,175,55,.065)",
    borderWidth: 1,
    borderColor: "rgba(212,175,55,.24)",
    shadowColor: colors.gold,
    shadowOpacity: 0.1,
  ...
  interactionCard: {
    ...
    padding: 15,
    borderRadius: 26,
    backgroundColor: "rgba(229,9,47,.055)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.22)",
    shadowColor: colors.pink,
    shadowOpacity: 0.08,
    shadowRadius: 16,
  },
  ...
  releaseCard: {
    ...
    padding: 15,
    borderRadius: 26,
    backgroundColor: "rgba(122,31,224,.055)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.10)",
    shadowColor: "#7A1FE0",
    shadowOpacity: 0.08,
  ...
  riskLow: {
    backgroundColor: "rgba(88,201,128,.22)",
    borderColor: "rgba(88,201,128,.36)",
  },
  riskMedium: {
    backgroundColor: "rgba(212,175,55,.24)",
    borderColor: "rgba(212,175,55,.38)",
  },
  riskHigh: {
    backgroundColor: "rgba(229,9,47,.30)",
    borderColor: "rgba(229,9,47,.48)",
  },
  riskCritical: {
    backgroundColor: "rgba(228,107,114,.40)",
    borderColor: "rgba(255,185,190,.58)",
  },
  ...
  autoPill: {
    ...
    paddingHorizontal: 10,
    borderRadius: 14,
    backgroundColor: "rgba(228,107,114,.12)",
    borderWidth: 1,
    borderColor: "rgba(228,107,114,.30)",
    alignItems: "center",
    justifyContent: "center",
  },
...
const profilePremiumStyles = StyleSheet.create({
  ...
  heroGlow: {
    ...
    top: -80,
    right: -70,
    backgroundColor: "rgba(229,9,47,.20)",
  },
  avatarHalo: {
    width: 118,
    height: 118,
...
const backendReadyStyles = StyleSheet.create({
  ...
  providerRow: {
    ...
    padding: 10,
    borderRadius: 16,
    backgroundColor: "rgba(212,175,55,.055)",
    borderWidth: 1,
...
const vibeStyles = StyleSheet.create({
  hero: {
    padding: 18,
    borderRadius: 28,
    backgroundColor: "rgba(229,9,47,.08)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.24)",
    alignItems: "center",
    gap: 10,
  },
...
const homeCleanStyles = StyleSheet.create({
  ...
  sideButton: {
    ...
    minHeight: 58,
    borderRadius: 20,
    backgroundColor: "rgba(27,4,10,.88)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.10)",
    alignItems: "center",
    justifyContent: "center",
  ...
  goldButton: {
    borderColor: "rgba(212,175,55,.35)",
    backgroundColor: "rgba(42,18,8,.9)",
  },
...
const growthLoopStyles = StyleSheet.create({
  ...
  commandCard: {
    ...
    padding: 15,
    borderRadius: 28,
    backgroundColor: "rgba(58,6,15,.72)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.10)",
    shadowColor: "#FF2448",
    shadowOpacity: 0.12,
  ...
  networkCard: {
    ...
    padding: 15,
    borderRadius: 28,
    backgroundColor: "rgba(24,5,9,.86)",
    borderWidth: 1,
    borderColor: "rgba(212,175,55,.18)",
    shadowColor: colors.gold,
    shadowOpacity: 0.1,
  ...
  challengeMeter: {
    ...
    height: 54,
    borderRadius: 27,
    backgroundColor: "rgba(229,9,47,.22)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.14)",
    alignItems: "center",
    justifyContent: "center",
  ...
  storyCard: {
    padding: 12,
    borderRadius: 19,
    backgroundColor: "rgba(229,9,47,.07)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.18)",
    flexDirection: "row",
    alignItems: "center",
    gap: 9,
  },
  ...
  retentionStat: {
    ...
    minHeight: 58,
    borderRadius: 18,
    backgroundColor: "rgba(12,1,4,.45)",
    borderWidth: 1,
  ...
  notificationCard: {
    padding: 13,
    borderRadius: 20,
    backgroundColor: "rgba(229,9,47,.075)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.20)",
    flexDirection: "row",
    alignItems: "center",
    gap: 10,
  },
...
const ventureStyles = StyleSheet.create({
  hero: {
    ...
    padding: 20,
    borderRadius: 30,
    backgroundColor: "rgba(229,9,47,.09)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.26)",
    overflow: "hidden",
  },
  heroBadge: {
    width: 64,
  ...
  priceSeal: {
    ...
    paddingVertical: 10,
    borderRadius: 22,
    backgroundColor: "rgba(212,175,55,.13)",
    borderWidth: 1,
    borderColor: "rgba(212,175,55,.36)",
    alignItems: "center",
  },
  priceSealText: {
    fontFamily: "Poppins_700Bold",
  ...
  errorCard: {
    padding: 10,
    borderRadius: 16,
    backgroundColor: "rgba(228,107,114,.10)",
    borderWidth: 1,
    borderColor: "rgba(228,107,114,.28)",
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
  },
...
const coachStyles = StyleSheet.create({
  ...
  homeCard: {
    ...
    borderRadius: 24,
    padding: 14,
    backgroundColor: "rgba(37,8,14,.92)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.09)",
    gap: 8,
  },
  ...
  savedNote: {
    padding: 10,
    borderRadius: 16,
    backgroundColor: "rgba(88,201,128,.10)",
    borderWidth: 1,
    borderColor: "rgba(88,201,128,.28)",
    flexDirection: "row",
    alignItems: "center",
    gap: 8,
  },
  ...
  marketReadinessCard: {
    ...
    backgroundColor: "rgba(212,175,55,.065)",
    borderWidth: 1,
    borderColor: "rgba(212,175,55,.23)",
    shadowColor: colors.gold,
    shadowOpacity: 0.1,
    shadowRadius: 15,
  },
  ...
  marketPillarOn: {
    backgroundColor: "rgba(212,175,55,.055)",
    borderColor: "rgba(212,175,55,.17)",
  },
  marketPillarTitle: {
    fontFamily: "Poppins_700Bold",
    fontSize: 10.8,
  ...
  launchEventPill: {
    ...
    paddingVertical: 5,
    borderRadius: 13,
    backgroundColor: "rgba(229,9,47,.11)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.24)",
  },
  launchEventText: {
  ...
  eventType: {
    ...
    paddingVertical: 5,
    borderRadius: 12,
    backgroundColor: "rgba(229,9,47,.13)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.30)",
  },
  eventTypeText: {
    fontFamily: "Poppins_700Bold",
    fontSize: 8,
  ...
  suggestionChip: {
    ...
    paddingVertical: 7,
    borderRadius: 16,
    backgroundColor: "rgba(229,9,47,.10)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.22)",
  },
  suggestionText: {
...
const supportStyles = StyleSheet.create({
  ...
  liveStatus: {
    ...
    paddingVertical: 6,
    borderRadius: 16,
    backgroundColor: "rgba(105,187,138,.08)",
    borderWidth: 1,
    borderColor: "rgba(105,187,138,.24)",
  },
  liveDot: {
    width: 9,
    height: 9,
...
const styles = StyleSheet.create({
  ...
  resetButton: {
    ...
    borderRadius: radius.md,
    borderWidth: 1,
    borderColor: "rgba(228,107,114,.35)",
    backgroundColor: "rgba(228,107,114,.06)",
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    gap: 9,
  ...
  alignmentRow: { flexDirection: "row", alignItems: "center", gap: 12 },
  alignmentRowIcon: {
    width: 36,
    height: 36,
    borderRadius: 18,
    backgroundColor: "#3D1237",
    alignItems: "center",
    justifyContent: "center",
  },
  alignmentRowLabel: {
  ...
  welcomeGlowOne: {
    ...
    height: 270,
    borderRadius: 150,
    backgroundColor: "rgba(229,9,47,.17)",
    top: 70,
    right: -100,
  },
  welcomeGlowTwo: {
  ...
  sparkOne: {
    ...
    height: 39,
    borderRadius: 20,
    backgroundColor: "rgba(229,9,47,.12)",
    alignItems: "center",
    justifyContent: "center",
  },
  valueTag: {
    ...
    paddingVertical: 7,
    borderRadius: 20,
    backgroundColor: "rgba(30,6,10,.9)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.45)",
  },
  valueTagText: {
    fontFamily: "Poppins_600SemiBold",
    fontSize: 9,
  ...
  resend: { alignSelf: "center", padding: 10 },
  resendText: {
    fontFamily: "Poppins_600SemiBold",
    fontSize: 13,
  ...
  glow: {
    ...
    height: 230,
    borderRadius: 115,
    backgroundColor: "rgba(229,9,47,.15)",
  },
  tagline: {
    fontFamily: "Poppins_600SemiBold",
    fontStyle: "italic",
  ...
  welcomeTop: { paddingTop: 8, flexDirection: "row", alignItems: "center" },
  welcomeArt: { height: 285, justifyContent: "center" },
  orbit: {
    position: "absolute",
    alignSelf: "center",
    width: 245,
    ...
    borderRadius: 130,
    borderWidth: 1,
    borderColor: "rgba(255,138,152,.24)",
  },
  photoMini: {
    position: "absolute",
    width: 142,
    ...
    overflow: "hidden",
    borderWidth: 2,
    borderColor: "rgba(255,110,128,.65)",
    shadowColor: colors.pink,
    shadowOpacity: 0.22,
    shadowRadius: 18,
  },
  fill: { width: "100%", height: "100%" } as ImageStyle,
  heart: {
    position: "absolute",
    alignSelf: "center",
    width: 54,
  ...
  segment: {
    height: 48,
    backgroundColor: "rgba(37,9,15,.9)",
  ...
  segmentActive: { backgroundColor: "#6F1627" },
  segmentText: {
    fontFamily: "Poppins_600SemiBold",
    fontSize: 13,
    color: colors.muted,
  ...
  upload: {
    ...
    borderWidth: 1,
    borderColor: colors.line,
    backgroundColor: "rgba(32,8,13,.72)",
  },
  ...
  photoNum: {
    ...
    height: 21,
    borderRadius: 11,
    backgroundColor: "rgba(13,3,12,.8)",
    alignItems: "center",
    justifyContent: "center",
  },
  photoNumText: {
  ...
  vibeCard: {
    ...
    borderColor: colors.line,
    borderRadius: radius.md,
    backgroundColor: "rgba(45,13,19,.88)",
    padding: 13,
    justifyContent: "space-between",
  },
  vibeSelected: { borderColor: colors.pink, backgroundColor: "#5D0A1A" },
  vibeText: {
    fontFamily: "Poppins_600SemiBold",
    fontSize: 13,
    color: colors.muted,
  ...
  intentSelected: { borderColor: colors.pink, backgroundColor: "#5D0A1A" },
  intentIcon: {
    width: 45,
    height: 45,
    borderRadius: 23,
    backgroundColor: "#470D18",
    alignItems: "center",
    justifyContent: "center",
  },
  radio: {
  ...
  curated: {
    ...
    paddingVertical: 11,
    borderRadius: radius.md,
    backgroundColor: "rgba(78,9,23,.45)",
    borderWidth: 1,
    borderColor: "#6E2463",
  },
  curatedText: {
  ...
  matchTop: { position: "absolute", top: 17, left: 17 },
  matchInfo: { position: "absolute", left: 19, right: 19, bottom: 18, gap: 9 },
  matchName: {
    fontFamily: "Poppins_700Bold",
    fontSize: 29,
    color: colors.ivory,
  ...
  nope: {
    ...
    height: 54,
    borderRadius: 27,
    backgroundColor: "rgba(27,16,27,.92)",
    borderWidth: 1,
    borderColor: colors.line,
    alignItems: "center",
    justifyContent: "center",
  ...
  yesText: { fontFamily: "Poppins_700Bold", fontSize: 14, color: colors.ivory },
  hero: { height: 580 },
  circleBtn: {
    margin: 16,
    width: 43,
    height: 43,
    borderRadius: 22,
    backgroundColor: "rgba(13,3,12,.72)",
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 1,
    borderColor: colors.line,
  ...
  detailBlockButton: {
    ...
    height: 43,
    borderRadius: 22,
    backgroundColor: "rgba(70,4,13,.80)",
    alignItems: "center",
    justifyContent: "center",
    borderWidth: 1,
    borderColor: "rgba(255,100,117,.35)",
  },
  heroText: { position: "absolute", left: 21, right: 21, bottom: 27, gap: 9 },
  detailName: {
    fontFamily: "Poppins_700Bold",
    fontSize: 36,
    color: colors.ivory,
  },
  detailBody: { padding: 22, gap: 29 },
  profileViewNotice: {
    padding: 13,
    borderRadius: 18,
    backgroundColor: "rgba(212,175,55,.08)",
  ...
  privateBlockCard: {
    padding: 15,
    borderRadius: 22,
    backgroundColor: "rgba(228,107,114,.08)",
    borderWidth: 1,
    borderColor: "rgba(228,107,114,.25)",
    flexDirection: "row",
    alignItems: "center",
    gap: 12,
  },
  ...
  privateBlockAction: {
    ...
    paddingHorizontal: 13,
    borderRadius: 18,
    backgroundColor: "rgba(255,100,117,.14)",
    borderWidth: 1,
    borderColor: "rgba(255,100,117,.35)",
    alignItems: "center",
    justifyContent: "center",
  ...
  wave: { height: 27, flexDirection: "row", alignItems: "center", gap: 3 },
  sectionLabel: {
    fontFamily: "Poppins_700Bold",
    fontSize: 10,
    letterSpacing: 1.6,
  ...
  fixedAction: {
    ...
    paddingVertical: 13,
    paddingBottom: 24,
    backgroundColor: "rgba(13,3,12,.96)",
    borderTopWidth: 1,
    borderTopColor: colors.line,
    flexDirection: "row",
    gap: 11,
  },
  matchFaces: { height: 145, width: 245 },
  face: {
    position: "absolute",
    width: 142,
    height: 142,
  ...
  messages: { flexGrow: 1, padding: 18, gap: 14 },
  iceReveal: {
    alignSelf: "center",
    alignItems: "center",
    gap: 7,
  ...
  nav: {
    ...
    height: 72,
    paddingTop: 11,
    backgroundColor: "rgba(27,8,24,.96)",
    borderWidth: 1,
    borderColor: "#531522",
    borderRadius: 25,
    flexDirection: "row",
  ...
  likesGrid: { flexDirection: "row", gap: 12 },
  likeCard: {
    flex: 1,
    height: 230,
    borderRadius: radius.lg,
  ...
  likeLock: {
    ...
    height: 42,
    borderRadius: 21,
    backgroundColor: "rgba(13,3,12,.75)",
    alignItems: "center",
    justifyContent: "center",
  },
  likeText: {
  ...
  per: { fontFamily: "Poppins_400Regular", fontSize: 12, color: colors.muted },
  popular: {
    paddingHorizontal: 10,
    paddingVertical: 6,
    borderRadius: 20,
...
const chatPremiumStyles = StyleSheet.create({
  ...
  composer: {
    ...
    backgroundColor: "rgba(12,2,5,.96)",
  },
  chatInput: {
    height: 43,
...
const swipeStyles = StyleSheet.create({
  ...
  matchSwipeHint: {
    ...
    paddingVertical: 6,
    borderRadius: 15,
    backgroundColor: "rgba(13,3,12,.66)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.12)",
  },
  matchSwipeHintText: {
  ...
  swipeYes: {
    left: 24,
    borderColor: "#8EE0AA",
    backgroundColor: "rgba(40,130,75,.18)",
  },
  swipeNope: {
    right: 24,
    borderColor: "#E46B72",
    backgroundColor: "rgba(228,107,114,.16)",
    transform: [{ rotate: "10deg" }],
  },
  swipeLabel: {
    fontFamily: "Poppins_700Bold",
  ...
  swipeRose: {
    ...
    paddingVertical: 11,
    borderRadius: 24,
    backgroundColor: "rgba(70,0,15,.75)",
    borderWidth: 1,
    borderColor: colors.pink,
  },
  ...
  profileSummary: {
    ...
    alignItems: "stretch",
    borderRadius: 18,
    backgroundColor: "rgba(12,2,6,.46)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.10)",
    overflow: "hidden",
  },
  ...
  reasonCard: {
    ...
    padding: 10,
    borderRadius: 18,
    backgroundColor: "rgba(212,175,55,.085)",
    borderWidth: 1,
    borderColor: "rgba(212,175,55,.22)",
  },
  reasonTitle: {
...
const giftFlowStyles = StyleSheet.create({
  quoteCard: {
    ...
    padding: 14,
    borderRadius: 22,
    backgroundColor: "rgba(36,8,13,.96)",
    borderWidth: 1,
    borderColor: "rgba(212,175,55,.26)",
    shadowColor: colors.gold,
    shadowOpacity: 0.12,
...
const pricingStyles = StyleSheet.create({
  ...
  executiveCard: {
    ...
    borderRadius: 30,
    borderWidth: 1,
    borderColor: "rgba(245,212,106,.45)",
    backgroundColor: "#23190A",
    overflow: "hidden",
    shadowColor: colors.gold,
    shadowOpacity: 0.18,
    shadowRadius: 22,
  ...
  sparkCardBest: {
    borderColor: "rgba(212,175,55,.42)",
    backgroundColor: "rgba(212,175,55,.10)",
  },
  sparkCount: {
    fontFamily: "Poppins_700Bold",
...
const callStyles = StyleSheet.create({
  ...
  callActionFrameDanger: {
    backgroundColor: "rgba(228,107,114,.12)",
    borderColor: "rgba(228,107,114,.34)",
  },
  callActionIcon: {
    width: 58,
    height: 58,
...
const aiStyles = StyleSheet.create({
  ...
  aiPill: {
    ...
    paddingVertical: 6,
    borderRadius: 14,
    backgroundColor: "rgba(229,9,47,.14)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.3)",
  },
  aiPillText: {
    fontFamily: "Poppins_600SemiBold",
    fontSize: 9.5,
  ...
  roseAction: {
    ...
    height: 56,
    borderRadius: 28,
    backgroundColor: "rgba(62,36,8,.92)",
    borderWidth: 1,
...
const launchStyles = StyleSheet.create({
  ...
  petal: {
    ...
    color: "#FF7D91",
    opacity: 0.72,
    textShadowColor: "rgba(229,9,47,.65)",
    textShadowRadius: 12,
  },
  velvetGlowTop: {
    position: "absolute",
  ...
  velvetGlowBottom: {
    ...
    height: 430,
    borderRadius: 215,
    backgroundColor: "rgba(120,0,24,.16)",
  },
  logoFrame: {
    width: 230,
    height: 230,
  ...
  preloadBrand: {
    ...
    color: colors.ivory,
    marginTop: 22,
    textShadowColor: "rgba(229,9,47,.32)",
    textShadowRadius: 12,
  },
  ...
  reservedPill: {
    minHeight: 45,
    borderRadius: 22,
    backgroundColor: "rgba(67,148,95,.15)",
    borderWidth: 1,
    borderColor: "rgba(121,214,155,.4)",
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    gap: 7,
...
const chatStyles = StyleSheet.create({
  ...
  safetyNudgeButton: {
    ...
    backgroundColor: "rgba(229,9,47,.16)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.35)",
  },
  safetyNudgeButtonText: {
    fontFamily: "Poppins_700Bold",
    fontSize: 9,
    color: colors.pinkSoft,
  },
  attachmentTray: {
    ...
    backgroundColor: "rgba(15,3,7,.98)",
  },
  ...
  emojiPanel: {
    ...
    borderTopWidth: 1,
    borderTopColor: "rgba(255,255,255,.07)",
    backgroundColor: "rgba(13,3,7,.98)",
    paddingTop: 10,
  },
  emojiHeader: {
    paddingHorizontal: 18,
  ...
  gifBadge: {
    ...
    paddingVertical: 3,
    borderRadius: 6,
    backgroundColor: "rgba(13,3,12,.7)",
  },
  gifBadgeText: {
    fontFamily: "Poppins_700Bold",
    fontSize: 9,
  ...
  locationBubble: {
    width: 255,
    padding: 7,
    backgroundColor: "rgba(71,13,25,.96)",
    borderWidth: 1,
    borderColor: "rgba(212,175,55,.25)",
    overflow: "hidden",
  },
  ...
  gifTitle: {
    ...
    fontSize: 10,
    color: colors.ivory,
    backgroundColor: "rgba(13,3,12,.65)",
    paddingHorizontal: 7,
    paddingVertical: 4,
    borderRadius: 7,
  },
  ...
  giftCard: {
    ...
    minHeight: 216,
    borderRadius: 22,
    backgroundColor: "rgba(32,8,13,.92)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.10)",
    padding: 9,
    alignItems: "center",
...
const circleStyles = StyleSheet.create({
  homeBanner: {
    ...
    borderRadius: 20,
    padding: 13,
    backgroundColor: "rgba(96,12,28,.55)",
    borderWidth: 1,
    borderColor: "#801B34",
    flexDirection: "row",
    alignItems: "center",
  ...
  qualityPill: {
    ...
    paddingVertical: 6,
    borderRadius: 14,
    backgroundColor: "rgba(229,9,47,.1)",
    borderWidth: 1,
    borderColor: "rgba(229,9,47,.28)",
  },
  qualityText: {
  ...
  boundaryCard: {
    padding: 14,
    borderRadius: 18,
    backgroundColor: "rgba(51,11,19,.65)",
    flexDirection: "row",
    alignItems: "center",
    gap: 10,
  },
...
const discoveryStyles = StyleSheet.create({
  ...
  clearButton: {
    ...
    borderRadius: 15,
    borderWidth: 1,
    borderColor: "rgba(228,107,114,.3)",
    flexDirection: "row",
    alignItems: "center",
    justifyContent: "center",
    gap: 7,
...
const dateStyles = StyleSheet.create({
  ...
  planStep: {
    ...
    minHeight: 39,
    borderRadius: 17,
    backgroundColor: "rgba(12,2,6,.36)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.08)",
    flexDirection: "row",
    alignItems: "center",
  ...
  timeChip: {
    ...
    minHeight: 46,
    borderRadius: 18,
    backgroundColor: "rgba(255,255,255,.052)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.10)",
    alignItems: "center",
    justifyContent: "center",
  ...
  safePill: {
    ...
    paddingVertical: 5,
    borderRadius: 12,
    backgroundColor: "rgba(88,201,128,.12)",
    marginTop: 4,
  },
  safePillText: {
    fontFamily: "Poppins_600SemiBold",
...
const safetyStyles = StyleSheet.create({
  ...
  confirmCard: {
    ...
    backgroundColor: "rgba(228,107,114,.08)",
    borderWidth: 1,
    borderColor: "rgba(228,107,114,.26)",
    flexDirection: "row",
    alignItems: "center",
    gap: 12,
  },
...
const noticeStyles = StyleSheet.create({
  sheet: {
    ...
    padding: 16,
    gap: 14,
    backgroundColor: "rgba(22,4,9,.98)",
    borderWidth: 1,
    borderColor: "rgba(255,255,255,.10)",
    shadowColor: colors.pink,
    shadowOpacity: 0.22,
